...
//#define AP_UART0_PHY_BASE   0x11002000
#define REG_MT6795_UART_BASE			0x00
#define REG_MT6795_UART_STATUS		0x14
static void mt5795_printch(char ch)
{
	...
	status = readl_relaxed(early_base+REG_MT6795_UART_STATUS);
	while (! (status & 0x20))
		status = readl_relaxed(early_base+REG_MT6795_UART_STATUS);

	writel_relaxed(ch & 0xFF, early_base+REG_MT6795_UART_BASE);
}
...
static const struct earlycon_match earlycon_match[] __initconst = {
	...
	{ .name = "mt6795", .printch = mt5795_printch, },
	{}
};
...
static int __init setup_early_printk(char *buf)
{
	...
	pr_info("earlyprintk arguments: %s\n", buf);
