...
extern void temperature_poll_control(bool en);
...
static int bat_update_time = BAT_TASK_PERIOD;
static int retry_count = 0;
#define BAT_RETRY_TIMES	5
...
static kal_bool temperature_debug = KAL_FALSE;
static INT32	temperature_set = 25;;
				...
				enum power_supply_property psp, union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_BatteryAverageCurrent:
		...
		if (current_sign == KAL_TRUE)
			val->intval = battery_meter_get_battery_current() / 10;
		else
			val->intval = 0 - (battery_meter_get_battery_current() / 10);
		break;
...
PMU_STATUS do_batt_temp_state_machine(void)
{
	...
	if (BMT_status.temperature < MIN_CHARGE_TEMPERATURE) {
		printk("[BATTERY] Battery Under Temperature 0OC or NTC fail !!\n\r");
		g_batt_temp_status = TEMP_POS_LOW;
		return PMU_STATUS_FAIL;
	} else if (g_batt_temp_status == TEMP_POS_LOW) {
		if ((BMT_status.temperature >= MIN_CHARGE_TEMPERATURE) && (BMT_status.temperature < T10_CHARGE_TEMPERATURE)){
			printk("[BATTERY] Battery Temperature raise from %d to %d, allow charging!!\n\r",
					    MIN_CHARGE_TEMPERATURE, BMT_status.temperature);
			g_batt_temp_status = TEMP_POS_T10;
			BMT_status.bat_charging_state = CHR_PRE;
			return PMU_STATUS_OK;
		} else if ((BMT_status.temperature >= MIN_CHARGE_TEMPERATURE) && (BMT_status.temperature < MAX_CHARGE_TEMPERATURE)) {
			printk("[BATTERY] Battery Temperature raise from %d to %d, allow charging!!\n\r",
					    MIN_CHARGE_TEMPERATURE, BMT_status.temperature);
...
#endif
	/* add charging temperature 10oC, between 0<=T<10oC set charging current 0.3C*/
	if (BMT_status.temperature < T10_CHARGE_TEMPERATURE) {
		if (g_batt_temp_status == TEMP_POS_T10) {
		    return PMU_STATUS_OK;
		}
		...
		BMT_status.bat_charging_state = CHR_PRE;
		return PMU_STATUS_OK;
	} else if (g_batt_temp_status == TEMP_POS_T10) {
		if ((BMT_status.temperature >= T10_CHARGE_TEMPERATURE) && (BMT_status.temperature < MAX_CHARGE_TEMPERATURE))
		{
			printk("[BATTERY] Battery Temperature raise from %d to %d, charging current rising 0.5C!!\n\r", 
					T10_CHARGE_TEMPERATURE,BMT_status.temperature); 
			g_batt_temp_status = TEMP_POS_NORMAL;
			BMT_status.bat_charging_state = CHR_PRE;
			return PMU_STATUS_OK;
		}
	...
	if (BMT_status.temperature >= MAX_CHARGE_TEMPERATURE) {
		printk("[BATTERY] Battery Over Temperature 45oc!!\n\r");
...
void mt_battery_GetBatteryData(void)
{
	...
	printk("[kernel]AvgVbat %d,bat_vol %d, AvgI %d, I %d, VChr %d, AvgT %d, T %d, ZCV %d, SOC %d\n",
			    BMT_status.bat_vol, bat_vol, BMT_status.ICharging, ICharging,
			    BMT_status.charger_vol, BMT_status.temperature, temperature,
			    BMT_status.ZCV, BMT_status.UI_SOC2);
}

static PMU_STATUS mt_battery_CheckUsbTemp(void)
{
	...
	if (state == MOS_OFF) {
		printk("%s:usb temp is High, so discharging\n", __func__);
		status = PMU_STATUS_FAIL;
	} else if (state == MOS_ON) {
		battery_log(BAT_LOG_FULL, "%s:usb temp is Normal, enable charging\n", __func__);
...
static void mt_battery_CheckBatteryStatus(void)
{
	...
	if (mt_battery_CheckUsbTemp() != PMU_STATUS_OK) {
		BMT_status.bat_charging_state = CHR_ERROR;
		return;
	}
...
int bat_routine_thread(void *x)
{
	...
	while (1) {
		...
		if ((BMT_status.charger_exist == KAL_TRUE) && (retry_count <= BAT_RETRY_TIMES) &&
			...
			bat_update_time = BAT_TASK_PERIOD;
		}
		...
		ktime = ktime_set(bat_update_time, 0);	/* 10s, 10* 1000 ms */
