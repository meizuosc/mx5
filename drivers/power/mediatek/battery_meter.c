...
kal_int32 g_Q_MAX_POS_50[] = {3095, 3095, 3095, 3095};
kal_int32 g_Q_MAX_POS_25[] = {3063, 3063, 3063, 3063};
kal_int32 g_Q_MAX_POS_0[] = {2890, 2890, 2890, 2890};
kal_int32 g_Q_MAX_NEG_10[] = {2790, 2790, 2790, 2790};

kal_int32 g_Q_MAX_POS_50_H_CURRENT[] = {3079, 3079, 3079, 3079};
kal_int32 g_Q_MAX_POS_25_H_CURRENT[] = {3025, 3025, 3025, 3025};
kal_int32 g_Q_MAX_POS_0_H_CURRENT[] = {2771, 2771, 2771, 2771};
kal_int32 g_Q_MAX_NEG_10_H_CURRENT[] = {2139, 2139, 2139, 2139};
...
//	ret = IMM_GetOneChannelValue_Cali(AUXADC_BATTERY_ID_CHANNEL, &id_volt);
	ret = IMM_GetOneChannelValue(AUXADC_BATTERY_ID_CHANNEL, data, &rawdata);
	
	if (ret != 0)
		bm_print(BM_LOG_CRTI, "[fgauge_get_profile_id]id_volt read fail\n");
	else
		printk("[fgauge_get_profile_id]battery_id rawdata = %d\n", rawdata);

	for (id = 0; id < TOTAL_BATTERY_NUMBER; id++) {
		if (rawdata < g_battery_id_voltage[id]) {
			g_fg_battery_id = id;
			break;
...
void dod_init(void)
{
	...
	printk("tangxingyan[FGADC] get_hw_ocv=%d, HW_SOC=%d, SW_SOC = %d\n",
	...
	gFG_capacity_by_v = gFG_capacity_by_v_init;
	printk("tangxingyan[FGADC] HW_FG_FORCE_USE_SW_OCV : HW_SOC=%d, SW_SOC = %d\n",
		 gFG_capacity_by_v, gFG_capacity_by_v_init);
#endif
	/* ------------------------------------------------------------------------------- */
#endif
	...
	printk("tangxingyan [FGADC] g_rtc_fg_soc=%d, gFG_capacity_by_v=%d\n",
		 g_rtc_fg_soc, gFG_capacity_by_v);
...
/* 4. voltage mode */
	...
	printk("tangxingyan %s:gFG_capacity_by_v %d, gFG_capacity_by_c %d\n", __func__,
			gFG_capacity_by_v, gFG_capacity_by_c);
...
void fgauge_initialization(void)
{
	...
	gFG_DOD0 = 100 - gFG_capacity;
	printk("tangxingyan [fgauge_initialization] gFG_DOD0 =%d gFG_capacity = %d \n",gFG_DOD0,gFG_capacity);
	...
	printk("tangxingyan %s:Done HW_OCV:%d FG_Current:%d FG_CAR:%d tmp=%d capacity=%d Qmax=%d\n",
		__func__, gFG_voltage,gFG_current,gFG_columb,gFG_temp,gFG_capacity,gFG_BATT_CAPACITY);
