				...
				enum power_supply_property psp, union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_BatteryAverageCurrent:
		val->intval = battery_meter_get_charging_current_imm();
		break;
...
void mt_battery_GetBatteryData(void)
{
	...
	printk("AvgVbat=(%d),bat_vol=(%d),AvgI=(%d),I=(%d),VChr=(%d),AvgT=(%d),T=(%d),pre_SOC=(%d),SOC=(%d),ZCV=(%d)\n",
...
static void battery_timer_resume(void)
{
	...
	if(g_call_state == CALL_ACTIVE &&
			(bat_time_after_sleep.tv_sec - g_bat_time_before_sleep.tv_sec >= TALKING_SYNC_TIME))	// phone call last than x min
	{
		BMT_status.UI_SOC = battery_meter_get_battery_percentage();
