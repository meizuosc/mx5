...
#define DIFFERENCE_HWOCV_RTC		100  // 30% difference
#endif
...
//	bm_print(BM_LOG_CRTI, "[Netlink] sizeof(struct fgd_nl_msg_t) %d\n", (int)sizeof(struct fgd_nl_msg_t));
	...
	switch (msg->fgd_cmd) {
		...
		case FG_DAEMON_CMD_SET_SOC:
		{
			...
			printk("[fg_res] gFG_capacity_by_c SOC = %d\n", gFG_capacity_by_c);
