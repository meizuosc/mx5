...
extern void inv_mpu_enable_irq(bool enable);
#define INV_INFO(fmt,args...)  printk("20645:%s " fmt,__func__,##args)
...
#define MAX_COMPASS_RATE         70
#define MIN_COMPASS_RATE         5
...
#define PRESSURE_WAKE_HDR        0x8100
#define STEP_COUNTER_HDR         0x8800
#define STEP_COUNTER_WAKE_HDR    0x8880
#define ACCEL_HDR                0x4000
#define ACCEL_WAKE_HDR           0x4100
#define GYRO_HDR                 0x2000
#define GYRO_WAKE_HDR            0x2100
#define COMPASS_HDR              0x1000
#define COMPASS_WAKE_HDR         0x1100
#define COMPASS_HDR_2            0x1800
#define ALS_HDR                  0x0800
#define ALS_WAKE_HDR             0x0810
#define SIXQUAT_HDR              0x0400
#define PEDQUAT_HDR              0x0200
#define STEP_DETECTOR_HDR        0x0100
#define STEP_DETECTOR_WAKE_HDR   0x0110
#define ACTIVITY_HDR             0x0180
#define PICK_UP_HDR              0x0280
 ...
 *  @time_calib_counter: calibration counter for timestamp.
 */
struct inv_mpu_state {
	struct device *dev;
	...
	struct semaphore suspend_resume_sema;
	...
	int aut_key_in;
	int aut_key_out;
	int time_calib_counter;
#if SUSPEND_DISABLE_IRQ
	atomic_t acc_wake;
	int irq_should_disable;
#ifdef	CONFIG_HAS_EARLYSUSPEND
	struct early_suspend sensor_early_suspend;
#endif
#endif
	struct mutex inv_rw_mutex;
	struct mutex inv_suspend_mutex;
	u8 irqreg0;
	u8 irqreg1;
	u8 irqreg2;
	u8 irqreg3;
...
int inv_push_16bytes_buffer(struct inv_mpu_state *st, u16 hdr, u64 t, int *q);
int inv_push_16bytes_buffer_compass(struct inv_mpu_state *st, u16 hdr,
								u64 t, int *q);
...
int inv_send_steps(struct inv_mpu_state *st,  u32 step, u64 t);
