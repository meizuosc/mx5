...
#define ENABLE_LSB_CONERT_TO_DMP
	...
	printk(KERN_ERR" invn: accel bias is %d %d %d\n", st->accel_st_bias[0],
									st->accel_st_bias[1], st->accel_st_bias[2]);
	if( st->accel_st_bias[0]==0 )
		st->accel_st_bias[0]++;
	if( st->accel_st_bias[1]==0 )
		st->accel_st_bias[1]++;
	if( st->accel_st_bias[2]==0 )
		st->accel_st_bias[2]++;

	printk(KERN_ERR" invn: accel bias is %d %d %d after\n", st->accel_st_bias[0],
									st->accel_st_bias[1], st->accel_st_bias[2]);

	st->accel_st_bias[0] = st->accel_st_bias[0] * (1 << 16);
	st->accel_st_bias[1] = st->accel_st_bias[1] * (1 << 16);
	st->accel_st_bias[2] = ((st->accel_st_bias[2]  > 0) ? (st->accel_st_bias[2] - 16384) : (st->accel_st_bias[2]  + 16384)) * (1 << 16);//16384=2^14
