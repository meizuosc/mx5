...
{
	int result;
	unsigned char averaging = 1; //no sample averaging
	...
	if (result)
		return result;
    result = inv_plat_read(st, REG_GYRO_CONFIG_2, 1, &gyro_config_2_reg);
	if (result)
		return result;
    switch(averaging) {
    case 1:
	...
	result = inv_plat_single_write(st, REG_GYRO_CONFIG_2, gyro_config_2_reg);
    if (result)
		return result;
...
{
	int result;
//--yd 	unsigned char averaging = 1; //no sample averagin
	unsigned char averaging = 8; //16 sample averaging
    ...
    if(averaging > 1)
        accel_config_1_reg |= (7 << 3) | (st->chip_config.accel_fs << 1) | 1;   //fchoice = 1, filter = 7.
    else
        accel_config_1_reg |= (st->chip_config.accel_fs << 1);  //fchoice = 0, filter = 0.
