...
extern void inv_mpu_init_irq(void) ;
extern int inv_mpu_get_irq(void);
...
int inv_send_steps(struct inv_mpu_state *st, u32 step, u64 ts)
{

	unsigned short s[3];

	s[0] = 0;
	s[1] = (unsigned short)(step & 0xffff);
	s[2] = (unsigned short)((step >> 16) & 0xffff);
	...
	if (st->step_counter_wake_l_on)
		inv_push_special_8bytes_buffer(st, STEP_COUNTER_WAKE_HDR,
								ts, s);
	INV_INFO("stepcounter step:0x%x--%u,s1:0x%x,s2:0x%x,sizeof(short):%lu\n",step,step,s[1],s[2],sizeof(short));
	return 0;
}
...
int inv_mpu_configure_ring(struct iio_dev *indio_dev)
{
	...
	if(node) {
		st->irq = irq_of_parse_and_map(node,0);
	}
	else {
		st->irq = inv_mpu_get_irq();
	}
#else
	st->irq = inv_mpu_get_irq();
#endif
	...
	ret = request_threaded_irq(st->irq, NULL/*inv_irq_handler*/,
			inv_read_fifo, IRQF_TRIGGER_RISING|IRQF_ONESHOT, "inv_irq", st);
	if (ret) {
		INV_INFO("sensor req threaded irq:%d failed\n",st->irq);
