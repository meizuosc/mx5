...
static int mmc_rescan_try_freq(struct mmc_host *host, unsigned freq)
{
	...
	if (!mmc_attach_sdio(host)){			
	    printk("%s:attach sdio success\n",__FUNCTION__);
		return 0;
	}
	if (!mmc_attach_sd(host)){		
		printk("%s:attach sdcard success\n",__FUNCTION__);
		return 0;
	}
	if (!mmc_attach_mmc(host)){		
		printk("%s:attach mmc success\n",__FUNCTION__);
		return 0;
	}
...
extern int msdc1_poll_sdcard_eint(void) ;
  if(host->pm_flags&MMC_PM_SDCARD_POWER){
	host->pm_flags &=~MMC_PM_SDCARD_POWER ;
   if(!msdc1_poll_sdcard_eint()){
	printk(KERN_ERR"%s:MMC host1 slot not sdcard\n",__FUNCTION__);
	return  ;
  }
}
#endif
