...
struct i2c_client *sClient = NULL;


unsigned char motor_read_byte(unsigned char cmd)
{
...
EXPORT_SYMBOL(motor_read_byte); //Add by Immersion/Arvin, for ImmVibeSPI.c, demo purpose


int motor_write_byte(unsigned char cmd, unsigned char writeData)
{
    char    write_data[2] = {0};
    int     ret=0;    
    write_data[0] = cmd;
...
EXPORT_SYMBOL(motor_write_byte); //Add by Immersion/Arvin, for ImmVibeSPI.c, demo purpose
...
void get_motor_standby(void)
{
	mt_set_gpio_out(GPIO_MOTOR_ON_PIN,GPIO_OUT_ZERO);
}
EXPORT_SYMBOL(get_motor_standby); //Add by Immersion/Arvin, for ImmVibeSPI.c, demo purpose

void get_motor_ready(void)
{
...
EXPORT_SYMBOL(get_motor_ready); //Add by Immersion/Arvin, for ImmVibeSPI.c, demo purpose
...
//	get_motor_ready();
	...
	switch( data&0xe0 ) {
		...
		case 0xe0:
			check_status = CHECK_IN;
			printk("[motor] %s this is DRV2605L\n",__func__);
			break;
		default:
...
void drv2605_motor_hw_init(void)
{
	...
	int i=0;
	get_motor_ready();
	motor_write_byte(REG_RTP, 0x00);		//set RTP strength to 0 scale at the beginning

	I2C_SetBit(REG_INPUT_MODE, 0x40, 0x00);	//wakeup device
	mdelay(1);						//wait 1 ms for device ready
	check_dev_id();

	motor_write_byte(REG_RATED_VOLTAGE, 0x4e);
	motor_write_byte(REG_OVERDRIVE_CLAMP, 0x80);
...
{
	int status;
	status = i2c_register_board_info(I2C_BUS_NUM, motor_board_info, I2C_BOARD_INFO_LEN);
	printk("%s status:%d\n",__func__, status);
	return 0;
}
static int __exit motor_board_exit(void)
{
	return 0;
}
