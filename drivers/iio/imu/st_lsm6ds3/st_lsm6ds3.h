...
#define ST_INFO(fmt,args...)  printk(KERN_EMERG "Lsm6ds3:%s " fmt,__func__,##args)

#define enable_irq(args...) {\
		extern void mt_enable_irq(bool enable); \
		do {mt_enable_irq(1); } while(0);		\
	}
#define disable_irq(args...) {\
		extern void mt_enable_irq(bool enable); \
		do {mt_enable_irq(0); } while(0);		\
	}
