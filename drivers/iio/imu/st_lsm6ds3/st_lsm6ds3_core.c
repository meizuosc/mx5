...
/* add by Jay */
#define ST_LSM6DS3_STEP_COUNTER_THR_ADDR    0x0f
#define ST_LSM6DS3_STEP_COUNTER_DEB_ADDR    0x14
/* end by Jay */
...
static int st_lsm6ds3_init_sensor(struct lsm6ds3_data *cdata)
{
	...
	default_reg_value = 0x90; // recommend value, use 4g pedometer
	err = sdata->cdata->tf->write(sdata->cdata,
					ST_LSM6DS3_STEP_COUNTER_THR_ADDR,
					1, &default_reg_value, false);
	if (err < 0)
		goto st_lsm6ds3_init_sensor_mutex_unlock;
	...
	err = sdata->cdata->tf->write(sdata->cdata,
					ST_LSM6DS3_STEP_COUNTER_DEB_ADDR,
					1, &default_reg_value, false);
	if (err < 0)
		goto st_lsm6ds3_init_sensor_mutex_unlock;
	/* end by Jay */
...
#if 1 // // acc sys interface
static ssize_t lsm6ds3_acc_get_x_offset(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	int result;
	int32_t offset[3] = {0,};

	mz_sensors_lsm6ds3_get_offset(dev, offset);
	ST_INFO("x offset:%d\n", offset[0]);

	return sprintf(buf, "%d\n", offset[0]);
}

static ssize_t lsm6ds3_acc_set_x_offset(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	mz_sensors_lsm6ds3_set_offset(dev, data, 0);
	ST_INFO("x offset:%d\n", data);
	return count;
}

static ssize_t lsm6ds3_acc_get_y_offset(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	int result;
	int32_t offset[3] = {0,};
	...
	mz_sensors_lsm6ds3_get_offset(dev, offset);
	ST_INFO("y calibbias:%d\n", offset[1]);

	return sprintf(buf, "%d\n", offset[1]);
}

static ssize_t lsm6ds3_acc_set_y_offset(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	int result, data = 0;
	...
	mz_sensors_lsm6ds3_set_offset(dev, data, 1);
	ST_INFO("y offset:%d\n", data);
	return count;
}

static ssize_t lsm6ds3_acc_get_z_offset(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	int result;
	int32_t offset[3] = {0,};
	...
	mz_sensors_lsm6ds3_get_offset(dev, offset);
	ST_INFO("z calibbias:%d\n", offset[2]);

	return sprintf(buf, "%d\n", offset[2]);
}

static ssize_t lsm6ds3_acc_set_z_offset(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	int result, data = 0;
	...
	mz_sensors_lsm6ds3_set_offset(dev, data, 2);
	ST_INFO("z offset:%d\n", data);
	return count;
}

static ssize_t lsm6ds3_acc_get_x_calibbias(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	int result;
	int32_t calibbias[3] = {0,};

	mz_sensors_lsm6ds3_get_calibbias(dev, calibbias);
	ST_INFO("x calibbias:%d\n", calibbias[0]);

	return sprintf(buf, "%d\n", calibbias[0]);
}

static ssize_t lsm6ds3_acc_get_y_calibbias(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	int result;
	int32_t calibbias[3] = {0,};
...
static ssize_t lsm6ds3_acc_get_z_calibbias(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	int result;
	int32_t calibbias[3] = {0,};
...
static ssize_t lsm6ds3_acc_self_test(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	int result, val = 0;
	result = mz_sensors_lsm6ds3_acc_self_test(dev);
	ST_INFO("self test result:%d\n", result);
	if( result==1 ) {
		ST_INFO("selt test success\n");
		val = 0;
	} else {
		ST_INFO("selt test failed\n");
		val = 1;
	}
	return sprintf(buf, "%d\n", val);
}

static ssize_t lsm6ds3_acc_calibrate(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	int result;
	result = mz_sensors_lsm6ds3_acc_calibrate(dev);
	ST_INFO("acc cali result:%d\n", result);
	return sprintf(buf, "%d\n", result);
}

static ssize_t lsm6ds3_gyr_self_test(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	int result, val = 0;
	result = mz_sensors_lsm6ds3_gyr_self_test(dev);
	ST_INFO("self test result:%d\n", result);
	if( result==0 ) {
		ST_INFO("selt test success\n");
		val = 0;
...
//acc
static DEVICE_ATTR(acc_x_offset, S_IRUGO | S_IWUGO, lsm6ds3_acc_get_x_offset, lsm6ds3_acc_set_x_offset);
static DEVICE_ATTR(acc_y_offset, S_IRUGO | S_IWUGO, lsm6ds3_acc_get_y_offset, lsm6ds3_acc_set_y_offset);
static DEVICE_ATTR(acc_z_offset, S_IRUGO | S_IWUGO, lsm6ds3_acc_get_z_offset, lsm6ds3_acc_set_z_offset);

static DEVICE_ATTR(acc_x_calibbias, S_IRUGO | S_IWUGO, lsm6ds3_acc_get_x_calibbias, NULL);
static DEVICE_ATTR(acc_y_calibbias, S_IRUGO | S_IWUGO, lsm6ds3_acc_get_y_calibbias, NULL);
static DEVICE_ATTR(acc_z_calibbias, S_IRUGO | S_IWUGO, lsm6ds3_acc_get_z_calibbias, NULL);
static DEVICE_ATTR(acc_self_test,   S_IRUGO | S_IWUGO, lsm6ds3_acc_self_test, NULL);
static DEVICE_ATTR(acc_calibration, S_IRUGO | S_IWUGO, lsm6ds3_acc_calibrate, NULL);

//gyro
static DEVICE_ATTR(gyr_self_test, S_IRUGO | S_IWUGO, lsm6ds3_gyr_self_test, NULL);

static struct attribute *acc_attributes[] = {
			// acc
			&dev_attr_acc_x_offset.attr,
			&dev_attr_acc_y_offset.attr,
			&dev_attr_acc_z_offset.attr,
			&dev_attr_acc_x_calibbias.attr,
			&dev_attr_acc_y_calibbias.attr,
			&dev_attr_acc_z_calibbias.attr,
			&dev_attr_acc_self_test.attr,
			&dev_attr_acc_calibration.attr,
			NULL
};

static struct attribute *gyr_attributes[] = {
			//gyro
			&dev_attr_gyr_self_test.attr,
			NULL
};
...
static struct attribute_group gyr_attribute_group = {
	.attrs = gyr_attributes
};

static void acc_create_sys_group(struct device *dev)
{
	int ret = 0;
	ret = sysfs_create_group(&dev->kobj, &acc_attribute_group);
	if(ret) {
		ST_INFO(" acc Failed to create sysfs\n");
	}
}

static void gyr_create_sys_group(struct device *dev)
{
	int ret = 0;
	ret = sysfs_create_group(&dev->kobj, &gyr_attribute_group);
	if(ret) {
		ST_INFO(" gyr Failed to create sysfs\n");
	}
}
#endif // acc sys interface
...
int st_lsm6ds3_common_probe(struct lsm6ds3_data *cdata, int irq)
{
	...
	if (err < 0) {
		ST_INFO("init sensor failed\n");
		goto iio_device_free;
	}

	err = st_lsm6ds3_allocate_rings(cdata);
	if (err < 0) {
		ST_INFO("alloc rings failed\n");
		goto iio_device_free;
	}
	...
	if (err < 0) {
		ST_INFO("i2c master probe failed\n");
		goto iio_device_unregister_and_trigger_deallocate;
	}

	device_init_wakeup(cdata->dev, true);
//	acc_create_sys_group(&cdata->indio_dev[ST_INDIO_DEV_ACCEL]->dev);
//	gyr_create_sys_group(&cdata->indio_dev[ST_INDIO_DEV_GYRO]->dev);

#if 0
	err = meizu_sysfslink_register_n(&cdata->indio_dev[ST_INDIO_DEV_ACCEL]->dev, "acc");
	if (err < 0) {
		dev_err(cdata->dev, "failed to meizu_sysfslink_register_name.\n");
		goto iio_device_unregister_and_trigger_deallocate;
	}

	err = meizu_sysfslink_register_n(&cdata->indio_dev[ST_INDIO_DEV_GYRO]->dev, "gyr");
	if (err < 0) {
		dev_err(cdata->dev, "failed to meizu_sysfslink_register_name.\n");
		goto iio_device_unregister_and_trigger_deallocate;
	}
...
int st_lsm6ds3_common_suspend(struct lsm6ds3_data *cdata)
{
	...
	extern void mt_enable_irq(bool enable);
