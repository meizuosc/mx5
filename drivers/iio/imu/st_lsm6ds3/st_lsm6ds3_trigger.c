...
static struct lsm6ds3_data *private_cd = NULL;
...
void st_lsm6ds3_flush_works()
{
	if( st_lsm6ds3_wq ) {
		...
		ST_INFO("goes wrong, wq is NULL\n");
	}
}

// irqreturn_t st_lsm6ds3_save_timestamp(int irq, void *private)
void st_lsm6ds3_save_timestamp(void)
{
	struct timespec ts;
//	struct lsm6ds3_data *cdata = private;
	struct lsm6ds3_data *cdata = private_cd;
...
EXPORT_SYMBOL(st_lsm6ds3_save_timestamp);
				...
				const struct iio_trigger_ops *trigger_ops)
{
	...
	if (!st_lsm6ds3_wq) {
		ST_INFO(" create workqueue:%s\n", cdata->name);
		st_lsm6ds3_wq = create_singlethread_workqueue("Lsm6ds3_wq"/*cdata->name*/);
	}

	if (!st_lsm6ds3_wq) {
		ST_INFO( " create workqueue:%s  failed\n", cdata->name);
		return -EINVAL;
	}
	 ...
	 */
	private_cd = cdata;
	extern void mt_register_irq(void);
	mt_register_irq();
