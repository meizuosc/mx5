...
#define ST_INFO(fmt,args...)  printk(KERN_EMERG "Lsm6ds3:mag %s " fmt,__func__,##args)
...
static int akm09911_hw_init(struct mz_mag_data *mz_mag)
{
	...
	if (AKM09911_DEVICE_ID != (buf[1]<<8|buf[0])) {
		ST_INFO( "mag device is not akm09911\n");
		return -1;
	}

	ST_INFO( "mag device is akm09911\n");
	ST_INFO( "device id: 0x%02X%02X\n", buf[1], buf[0]);
	...
	ST_INFO( "mag sens: 0x%X 0x%X 0x%X\n",
		buf[0], buf[1], buf[2]);
...
static int af8133_hw_init(struct mz_mag_data *mz_mag)
{
	...
	if (AF8133_DEVICE_ID != buf[0]) {
		ST_INFO( "mag device is not af8133, id:0x%x\n", buf[0]);
		return -1;
	}
...
static int mz_mag_input_init(struct mz_mag_data *mz_mag)
{
	...
	if (!mz_mag->input_dev) {
		ret = -ENOMEM;
		ST_INFO( "Failed to allocate input device\n");
		goto err0;
	}
	...
	if (ret) {
		ST_INFO( "Failed to register input device\n");
		goto err1;
	}
	...
	if (!ret) {
		...
		for (i = 0; i < MZ_MAG_IIO_NUMBER_DATA_CHANNELS; i++) {
			...
			if (mz_mag->indio_dev->active_scan_mask == NULL) {
				printk(KERN_EMERG "Lsm6ds3: mag bug: active_scan_mask: %p\n",
...
static int mz_mag_iio_init(struct mz_mag_data *mz_mag)
{
	...
	if (mz_mag->iio_buffer_data == NULL) {
		ST_INFO("failed to call kmalloc\n");
		return -ENOMEM;
	}
	...
	if (!mz_mag->iio_workq) {
		ST_INFO("failed to call create_singlethread_workqueue\n");
		goto free_iio_buffer_data;
	}

	mz_mag->indio_dev = iio_device_alloc(0);
	if (!mz_mag->indio_dev) {
		ST_INFO("failed to call iio_device_alloc\n");
		goto wq_destory;
	}
	...
	if (ret < 0) {
		ST_INFO("failed to setup triggered buffer\n");
		goto iio_device_free;
	}

	mz_mag->trig = iio_trigger_alloc("%s-trigger", "magn");
	if (!mz_mag->trig) {
		ST_INFO("failed to call iio_trigger_alloc\n");
	...
	if (ret < 0) {
		ST_INFO("failed to register iio trigger.\n");
	...
	if (ret < 0) {
		ST_INFO("failed to register iio device.\n");
		goto iio_deallocate_trigger;
	}
#if 1
	ret = meizu_sysfslink_register_n(&mz_mag->indio_dev->dev, "compass");
	if (ret < 0) {
...
static int mz_mag_setup(struct mz_mag_data *mz_mag)
{
	...
	ST_INFO("setup magnetometer sensor, mz_mag: %llu!!\n", (uint64_t)mz_mag);
	...
	ST_INFO("Failed to setup magnetometer sensor\n");
...
int mz_mag_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	ST_INFO("%s mz_mag_probe\n", client->name);

	mz_mag = kzalloc(sizeof(struct mz_mag_data), GFP_KERNEL);
	if (!mz_mag) {
		ST_INFO("%s mz_mag_probe: memory allocation failed.\n", MZ_MAG_DEV_NAME);
		ret = -ENOMEM;
		goto err0;
	}
...
//		I2C_BOARD_INFO(MZ_MAG_DEV_NAME, 0x0c), /* st480 & akm09911 */
		I2C_BOARD_INFO("mz_mag_af8133", 0x1e), /* af8133 */
	},
	{
		I2C_BOARD_INFO("mz_mag_ak_st", 0x0c), /* st480 & akm09911 */
	},
};
...
static int __init mz_mag_init(void)
{
	...
	ret = i2c_register_board_info(2, mz_mag_i2c, 2);
	if (ret < 0) {
		ST_INFO("%s i2c_register_board_info failed\n", MZ_MAG_DEV_NAME);
	}

	ST_INFO("%s register board_info success\n", MZ_MAG_DEV_NAME);
