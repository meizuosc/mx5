	...
	for_each_online_pgdat(pgdat) {
		for (zoneidx = 0; zoneidx <= high_zoneidx; zoneidx++) {
			...
			if (fragmentation_index(z, THREAD_SIZE_ORDER) > sysctl_extfrag_threshold) {
				lowmem_print(4, "fragmentation，We should kill process more aggressive\n");
				return 0;
			}
		}
	}
...
static int lowmem_shrink(struct shrinker *s, struct shrink_control *sc)
{
	...
	/*only call aggressive_lmk_for_frag when monkeytest*/
	rcu_read_lock();
	for_each_process(tsk) {
		...
		if (!strcmp(t->comm, "com.android.commands.monkey")) {
			task_unlock(t);
			in_monkey_test = 1;
			break;
		}
	...
	for_each_process(tsk) {
		...
		if (!strcmp(p->comm, ".flyme.launcher")) {
			lowmem_print(2, "lowmem_scan filter process: %d (%s), ignore it.", p->pid, p->comm);
			task_unlock(p);
			continue;
		}
