...
/*
** ADUX1001 register addresses
*/
#define ADUX1001_CONTROL        0x01
#define ADUX1001_CALIBRATE      0x12
#define ADUX1001_SLRA_CAL0      0x13
#define ADUX1001_STATUS         0x1C
#define ADUX1001_VERSION        0x1D

/*
** Commonly used register values and flags
*/
#define ADUX1001_CONTROL_CLEAR      0x01
#define ADUX1001_CONTROL_RESET      0x02
#define ADUX1001_CONFIG_PWM         0x68
#define ADUX1001_CONFIG_I2C         0x64

#define ADUX1001_CALIBRATE_FLAG     0x01
#define ADUX1001_STATUS_BUSY_FLAG   0x01
#define ADUX1001_STATUS_FAULT_FLAG  0x08

#define I2C_RETRY_WAIT_TIME     5       /* milliseconds */
#define POWER_ON_WAIT_TIME      200     /* microseconds */
#define POWER_TOGGLE_WAIT_TIME  200     /* microseconds */
#define CALIBRATE_WAIT_TIME     90      /* milliseconds */
#define CALIBRATE_EXTENSION     15      /* milliseconds */
...
#define INIT_SEMCO_DMJBRM0850       4
#define INIT_SEMCO_DMJBRN0934AA     5

#define INIT_DATA   INIT_SEMCO_DMJBRN0934AA
...
#define ADUX1001_DEFAULT_CAL0   0x00
#define ADUX1001_DEFAULT_CAL1   0x25
...
static VibeInt8 g_initData2[INITDATA2_SIZE] =
{
    ...
    ADUX1001_DEFAULT_CAL2,
    ADUX1001_DEFAULT_CAL3, /* ADUX1001_SLRA_CAL3 */
    0x65, /* ADUX1001_RESERVED0 */
    0xE0,
    0x09,
    0xE9,
    0x04 /* ADUX1001_RESERVED4 */
};
...
#define ADUX1001_DEFAULT_CAL1   0x25
#define ADUX1001_DEFAULT_CAL2   0x02
#define ADUX1001_DEFAULT_CAL3   0xE5
static VibeInt8 g_initData2[INITDATA2_SIZE] =
{
    0x11,
    0x00, /* ADUX1001_ACTIVE  */
...
#define ADUX1001_DEFAULT_CAL1   0x40
#define ADUX1001_DEFAULT_CAL2   0x03
#define ADUX1001_DEFAULT_CAL3   0x50
static VibeInt8 g_initData2[INITDATA2_SIZE] =
{
    0x11,
    0x00, /* ADUX1001_ACTIVE */
...
#elif (INIT_DATA == INIT_SEMCO_DMJBRM0850)
static VibeInt8 g_initData1[INITDATA1_SIZE] =
{
    0x02,
    0x68, /* ADUX1001_CONFIG - default to PWM mode */
...
#define ADUX1001_DEFAULT_CAL0   0x00
#define ADUX1001_DEFAULT_CAL1   0x35
#define ADUX1001_DEFAULT_CAL2   0x03
...
#elif (INIT_DATA == INIT_SEMCO_DMJBRN0934AA)
static VibeInt8 g_initData1[INITDATA1_SIZE] =
{
    0x02,
    0x68, /* ADUX1001_CONFIG - default to PWM mode */
...
#define ADUX1001_DEFAULT_CAL0   0x00
#define ADUX1001_DEFAULT_CAL1   0x32
#define ADUX1001_DEFAULT_CAL2   0x02
#define ADUX1001_DEFAULT_CAL3   0xDA
static VibeInt8 g_initData2[INITDATA2_SIZE] =
{
    0x11,
    0x00, /* ADUX1001_ACTIVE */
...
static void AmpDisableWorkQueueHandler(struct work_struct *w)
{
    ...
    {
        DbgOutErr(("AmpDisableWorkQueueHandler: down_timeout timed out.\n"));
        return;
    }
...
IMMVIBESPIAPI VibeStatus ImmVibeSPI_ForceOut_AmpDisable(VibeUInt8 nActuatorIndex)
{
    if (g_bAmpEnabled)
    {
        ...
        if (g_nTrailingNegativeForce == 0 || (g_nTrailingNegativeForce*5) > WAIT_TIME_BEFORE_DISABLE_AMP_MS)
        {
            /*
...
static void AmpEnable(VibeUInt8 nActuatorIndex)
{
    ...
    if (0 != down_timeout(&g_hSemaphoreAmpDisableEnable, msecs_to_jiffies(AMP_ENABLE_DISABLE_TIMEOUT_MS)))
    {
        DbgOutErr(("ImmVibeSPI_ForceOut_AmpEnable: down_timeout timed out.\n"));
    }

    if (!g_bAmpEnabled)
    {
        DbgOutInfo(("AmpEnable %d.\n", nActuatorIndex));
        g_bAmpEnabled = true;
        ...
        schedule_timeout_interruptible(usecs_to_jiffies(POWER_ON_WAIT_TIME));
...
static int calibrate_adux1001(struct adux1001_calib_data *calib_data)
{
    ...
    */
    g_bAmpEnabled = false;
    g_initData1[1] = ADUX1001_CONFIG_I2C; /* initialize to I2C mode for calibration */
    AmpEnable(0);
    g_initData1[1] = ADUX1001_CONFIG_PWM; /* initialize to PWM for effect play */
    ...
    g_i2cBuf[0] = ADUX1001_CALIBRATE;
    g_i2cBuf[1] = g_initData2[2] | ADUX1001_CALIBRATE_FLAG;
    msg[0].len = 2;
    i2c_transfer(g_pTheClient->adapter, msg, 1);
    schedule_timeout_interruptible(msecs_to_jiffies(CALIBRATE_WAIT_TIME));
    ...
    while (((calibrationTimeoutCycles--) > 0) &&
            ((g_i2cBuf[0] & ADUX1001_STATUS_BUSY_FLAG) == ADUX1001_STATUS_BUSY_FLAG))
    {
        /* Just in case calibration is still not done, wait a bit longer */
        schedule_timeout_interruptible(msecs_to_jiffies(CALIBRATE_EXTENSION));
        g_i2cBuf[0] = ADUX1001_STATUS;
        i2c_transfer(g_pTheClient->adapter, msg, 2);
    }

    if ((g_i2cBuf[0] & ADUX1001_STATUS_FAULT_FLAG) != ADUX1001_STATUS_FAULT_FLAG)
    {
        /* status okay, retrieve and store calibration values */
        g_i2cBuf[0] = ADUX1001_SLRA_CAL0;
        msg[1].len = 4;
        i2c_transfer(g_pTheClient->adapter, msg, 2);
        ...
        g_i2cBuf[0] = ADUX1001_CONTROL;
        g_i2cBuf[1] = ADUX1001_CONTROL_CLEAR;
        i2c_transfer(g_pTheClient->adapter, msg, 1);
        ImmVibeSPI_ForceOut_AmpDisable(0);
        return -1;
    }
...
IMMVIBESPIAPI VibeStatus ImmVibeSPI_ForceOut_Initialize(void)
{
    ...
    if (!adapter)
    {
        DbgOutErr(("ADUX1001: Cannot get adapter\n"));
        return VIBE_E_FAIL;
    }
    ...
    if (!client)
    {
        DbgOutErr(("ADUX1001: Cannot create new device \n"));
        return VIBE_E_FAIL;
    }
    ...
    if (retval)
    {
        DbgOutErr(("ADUX1001 driver initialization error \n"));
...
*/
    if (0 > get_calibdata(&g_calib_data))
    {
        while (calibrate_adux1001(&g_calib_data) < 0)
        {
            if ((nRetries--) == 0)
            {
                DbgOutErr(("ADUX1001 calibration failed.\n"));
                return VIBE_E_FAIL;
            }
        }
        ...
        {
            DbgOutErr(("ADUX1001 storing calibration data failed.\n"));
        }
    }
#endif
...
IMMVIBESPIAPI VibeStatus ImmVibeSPI_ForceOut_SetSamples(VibeUInt8 nActuatorIndex, VibeUInt16 nOutputSignalBitDepth, VibeUInt16 nBufferSizeInBytes, VibeInt8* pForceOutputBuffer)
{
    ...
    if ((0 == nActuatorIndex) && (8 == nOutputSignalBitDepth) && (1 == nBufferSizeInBytes))
    {
        ...
        { /* don't call pwm_config if PWM isn't changing because the ADUX1001 is sensitive to glitches in the signal */
            if (force < 10) g_nTrailingNegativeForce += 1;
            return VIBE_S_SUCCESS;
        } else
        {
            g_nLastForce = force;
...
static int adux1001_probe(struct i2c_client* client, const struct i2c_device_id* id)
{
    ...
    else
    {
        ...
        */
            ...
            schedule_timeout_interruptible(usecs_to_jiffies(POWER_ON_WAIT_TIME));
            gpio_direction_output(GPIO_VIBTONE_EN1, GPIO_LEVEL_LOW);
            schedule_timeout_interruptible(usecs_to_jiffies(POWER_TOGGLE_WAIT_TIME));
            gpio_direction_output(GPIO_VIBTONE_EN1, GPIO_LEVEL_HIGH);
            schedule_timeout_interruptible(usecs_to_jiffies(POWER_ON_WAIT_TIME));
            gpio_direction_output(GPIO_VIBTONE_EN1, GPIO_LEVEL_LOW);
#else
