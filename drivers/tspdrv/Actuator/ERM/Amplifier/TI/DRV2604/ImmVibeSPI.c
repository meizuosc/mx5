...
*/
#define GUARANTEE_AUTOTUNE_BRAKE_TIME  0 /* Not used when running in open-loop mode */
...
static int drv2604_probe(struct i2c_client* client, const struct i2c_device_id* id)
{
    ...
    {
        DbgOutErr(("drv2605 probe failed"));
        return -ENODEV;
    }
    g_pTheClient = client;
    ...
    {
      DbgOutErr(("drv2604 auto-calibration failed after %d attempts.\n", nCalibrationCount));
    }
    else
    {
    ...
    DbgOutInfo(("drv2604 probe succeeded"));
...
IMMVIBESPIAPI VibeStatus ImmVibeSPI_ForceOut_AmpEnable(VibeUInt8 nActuatorIndex)
{
    if (!g_bAmpEnabled)
    {
        ...
        if(drv2604_read_reg(RATED_VOLTAGE_REG) != init_sequence[5])
        {
            drv2604_write_reg_val(init_sequence, sizeof(init_sequence));
        }
...
IMMVIBESPIAPI VibeStatus ImmVibeSPI_ForceOut_Initialize(void)
{
    ...
    if (adapter) {
        ...
        if (client) {
            ...
            DbgOutVerbose(("drv2605: Cannot create new device.\n"));
            return VIBE_E_FAIL;
        }
