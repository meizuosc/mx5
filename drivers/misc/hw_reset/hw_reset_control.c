			   ...
			   const char *buf, size_t count)
{
	...
	if (ret == 0) {
		printk("[hw_reset] EINVAL.\n");
		return -EINVAL;
	}

	printk("[hw_reset]state: %d\n", disable);
...
err_create_file:
	device_destroy(meizu_class, MKDEV(0, 0));
	pr_err("%s: Failed to register %s driver\n", __func__, hrc_dev.name);
