...
re_start:
	...
	if( (threshold_min_value < 0) && (!fpc1020->worker.stop_request))
	{		
	...
	while (!fpc1020->worker.stop_request &&
		...
		if(old_dir != dir)
		{
			ktime_t ctime;
			ctime = ktime_get();
			if(dir == FPC1020_DIR_FINGER_DOWN)
			{
				last_key_press_time = ctime;
				dev_dbg(&fpc1020->spi->dev,"%s: Key down time %lld ms.\n",__func__,last_key_press_time.tv64);
			}
			else
			{
				dev_dbg(&fpc1020->spi->dev,"%s: Key pressed %llu ms.\n",__func__,ktime_to_ms(ktime_sub(ctime,last_key_press_time)));
			...
			if( (old_dir == dir) && (dir == FPC1020_DIR_NONE) && (debaunce_cnt == 1) )
				fpc1020_input_report_key_downup(fpc1020);
#endif //GF_INPUT_KEY_DEBOUNCE
#endif //FPC1020_KEY_REPORT_CONTINUE
...
static int fpc1020_write_nav_setup(fpc1020_data_t *fpc1020)
{
	...
	if (error || fpc1020->worker.stop_request )
		goto out;
