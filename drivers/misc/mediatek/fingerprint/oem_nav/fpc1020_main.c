...
static int  fpc1020_probe(struct spi_device *spi)
{
	...
	if (!fpc1020) {
		...
		printk(KERN_ERR "failed to allocate memory for struct fpc1020_data\n");
		return -ENOMEM;
	}
					...
					struct fpc1020_platform_data *pdata)
{
	...
	error = request_threaded_irq(fpc1020->irq, NULL, fpc1020_interrupt, 
		...
		dev_name(&fpc1020->spi->dev), fpc1020);
#else
					...
					struct fpc1020_platform_data *pdata)
{
	...
	fpc1020->spi->irq = mt_gpio_to_irq(CUST_EINT_FP_EINT_NUM);
...
irqreturn_t fpc1020_interrupt(int irq, void *_fpc1020)
{
	...
	state =	gpio_get_value_a(fpc1020->irq_gpio);
	printk("fpc irq = %d,state = %d\n",irq,state);
	if ( state ) {
