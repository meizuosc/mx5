...
#define gpio_get_value_a mt_get_gpio_in
#define gpio_set_value_a mt_set_gpio_out

#define gpio_is_valid_a

#else
#define gpio_get_value_a gpio_get_value
#define gpio_set_value_a gpio_set_value

#define gpio_is_valid_a	gpio_is_valid
...
struct fpc1020_platform_data fpc1020_data = {
	.irq_gpio = GPIO_FP_INT_PIN,
	.reset_gpio = GPIO_FP_RESET_PIN,
	.cs_gpio = GPIO_FP_SPICS_PIN,
...
#define INFO(fmt,args...)  	printk("mzTeeBoost:%s " fmt,__func__,##args)
#define MIN_CPUS		5
...
static int  fpc1020_probe(struct spi_device *spi)
{
	...
	INIT_WORK(&fpc1020->poll_work, fpc1020_poll_work);
#endif
...
static int fpc1020_suspend(struct device *dev)
{
	...
	if(1)
	{
		mt_set_gpio_pull_enable(fpc1020->irq_gpio, GPIO_PULL_ENABLE);
		mt_set_gpio_pull_select(fpc1020->irq_gpio, GPIO_PULL_DOWN);
		
		// note
		mt_set_gpio_pull_select(fpc1020->reset_gpio, GPIO_PULL_DOWN);	
		mt_set_gpio_pull_enable(fpc1020->reset_gpio, GPIO_PULL_ENABLE);
		mt_set_gpio_dir(fpc1020->reset_gpio, GPIO_DIR_IN);
		//mt_set_gpio_out(fpc1020->reset_gpio, GPIO_OUT_ZERO);	
		
		/*set spi gpio*/	
		mt_set_gpio_mode(GPIO_FP_SPICLK_PIN, GPIO_FP_SPICLK_PIN_M_GPIO);
		mt_set_gpio_dir(GPIO_FP_SPICLK_PIN, GPIO_DIR_IN);
		mt_set_gpio_pull_select(GPIO_FP_SPICLK_PIN, GPIO_PULL_DOWN);	
		...
		mt_set_gpio_pull_select(GPIO_FP_SPIMISO_PIN, GPIO_PULL_DOWN);	
		mt_set_gpio_pull_enable(GPIO_FP_SPIMISO_PIN, GPIO_PULL_ENABLE);
		
		mt_set_gpio_mode(GPIO_FP_SPIMOSI_PIN, GPIO_FP_SPIMOSI_PIN_M_GPIO);
		mt_set_gpio_dir(GPIO_FP_SPIMOSI_PIN, GPIO_DIR_IN);
		mt_set_gpio_pull_select(GPIO_FP_SPIMOSI_PIN, GPIO_PULL_DOWN);	
		mt_set_gpio_pull_enable(GPIO_FP_SPIMOSI_PIN, GPIO_PULL_ENABLE);
		
		// note
		mt_set_gpio_mode(GPIO_FP_SPICS_PIN, GPIO_FP_SPICS_PIN_M_GPIO);
		mt_set_gpio_dir(GPIO_FP_SPICS_PIN, GPIO_DIR_IN);
		mt_set_gpio_pull_select(GPIO_FP_SPICS_PIN, GPIO_PULL_DOWN); 
		mt_set_gpio_pull_enable(GPIO_FP_SPICS_PIN, GPIO_PULL_ENABLE);
...
#ifdef SUSPEND_WITH_POWER_OFF
	//Resume GPIO
	if(1){
		mt_set_gpio_pull_enable(fpc1020->irq_gpio, GPIO_PULL_DISABLE);
		//mt_set_gpio_pull_select(fpc1020->irq_gpio, GPIO_PULL_DOWN);

		// note
		mt_set_gpio_pull_enable(fpc1020->reset_gpio, GPIO_PULL_DISABLE);
		mt_set_gpio_dir(fpc1020->reset_gpio, GPIO_DIR_OUT);
		...
		mt_set_gpio_pull_select(GPIO_FP_SPICS_PIN, GPIO_PULL_UP);	
		mt_set_gpio_pull_enable(GPIO_FP_SPICS_PIN, GPIO_PULL_ENABLE);
...
static void fpc1020_late_resume(struct early_suspend *h)
{
	...
	// Don't wait for finger up after resume.
	//if( fpc1020->setup.capture_mode = FPC1020_MODE_WAIT_FINGER_UP )
	//{
					...
					struct fpc1020_platform_data *pdata)
{
	...
	if ( gpio_is_valid_a(pdata->reset_gpio) ) {
		...
		error = mt_set_gpio_mode(fpc1020->reset_gpio, GPIO_FP_RESET_PIN_M_GPIO);
		error = mt_set_gpio_dir(fpc1020->reset_gpio, GPIO_DIR_OUT);
		error = mt_set_gpio_out(fpc1020->reset_gpio, GPIO_OUT_ONE);	
		error = mt_set_gpio_pull_enable(fpc1020->reset_gpio, GPIO_PULL_DISABLE);
		...
		dev_info(&fpc1020->spi->dev, "Cool reset\n");
					...
					struct fpc1020_platform_data *pdata)
{
	...
	//if (gpio_is_valid_a(pdata->irq_gpio)) {
	if( pdata->irq_gpio ) {
		...
		error = mt_set_gpio_mode(fpc1020->irq_gpio,GPIO_FP_INT_PIN_M_EINT);
	...
	fpc1020->irq = mt_gpio_to_irq(CUST_EINT_FP_EINT_NUM);
	//fpc1020->spi->irq = fpc1020->irq;
	printk( "%s:fpc1020->irq = %d\n", __func__,fpc1020->irq);
	...
	error = request_irq(fpc1020->irq, fpc1020_interrupt,
			IRQF_TRIGGER_HIGH, dev_name(&fpc1020->spi->dev), fpc1020);
#else

	error = request_irq(fpc1020->irq, fpc1020_interrupt,
			IRQF_TRIGGER_RISING , dev_name(&fpc1020->spi->dev), fpc1020);
#endif

	printk( "%s:request_irq %i .error = %d\n", __func__,fpc1020->irq,error);
					...
					struct fpc1020_platform_data *pdata)
{
	...
	fpc1020->spi->irq = mt_gpio_to_irq(fpc1020->irq_gpio);
	fpc1020->spi->bits_per_word = 8; //?
	error = spi_setup(fpc1020->spi);
	
	printk( "spi_setup error = %d \n",error);
...
#ifdef MANUAL_CS
	if (gpio_is_valid_a(pdata->cs_gpio)) {
		...
		error = mt_set_gpio_mode(fpc1020->cs_gpio, GPIO_MODE_GPIO);
		error = mt_set_gpio_pull_select(fpc1020->cs_gpio, GPIO_PULL_UP); 
		error = mt_set_gpio_pull_enable(fpc1020->cs_gpio, GPIO_PULL_DISABLE);
		error = mt_set_gpio_dir(fpc1020->cs_gpio, GPIO_DIR_OUT);	

		error = mt_set_gpio_out(fpc1020->cs_gpio, GPIO_OUT_ONE);
		if (error) {
			dev_err(&fpc1020->spi->dev,
...
irqreturn_t fpc1020_interrupt(int irq, void *_fpc1020)
{
	...
	do
	{
		io = gpio_get_value_a(fpc1020->irq_gpio);
		udelay(500);
	}while( io != gpio_get_value_a(fpc1020->irq_gpio) && (cnt --) );
...
static u8 fpc1020_selftest_short(fpc1020_data_t *fpc1020)
{
	...
	error = gpio_get_value_a(fpc1020->irq_gpio) ? 0 : -EIO;
	...
	error = (gpio_get_value_a(fpc1020->irq_gpio) == 0) ? 0 : -EIO;
