...
const fpc1020_setup_t fpc1020_setup_default_1150_a1b1f1 = {
	.adc_gain			= {5, 2, 2, 2},
	.adc_shift			= {5, 10, 10, 10},
	.pxl_ctrl			= {0xf0a, 0x0e, 0x0a, 0x0a},
	.capture_settings_mux		= 0,
	.capture_count			= 1,
	.capture_mode			= FPC1020_MODE_WAIT_AND_CAPTURE,
	.capture_row_start		= 0,
...
int fpc1020_gpio_reset(fpc1020_data_t *fpc1020)
{
	...
	while (counter) {
		...
		gpio_set_value_a(fpc1020->reset_gpio, 1);
		udelay(FPC1020_RESET_HIGH1_US);

		gpio_set_value_a(fpc1020->reset_gpio, 0);
		...
		if (!error) {
			...
			dev_err(&fpc1020->spi->dev, "%s timed out,retrying ... irq_gpio = %d\n",
				__func__,gpio_get_value_a(fpc1020->irq_gpio));
...
int fpc1020_reset(fpc1020_data_t *fpc1020)
{
	...
	error = (gpio_get_value_a(fpc1020->irq_gpio) != 0) ? -EIO : 0;
...
int fpc1020_check_hw_id(fpc1020_data_t *fpc1020)
{
	...
	dev_dbg(&fpc1020->spi->dev, "FPC hardware id, detected 0x%X\n",hardware_id);
	...
	if (match) {
		...
		dev_err(&fpc1020->spi->dev, "Hardware id mismatch: got 0x%x\n", hardware_id);
...
volatile int g_fpc_irq_disable = 0;// volatile

void fpc1020_irq_disable(fpc1020_data_t *fpc1020)
{
	if(g_fpc_irq_disable == 0)
		disable_irq_nosync(fpc1020->irq);
	g_fpc_irq_disable = 1;
}

void fpc1020_irq_enable(fpc1020_data_t *fpc1020)
{
	if(g_fpc_irq_disable)
		enable_irq(fpc1020->irq);
	g_fpc_irq_disable = 0;
}
...
u32 r_buffer[FPC1020_REG_MAX_SIZE/4+1];
u32 t_buffer[FPC1020_REG_MAX_SIZE/4+1];

static int fpc_spi_write_bytes(fpc1020_data_t *fpc1020,
				u16 addr, u32 data_len, u8 *tx_buf)
{
    struct spi_message msg;
    struct spi_transfer *xfer;
    int ret = 0;
	u8 *ptx_buf = (u8 *)t_buffer;
	u8 *prx_buf = (u8 *)r_buffer;
	...
	memset(ptx_buf,0x00,FPC1020_REG_MAX_SIZE);

    /*send fpc command to device.*/
    spi_message_init(&msg);
	
    ptx_buf[0] = (u8)(addr & 0xFF);
	if( FPC1020_REG_ACCESS_DUMMY_BYTES(addr))
	{
		ptx_buf[1] = (u8)((addr >> 8)&0xFF);
		memcpy(&ptx_buf[2],tx_buf,data_len);	
		data_len = data_len + 2;
	}
	else
	{
		memcpy(&ptx_buf[1],tx_buf,data_len);	
		data_len = data_len + 1;
	}		
    xfer[0].tx_buf = ptx_buf;	
    xfer[0].rx_buf = prx_buf;
    xfer[0].len = data_len;
    xfer[0].delay_usecs = 5;
    spi_message_add_tail(xfer, &msg);

    ret = spi_sync(fpc1020->spi, &msg);
    if(ret == 0) {
        ret = msg.actual_length - 1;
    }
	
    printk("w ret = %d, actual_length = %d,0x%.X 0x%.X 0x%.X\n", ret, msg.actual_length,ptx_buf[0],ptx_buf[1],ptx_buf[2]);
...
/* -------------------------------------------------------------------- */
static int fpc_spi_read_bytes(fpc1020_data_t *fpc1020,
				u16 addr, u32 data_len, u8 *rx_buf)
{
    ...
    if(ret == 0){
        ret = msg.actual_length - 1;
		memcpy(rx_buf,&prx_buf[1],ret);
    }
    //printk("r ret = %d, actual_length = %d\n", ret, msg.actual_length);
		      ...
		      fpc1020_reg_access_t *reg_data)
{
	...
	printk( "%s: %s reg = 0x%.X ,data.len = %d\n", __func__,(reg_data->write) ? "WRITE" : "READ",reg_data->reg,reg_data->reg_size);
	
	if (reg_data->write)
	{		
		error = fpc_spi_write_bytes(fpc1020,
				reg_data->reg, reg_data->reg_size, reg_data->dataptr);
	}
	else
	{
		error = fpc_spi_read_bytes(fpc1020,
				reg_data->reg, reg_data->reg_size, temp_buffer);

		if(error >= 0 ){
			if( error != reg_data->reg_size)
				printk("warning ret = %d, actual_length = %d\n", error, reg_data->reg_size);
			error = 0;
		}
	}
...
#ifdef MANUAL_CS
	if (fpc1020->cs_gpio)
		gpio_set_value_a(fpc1020->cs_gpio, 1);
#endif
	//printk( "%s:error = %d--\n", __func__,error);
			...
			u8 wait_irq_mask)
{
	...
	error = fpc_spi_write_bytes(fpc1020,cmd,1,(u8 *)&cmd);
...
#ifdef MANUAL_CS
	if (fpc1020->cs_gpio)
		gpio_set_value_a(fpc1020->cs_gpio, 0);
#endif
			...
			u8 wait_irq_mask)
{
	...
	struct spi_transfer t = {
		.cs_change = 1,
		.delay_usecs = 0,
		.speed_hz = (u32)fpc1020->spi_freq_khz * 1000u,//12*1000*1000,
		.tx_buf = &cmd,
		.rx_buf = NULL,
		.len    = 1,
		.tx_dma = 0,
	...
	dev_dbg(&fpc1020->spi->dev,"+++++++%s start\n",__func__);
#ifdef MANUAL_CS
	if (fpc1020->cs_gpio)
		gpio_set_value_a(fpc1020->cs_gpio, 0);
#endif
...
#endif // fpc1020_reg_access fpc1020_cmd
...
int fpc1020_wait_finger_present(fpc1020_data_t *fpc1020)
{
	...
	while (1) {
		...
		if( (error == FPC1020_STATUS_REG_RESET_VALUE) || (error == 0xFF) )
		{
			dev_err(&fpc1020->spi->dev, "%s:read status 0x%.2X\n", __func__,error);
			return -EIO;
		}
		if(error & FPC_1020_STATUS_REG_BIT_IRQ )
		{
			dev_err(&fpc1020->spi->dev, "%s:read status 0x%.2X\n", __func__,error);
			error = fpc1020_read_irq(fpc1020, true);
			if (error &
		...
		//dev_dbg(&fpc1020->spi->dev, "%s:error %d\n", __func__,error);
	}
}
...
int fpc1020_wake_up(fpc1020_data_t *fpc1020)
{
	...
	fpc1020_regulator_set(fpc1020, true);
...
#if 0
	if (deep_sleep && sleep_ok && gpio_is_valid_a(fpc1020->reset_gpio))
		gpio_set_value_a(fpc1020->reset_gpio, 0);
#endif	

#ifdef MANUAL_CS
	if (deep_sleep && sleep_ok && fpc1020->cs_gpio)
		gpio_set_value_a(fpc1020->cs_gpio, 0);
#endif
					...
					int measured_val)
{
	...
	new_val = measured_val + 32; // Todo: awaiting calculated values
	...
	if (!error) {
		...
		dev_info(&fpc1020->spi->dev, "%s %d -> %d\n",
					__func__,
					old_val,
