...
#define GF66XX_INPUT_DEV_NAME   "fp-keys"
//"gf66xx_input_key"
#define GF66XX_INPUT_MENU_KEY   KEY_MENU
#define GF66XX_INPUT_BACK_KEY   KEY_BACK
#define GF66XX_INPUT_HOME_KEY   196//KEY_HOME
...
static int gf66xx_probe(struct spi_device *spi)
{
    ...
    if(gf66xx_parse_dts(gf66xx_dev) || gf66xx_power_on(gf66xx_dev)) {
        printk("gf66xx:Failed to parse dts.\n");
        goto error;
    }
#else
...
static int gf66xx_resume(struct spi_device *spi)
 {
                ...
                printk("[GF66XX] wake to reset\n");
                gf66xx_hw_reset(gf66xx_dev, 60);
                /*SPI Test*/
                gf66xx_spi_read_byte(gf66xx_dev,0x8000,&version);
                if(version != 0x47)
                {
                        printk("gf66xx:[ERR] %s device detect error!!\n", __func__);
                }else{
                printk(KERN_ERR" [GF66xx][0x8000]=%2x",version);
                }
                printk("[gf66xx]: gf66xx_resume.\n");
                return 0;
} 
