...
#endif
#ifdef CONFIG_OTG_BQ2589X_BOOST_5V
...
int mtk_is_hub_active(void){
	...
	/*xhci host dying or root hub not attached  */
	if((mtk_xhci->xhc_state&XHCI_STATE_DYING)||
		             rhdev->state==USB_STATE_NOTATTACHED){
	    ret = false ;
	}
	...
	printk(KERN_ERR"%s(%d):epcount(%d)eventlist(%d)ref(%d)state(%d)\n",__FUNCTION__,ret,
			mtk_ep_count,list_empty(&hub->event_list),atomic_read(&(hub->kref.refcount)),rhdev->state);
