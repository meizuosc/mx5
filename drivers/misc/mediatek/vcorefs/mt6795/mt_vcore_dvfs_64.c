...
static int kick_dvfs_by_index(struct pwr_ctrl *pwrctrl, enum dvfs_kicker kicker, u32 index)
{
	...
	if ((index == OPPI_LOW_PWR_2 && curr_opp_index != OPPI_LOW_PWR_2) ||
		...
		if (r)
			goto OUT;
		vcorefs_opptb[OPPI_PERF_2].opp_count++;
	}
 ...
 * vcore_debug_xxx Function
 **************************************/
static ssize_t vcore_stats_show(struct kobject *kobj, struct kobj_attribute *attr,
				char *buf)
{
	char *p = buf;
	int i;
	...
	for (i = 0; i < NUM_OPPS; i++) {
		p += sprintf(p, "%d: %u : %d\n", i, vcorefs_opptb[i].ddr_khz, vcorefs_opptb[i].opp_count);
	}
...
static int init_vcorefs_pwrctrl(void)
{
	...
	for (i = 0; i < NUM_OPPS; i++) {
		...
		if (i < OPPI_LOW_PWR) {		/* update performance OPP */
			///{@ MEIZU_BSP force vcore_pdn voltage = 1.1625V for 1466MHZ
			if(i == OPPI_PERF_2)
				opp->vcore_pdn = VCORE_1_P_1625;
			else
