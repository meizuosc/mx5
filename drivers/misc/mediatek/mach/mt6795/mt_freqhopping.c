...
#define MPLL_1333_FREQ          1333000  ///< Special case for MPLL 1333 SSC range issue (1~0%). 1333 is just a tag for code check.
#define MPLL_800_FREQ            800000  ///< Special case for MPLL 814/799 crach issue (0~-3%). 814/799 is just a tag for code check.
...
static const struct freqhopping_ssc ssc_mpll_setting[]= {
	...
	{MPLL_1333_FREQ ,0 ,9 ,1, 0, 0x1C000},  //1 ~  0% (1333Mbps) Comment from Black.Chen, range is 1356Mbps~1333Mbps.
	{MPLL_800_FREQ ,0 ,9 ,0, 3, 0x1C000},   //0 ~ -3% (800Mbps) Comment from Terry for 800MHz 0~-3% SCC. (Special Case)
	{0,0,0,0,0,0}
};
...
static int __freqhopping_ctrl(struct freqhopping_ioctl* fh_ctl,bool enable)
{
    ...
    if ((fh_ctl->pll_id == FH_M_PLLID) && (pfh_pll->curr_freq == MPLL_800_FREQ))
    {
        // 2014.12.24 Handle for MPLL 800MHz 0~-3% SCC (Terry.Chang)
        // 800MHz doesn't use FHCTL to hopping, but when DDR in 800Mhz, it should use 0~-3% SSC.
        // So, we suggested Terry to use freqhopping_config(FH_M_PLLID, MPLL_800_FREQ, 1).
        // Just don't set curr_freq to default, and keep user's argument MPLL_800_FREQ.
        // [Hopping flow in 6795 DDR]
        //    (Flow 1) 1466 -> SSC off -> 814 -> DVS  -> SSC on (0~-3%)
        //    (Flow 2)  814 -> SSC off -> DVS -> 1466 -> SSC on (0~-5%)
        //    (Flow 3) 1333 -> SSC off -> 799 -> DVS  -> SSC on (0~-3%) (MT6795M)
        //    (Flow 4)  799 -> SSC off -> DVS -> 1333 -> SSC on (0~-5%) (MT6795M 1333 SSC is 0~-5%)
        //  pfh_pll->curr_freq <-- Init in freqhopping_config() by user 2nd argument.
    }else
    {
