...
*
********************************************************************************/
static struct i2c_board_info __initdata kd_cam_cal_dev={ I2C_BOARD_INFO(CAM_CAL_DRVNAME, 0xA8>>1)};//A0 for page0 A2 for page 2 and so on for 8 pages
...
static int get_module_type()
{
	...
	ret = iReadRegI2C(puSendCmd , 2, &module_id, 1, IMX220OTP_LITEON_DEVICE_ID);
	if ((ret < 0) || (module_id != 0x01))
		CAM_CALDB("[IMX220OTP]I2C read liteon e2prom failed!! \n");
	else
		return LITEON_MODULE;

	/* module id reg addr = 0x0001 for imx220 primax module */
	ret = iReadRegI2C(puSendCmd , 2, &module_id, 1, IMX220OTP_PRIMAX_DEVICE_ID);
	if ((ret < 0) || (module_id != 0x03))
		CAM_CALDB("[IMX220OTP]I2C read primax e2prom failed!! \n");
	else
		return PRIMAX_MODULE;


	CAM_CALDB("[IMX220OTP] get module type failed!! \n");
	return -ENODEV;
}
...
static int selective_read_region(u32 addr, BYTE* data,u16 i2c_id,u32 size)
{
	...
	if ((addr + size) > size_of_otp) {
		CAM_CALERR("[IMX220OTP] Read ERR. size_of_otp:%d, Request Read addr:0x%x, Read size:%d.\n", size_of_otp, addr, size);
		return -EFAULT;
	} else {
		for (i = 0; i < size; i++) {
			data[i] = (u8)otp_data[addr + i];
			CAM_CALDB("[IMX220OTP] Get otp data addr:0x%x, data:0x%x.\n", (addr + i), data[i]);
		}
	}
...
static kal_uint8 IMX220_ReadOtp(kal_uint16 address,unsigned char *iBuffer,unsigned int buffersize)
{
		...
		int ret ;
		u16 i2c_id = 0;
		CAM_CALDB("[IMX220OTP]ENTER  address:0x%x buffersize:%d\n ", address, buffersize);
		...
		if (get_module_type() == LITEON_MODULE) {
			 ...
			 */
			memcpy(iBuffer, (liteon_otp_buf + address), buffersize);
		} else if (get_module_type() == PRIMAX_MODULE) {
			i2c_id = IMX220OTP_PRIMAX_DEVICE_ID;
			/*
			 * lsc data had been read out in sensor driver,
			 * so just copy it to user.
			 */
...
//			ret= iReadRegI2C(address+i, 2, &readbuff, 1, i2c_id);
			CAM_CALDB("[IMX220OTP]address+i = 0x%x,readbuff = 0x%x\n",(address+i),*(iBuffer+i));
		}
		return 0;
}
...
static int iReadData(u32 ui4_offset, unsigned int  ui4_length, u8 *pinputdata)
{
   int  i4RetValue = 0;
    ...
    CAM_CALDB( "[[IMX220OTP]]pinputdata[%d]=0x%x\n", i4RetValue,*(pinputdata+i4RetValue));}
    CAM_CALDB(" [[IMX220OTP]]ui4_length = %d,ui4_offset =0x%x\n ",ui4_length,ui4_offset);
    CAM_CALDB("[IMX220OTP] iReadData done\n" );
   return 0;
}
...
{
	CAM_CALDB("[IMX220OTP]not implemented!");
	return 0;
}
...
static long imx220otp_Ioctl_Compat(struct file *filp, unsigned int cmd, unsigned long arg)
{
    long ret;
    CAM_CALDB("[IMX220OTP] COMPAT_CAM_CALIOC_G_READ\n");
    COMPAT_stCAM_CAL_INFO_STRUCT __user *data32;
    stCAM_CAL_INFO_STRUCT __user *data;
    int err;
	  CAM_CALDB("[IMX220OTP] imx220otp_Ioctl_Compat,%p %p %x ioc size %d\n",filp->f_op ,filp->f_op->unlocked_ioctl,cmd,_IOC_SIZE(cmd) );
    ...
    case COMPAT_CAM_CALIOC_G_READ:
    {
        ...
        if(err != 0)
            CAM_CALERR("[IMX220OTP] compat_put_acdk_sensor_getinfo_struct failed\n");
        return ret;
    }
    default:
        return -ENOIOCTLCMD;
...
#endif
{
    ...
    else
    {
        ...
        if(_IOC_WRITE & _IOC_DIR(a_u4Command))
        {
            ...
            {    //get input structure address
                kfree(pBuff);
                CAM_CALDB("[IMX220OTP] ioctl copy from user failed\n");
                return -EFAULT;
            }
        }
    }
    ...
    switch(a_u4Command)
    {
        ...
        case CAM_CALIOC_G_READ:
            CAM_CALDB("[IMX220OTP] Read CMD \n");
#ifdef CAM_CALGETDLT_DEBUG
            do_gettimeofday(&ktv1);
#endif
            CAM_CALDB("[IMX220OTP] offset %d \n", ptempbuf->u4Offset);
            CAM_CALDB("[IMX220OTP] length %d \n", ptempbuf->u4Length);
            //CAM_CALDB("[IMX220OTP] Before read Working buffer address 0x%p \n", pu1Params);

            i4RetValue = iReadData((u16)(ptempbuf->u4Offset), ptempbuf->u4Length, pu1Params);
            CAM_CALDB("[IMX220OTP] After read Working buffer data  0x%x \n", *pu1Params);
    ...
    if(_IOC_READ & _IOC_DIR(a_u4Command))
    {
        ...
        CAM_CALDB("[IMX220OTP] to user length %d \n", ptempbuf->u4Length);
        CAM_CALDB("[IMX220OTP] to user  Working buffer address 0x%p \n", pu1Params);
        if(copy_to_user((u8 __user *) ptempbuf->pu1Params , (u8 *)pu1Params , ptempbuf->u4Length))
        {
            kfree(pBuff);
            kfree(pu1Params);
            CAM_CALDB("[IMX220OTP] ioctl copy to user failed\n");
            return -EFAULT;
        }
    }
...
{
    CAM_CALDB("[IMX220OTP] CAM_CAL_Open\n");
    spin_lock(&g_CAM_CALLock);
    if(g_u4Opened)
    {
        spin_unlock(&g_CAM_CALLock);
		CAM_CALDB("[IMX220OTP] Opened, return -EBUSY\n");
        return -EBUSY;
    }
    else
    {
...
static const struct file_operations g_stCAM_CAL_fops =
{
    ...
    //.ioctl = CAM_CAL_Ioctl
#ifdef CONFIG_COMPAT
    .compat_ioctl = imx220otp_Ioctl_Compat,
#endif
...
inline static int RegisterCAM_CALCharDrv(void)
{
    ...
    {
        CAM_CALDB("[IMX220OTP] Allocate device no failed\n");
    ...
    {
        CAM_CALDB("[IMX220OTP] Register device no failed\n");
    ...
    if(NULL == g_pCAM_CAL_CharDrv)
    {
        ...
        CAM_CALDB("[IMX220OTP] Allocate mem for kobject failed\n");
    ...
    {
        CAM_CALDB("[IMX220OTP] Attatch file operation failed\n");
...
//    spin_lock_init(&g_CAM_CALLock);
    ...
    g_pstI2Cclient->addr = IMX220OTP_PRIMAX_DEVICE_ID>>1;
    spin_unlock(&g_CAM_CALLock); // for SMP
    kdSetI2CSpeed(EEPROM_I2C_SPEED);

    CAM_CALDB("[IMX220OTP] g_pstI2Cclient->addr = 0x%x \n",g_pstI2Cclient->addr);
    //Register char driver
    i4RetValue = RegisterCAM_CALCharDrv();

    if(i4RetValue){
        CAM_CALDB("[IMX220OTP] register char device failed!\n");
        return i4RetValue;
    }


    CAM_CALDB("[IMX220OTP] Attached!! \n");
    return 0;
}
