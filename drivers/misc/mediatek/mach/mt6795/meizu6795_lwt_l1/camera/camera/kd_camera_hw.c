...
u32 pinSet[2][9] = {
			...
			CAMERA_LASER_EN_PIN,
                        CAMERA_LASER_EN_PIN_M_GPIO,   /* mode */
                        GPIO_OUT_ONE,              /* ON state */
                        GPIO_OUT_ZERO,             /* OFF state */
                        GPIO_CAMERA_INVALID,
                     },
...
int camera_flight_poweron(char *mode_name, u8 on)
{
	if (on) {
		if(TRUE != hwPowerOn(FLIGHT_POWER_A, VOL_2800,mode_name))
		{
			PK_DBG("[CAMERA SENSOR] Fail to enable flight power\n");
			return -EIO;
		}
	} else {
		if(TRUE != hwPowerDown(FLIGHT_POWER_A,mode_name))
		{
			PK_DBG("[CAMERA SENSOR] Fail to disable flight power\n");
			return -EIO;
		}
	}
	return 0;
...
int camera_flight_gpio(u8 on)
{
	u32 pinSetIdx = 0;	/* Flight-sense just be for main camera */
	if (on) {
		if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_LASER]) {
			if(mt_set_gpio_mode(pinSet[pinSetIdx][IDX_PS_LASER],pinSet[pinSetIdx][IDX_PS_LASER+IDX_PS_MODE])){PK_DBG("[CAMERA SENSOR] set gpio mode failed!! \n");}
			if(mt_set_gpio_dir(pinSet[pinSetIdx][IDX_PS_LASER],GPIO_DIR_OUT)){PK_DBG("[CAMERA SENSOR] set gpio dir failed!! \n");}
			if(mt_set_gpio_out(pinSet[pinSetIdx][IDX_PS_LASER],pinSet[pinSetIdx][IDX_PS_LASER+IDX_PS_OFF])){PK_DBG("[CAMERA SENSOR] set gpio failed!! \n");}
			usleep_range(3000, 3500);
		}

		if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_LASER]) {
			if(mt_set_gpio_out(pinSet[pinSetIdx][IDX_PS_LASER],pinSet[pinSetIdx][IDX_PS_LASER+IDX_PS_ON])){PK_DBG("[CAMERA SENSOR] set gpio failed!! \n");}
...
int kdCISModulePowerOn(CAMERA_DUAL_CAMERA_SENSOR_ENUM SensorIdx, char *currSensorName, BOOL On, char* mode_name)
{
	...
	/* VCAM_IO */
		...
		|| (0 == strcmp(currSensorName, "imx220liteonmipiraw")))) 
        {
            if(TRUE != hwPowerOn(CAMERA_POWER_VCAM_D, VOL_1000,mode_name))
            {
                 PK_DBG("[CAMERA SENSOR] Fail to enable digital power\n");
        ...
        else if (currSensorName && ((0 == strcmp(currSensorName,"ov5670mipi"))
		|| (0 == strcmp(currSensorName, "ov5670sunnymipi"))))
        {
	...
	usleep_range(3000, 3500);
#if 1
        if (currSensorName && ((0 == strcmp(currSensorName,"ov5670mipi"))
	   || (0 == strcmp(currSensorName, "ov5670sunnymipi")))) {
		/*
		 * FIXME: remove VCAMD LDO power-off func, this is workaround.
...
EXPORT_SYMBOL(camera_flight_gpio);
EXPORT_SYMBOL(camera_flight_poweron);
