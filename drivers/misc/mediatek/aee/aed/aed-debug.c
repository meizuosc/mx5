...
static ssize_t proc_pwron_alarm_read(struct file *file,
						char __user *buf, size_t size, loff_t *ppos)
{
	char buffer[BUFSIZE];
	int len =0;
	struct rtc_wkalrm wkalrm;
	struct rtc_wkalrm *alm = &wkalrm;
	struct rtc_time *tm = &alm->time;

	hal_rtc_get_pwron_alarm(tm, alm);
	
	len = snprintf(buffer, BUFSIZE,"power-on = %04d/%02d/%02d %02d:%02d:%02d (%d)(%d)\n",
...
static ssize_t proc_pwron_alarm_write(struct file *file,
				       const char __user *buf, size_t size, loff_t *ppos)
{
	return 0;
}
...
int aed_proc_debug_init(struct proc_dir_entry *aed_proc_dir)
{
	...
	AED_PROC_ENTRY(pwron_alarm, pwron_alarm, S_IRUSR);
...
int aed_proc_debug_done(struct proc_dir_entry *aed_proc_dir)
{
	...
	remove_proc_entry("pwron_alarm", aed_proc_dir);
	return 0;
}
