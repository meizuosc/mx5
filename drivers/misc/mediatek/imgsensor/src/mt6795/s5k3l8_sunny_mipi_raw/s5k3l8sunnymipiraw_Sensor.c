...
#include "s5k3l8sunnymipiraw_Sensor.h"

#define FANPENGTAO
#define PFX "S5K3L8_sunny_camera_sensor"
...
static imgsensor_info_struct imgsensor_info = { 
	.sensor_id = S5K3L8_SUNNY_SENSOR_ID & 0x00FFFF,		//Sensor ID Value: 0x30C8//record sensor id defined in Kd_imgsensor.h
...
static int s5k3l8_read_otp(u16 addr, u8 *buf)
{
	...
	if (ret < 0)
		LOG_INF("read data from s5k3l8 sunny otp e2prom failed!\n");
...
static void s5k3l8_read_otp_burst(u16 addr, u8 *otp_buf)
{
	...
	for (i = 0; i < OTP_DATA_SIZE; i += MAX_READ_WRITE_SIZE) {
		...
		if (ret < 0)
			LOG_INF("read lsc table from s5k3l8 sunny otp e2prom failed!\n");
...
{
	copy_to_user((void __user *)data,(void *)(s5k3l8_sunny_otp_buf + PDAF_DATA_OFFSET), PDAF_DATA_SIZE);
}
		...
		do {
			*sensor_id = ((read_cmos_sensor(0x0000) << 8) | read_cmos_sensor(0x0001));
			...
			if ((ret >= 0) && (*sensor_id == imgsensor_info.sensor_id) && (module_id == 0x07)) {
				...
				*sensor_id = S5K3L8_SUNNY_SENSOR_ID;
				goto otp_read;
			}	
...
otp_read:
	 ...
	 */
	s5k3l8_sunny_otp_buf = (u8 *)kzalloc(OTP_DATA_SIZE, GFP_KERNEL);

	/* read lsc calibration from E2PROM */
	s5k3l8_read_otp_burst(OTP_START_ADDR, s5k3l8_sunny_otp_buf);
	for (j = 0; j < OTP_DATA_SIZE; j++) {
		LOG_INF("========s5k3l8_sunny_otp RegIndex-%d=====val:0x%x======\n", j, *(s5k3l8_sunny_otp_buf + j));
	}
	return ERROR_NONE;
}
...
//UINT32 S5K3L8_SUNNY_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
UINT32 S5K3L8_SUNNY_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
	...
	if (pfFunc!=NULL)
		*pfFunc=&sensor_func;
	return ERROR_NONE;
}	/*	S5K3L8_SUNNY_MIPI_RAW_SensorInit	*/
