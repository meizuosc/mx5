...
static int ov13853_ofilm_chip_ver = OV13853_R2A;
...
struct ov13853_ofilm_write_buffer {
	u8 addr[2];
	u8 data[MAX_READ_WRITE_SIZE];
};

static imgsensor_info_struct imgsensor_info = {
    .sensor_id = OV13853_OFILM_SENSOR_ID & 0x00FFFF,        //record sensor id defined in Kd_imgsensor.h
...
{
	struct ov13853_ofilm_write_buffer buf;
	int i;
	int ret;
...
static int ov13853_read_otp(u16 addr, u8 *buf)
{
	...
	if (ret < 0)
		LOG_INF("read data from ofilm otp e2prom failed!\n");
...
static void ov13853_read_otp_burst(u16 addr, u8 *otp_buf)
{
	...
	for (i = 0; i < OTP_DATA_SIZE; i += MAX_READ_WRITE_SIZE) {
		...
		if (ret < 0)
			LOG_INF("read lsc table from ofilm otp e2prom failed!\n");
...
{
	copy_to_user((void __user *)data,(void *)(ofilm_otp_buf + PDAF_DATA_OFFSET), PDAF_DATA_SIZE);
}
...
static kal_uint16 gain2reg(const kal_uint16 gain)
{
	...
	if (ov13853_ofilm_chip_ver == OV13853_R1A)
	{
	...
	else if(ov13853_ofilm_chip_ver == OV13853_R2A)
	{
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	i2c_write_id = imgsensor.i2c_write_id;
		...
		*sensor_id = OV13853_OFILM_SENSOR_ID;
                goto otp_read;
            }
...
otp_read:
	 ...
	 */
	ofilm_otp_buf = (u8 *)kzalloc(OTP_DATA_SIZE, GFP_KERNEL);

	/* read lsc calibration from E2PROM */
	ov13853_read_otp_burst(OTP_START_ADDR, ofilm_otp_buf);
	for (j = 0; j < OTP_DATA_SIZE; j++) {
		LOG_INF("========ov13853_ofilm_otp RegIndex-%d=====val:0x%x======\n", j, *(ofilm_otp_buf + j));
	}
	return ERROR_NONE;
}
...
static kal_uint32 open(void)
{
    ...
    if (imgsensor_info.sensor_id != sensor_id)
		...
		{
				LOG_INF("----R1A---- \n");
				ov13853_ofilm_chip_ver = OV13853_R1A;
		}else if((read_cmos_sensor(0x302a))==0xb2)
		{
				LOG_INF("----R2A---- \n");
				ov13853_ofilm_chip_ver = OV13853_R2A;
		}
#endif
...
UINT32 OV13853_OFILM_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
    /* To Do : Check Sensor status here */
    if (pfFunc!=NULL)
        *pfFunc=&sensor_func;
    return ERROR_NONE;
}    /*    OV13853_OFILM_MIPI_RAW_SensorInit    */
