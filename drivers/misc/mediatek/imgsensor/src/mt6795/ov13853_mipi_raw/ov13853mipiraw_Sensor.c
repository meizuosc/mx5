...
#define PFX "OV13853_primax_camera_sensor"
...
struct ov13853_write_buffer {
	u8 addr[2];
	u8 data[MAX_READ_WRITE_SIZE];
};

static imgsensor_info_struct imgsensor_info = {
    .sensor_id = OV13853_SENSOR_ID & 0x00FFFF,        //record sensor id defined in Kd_imgsensor.h
...
{
	struct ov13853_write_buffer buf;
	int i;
	int ret;
...
static int ov13853_read_otp(u16 addr, u8 *buf)
{
	...
	if (ret < 0)
		LOG_INF("read data from primax otp e2prom failed!\n");
...
static void ov13853_read_otp_burst(u16 addr, u8 *otp_buf)
{
	...
	for (i = 0; i < OTP_DATA_SIZE; i += MAX_READ_WRITE_SIZE) {
		...
		if (ret < 0)
			LOG_INF("read lsc table from primax otp e2prom failed!\n");
...
{
	copy_to_user((void __user *)data,(void *)(primax_otp_buf + PDAF_DATA_OFFSET), PDAF_DATA_SIZE);
}
...
otp_read:
	...
	ov13853_read_otp_burst(OTP_START_ADDR, primax_otp_buf);
	for (j = 0; j < OTP_DATA_SIZE; j++) {
		LOG_INF("========ov13853_primax_otp RegIndex-%d=====val:0x%x======\n", j, *(primax_otp_buf + j));
	}
