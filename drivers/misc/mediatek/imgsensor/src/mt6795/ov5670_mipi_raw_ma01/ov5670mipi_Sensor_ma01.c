...
#include "ov5670mipi_Sensor_ma01.h"
...
static imgsensor_info_struct imgsensor_info = { 
	.sensor_id = MA01_OV5670MIPI_SENSOR_ID & 0x0FFF0,		//record sensor id defined in Kd_imgsensor.h
...
static imgsensor_struct imgsensor = {
	...
	.orientation = 270,                //mirrorflip information, 0, 90, 180, 270 degree.
...
#define OTP_DATA_LOG  	  1 

avc_otp_struct *avc_otp_data;

static int avc_read_otp(avc_otp_struct *pOtp)
{

	kal_uint16 otp_flag, addr, temp,temp1, i;

	write_cmos_sensor(0x0100, 0x01);// must software stream on

	/* set 0x5002[3] to “0” */
	temp1 = read_cmos_sensor(0x5002);
	write_cmos_sensor(0x5002, (u8)((u8)temp1 & (~0x08)));
	...
	LOG_INF("[ov5670]=0x7010,awb_flag=0x%x\n",otp_flag);
	addr = 0;
	...
	/* OTP WB Calibration */
	 ...
	 LOG_INF("[ov5670]=0x7023,awb_flag=0x%x\n",otp_flag);
	 addr = 0;
	 if((otp_flag & 0xc0) == 0x40) {
...
static kal_uint16 gain2reg(const kal_uint16 gain)
{
	...
	reg_gain = ((kal_uint32)(gain * 128)/BASEGAIN);
...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0x0302, 0x6C); //;78 MIPI datarate 960 -> 840
	...
	write_cmos_sensor(0x3503, 0x30);//0x00); //; real gain [2]   gain no delay, shutter no delay
	//;added					 
//	write_cmos_sensor(0x3d85, 0x17);
//	write_cmos_sensor(0x3655, 0x20);
}	/*	sensor_init  */
		...
		do {
			*sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
			if (*sensor_id == imgsensor_info.sensor_id) {				
				...
				printk("[ov5670]read otp data production year =%d\n",avc_otp_data->production_year);
				printk("[ov5670]read otp data production month =%d\n",avc_otp_data->production_month);
				printk("[ov5670]read otp data production day =%d\n",avc_otp_data->production_day);
				...
				#endif
				if (avc_otp_data->module_id == AVC_MODULE_ID)	//this driver is for avc module
				{
				LOG_INF("i2c write id: 0x%x, sensor id: 0x%x, module_id:0x%x.\n",
					imgsensor.i2c_write_id,*sensor_id, avc_otp_data->module_id);
					*sensor_id = MA01_OV5670MIPI_SENSOR_ID;
					front_cam_id = MA01_OV5670MIPI_SENSOR_ID;
					return ERROR_NONE;
				}
				else 
...
UINT32 MA01_OV5670_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
	/* To Do : Check Sensor status here */
	if (pfFunc!=NULL)
		*pfFunc=&sensor_func;
	return ERROR_NONE;
}	/*	MA01_OV5670_MIPI_RAW_SensorInit	*/
