...
#define PFX "imx220_camera_sensor_primax"
...
otp_read:
	 ...
	 */
	primax_otp_buf = (u8 *)kzalloc(OTP_LSC_SIZE, GFP_KERNEL);

	/* read lsc calibration from E2PROM */
	imx220_read_otp_lsc(OTP_START_ADDR, primax_otp_buf);
	for (j = 0; j < OTP_LSC_SIZE; j++) {
		printk("========imx220_primax_otp RegIndex-%d=====val:0x%x======\n", j, *(primax_otp_buf + j));
	}
    return ERROR_NONE;
}
