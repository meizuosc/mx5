...
#define PK_INF(fmt, args...)	xlog_printk(ANDROID_LOG_INFO , PFX, fmt, ##args)
				...
				char *buf)
{
	...
	if (platform_match == 0)
		p += sprintf(p, "MZ_Product: UNKNOWN\n");
	else
		p += sprintf(p, "MZ_Product: MX5\n");

	if (back_cam_id == IMX220_SENSOR_ID)
		p += sprintf(p, "Back_camera: 1\n");
	else if (back_cam_id == IMX220_LITEON_SENSOR_ID)
		p += sprintf(p, "Back_camera: 2\n");

	if (front_cam_id == OV5670MIPI_SENSOR_ID)
		p += sprintf(p, "Front_camera: 1\n");
	else if (front_cam_id == OV5670SUNNYMIPI_SENSOR_ID)
		p += sprintf(p, "Front_camera: 2\n");
...
static struct device_attribute dev_attr_camera = {
...
inline static int RegisterCAMERA_HWCharDrv(void)
{
	...
	device_create_file(sensor_device, &dev_attr_camera);
