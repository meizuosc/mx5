...
#define PFX "ov5670_qtech_camera_sensor"
...
static imgsensor_info_struct imgsensor_info = { 
	.sensor_id = OV5670MIPI_SENSOR_ID & 0xFFF0,		//record sensor id defined in Kd_imgsensor.h
...
static int qtech_read_otp(qtech_otp_struct *pOtp)
{
	...
	/* OTP WB Calibration */
	 ...
	 LOG_INF("[ov5670]=0x7020,awb_flag=0x%x\n",otp_flag);
	 addr = 0;
	 if((otp_flag & 0xc0) == 0x40) {
		...
		do {
			*sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
			if (*sensor_id == imgsensor_info.sensor_id) {				
				...
				#endif
				if (qtech_otp_data->module_id != MODULE_ID)	//this driver is for qtech module
				{
				LOG_INF("i2c write id: 0x%x, sensor id: 0x%x, module_id:0x%x.\n",
					...
					*sensor_id = OV5670MIPI_SENSOR_ID;
					...
					qtech_otp_data->module_id = QTECH_MODULE_ID;
					return ERROR_NONE;
				}
				else 
