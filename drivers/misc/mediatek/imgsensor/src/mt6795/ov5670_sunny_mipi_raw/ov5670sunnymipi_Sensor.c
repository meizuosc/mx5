...
#include "ov5670sunnymipi_Sensor.h"
#include "ov5670otp_common.h"

#define PFX "ov5670_sunny_camera_sensor"
...
static imgsensor_info_struct imgsensor_info = { 
	.sensor_id = OV5670SUNNYMIPI_SENSOR_ID & 0xFFF0,		//record sensor id defined in Kd_imgsensor.h
...
static int sunny_read_otp(sunny_otp_struct *pOtp)
{
	...
	if(addr != 0) {
		...
		pOtp->module_id = read_cmos_sensor(0x7011); //read module id in group 1.
	...
	if(addr != 0) {
 		...
 		pOtp->awb_rg_msb = 0x40;
 		pOtp->awb_bg_msb = 0x4c;
 		pOtp->awb_lsb = 0xb0;
 	}
			...
			*sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
			if (*sensor_id == imgsensor_info.sensor_id) {				
				...
				LOG_INF("i2c write id: 0x%x, sensor id: 0x%x, module_id:0x%x.\n",
					...
					*sensor_id = OV5670SUNNYMIPI_SENSOR_ID;
					front_cam_id = OV5670SUNNYMIPI_SENSOR_ID;
					return ERROR_NONE;
				}
				else 
...
UINT32 OV5670_SUNNY_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
	/* To Do : Check Sensor status here */
	if (pfFunc!=NULL)
		*pfFunc=&sensor_func;
	return ERROR_NONE;
}	/*	OV5670_SUNNY_MIPI_RAW_SensorInit	*/
