...
#define PFX "OV13853_sunny_camera_pdaf"
...
bool read_sunny_otp_pdaf_data( kal_uint16 addr, BYTE* data, kal_uint32 size){
	
	LOG_INF("read_sunny_otp_pdaf_data enter");
	if(!get_done || last_size != size || last_offset != addr) {
		if(!OV13853_read_eeprom(addr, data, size)){
			get_done = 0;
            last_size = 0;
            last_offset = 0;
			LOG_INF("read_sunny_otp_pdaf_data fail");
			return false;
		}
	}
	//memcpy(data, eeprom_data, size);
	LOG_INF("read_sunny_otp_pdaf_data end");
    return true;
}
