...
#define FANPENGTAO
#define PFX "S5K3L8_primax_camera_sensor"
...
static imgsensor_info_struct imgsensor_info = { 
	.sensor_id = S5K3L8_SENSOR_ID & 0x00FFFF,		//Sensor ID Value: 0x30C8//record sensor id defined in Kd_imgsensor.h
...
{
 { 4216, 3128,	  8,  	8, 4208, 3120, 2104, 1560,   0,	0, 2104, 1560, 	 0, 0, 2104, 1560}, // Preview 
 { 4216, 3128,	  8,  	8, 4208, 3120, 4208, 3120,   0,	0, 4208, 3120, 	 0, 0, 4208, 3120}, // capture 
 { 4216, 3128,	  8,  	8, 4208, 3120, 4208, 3120,   0,	0, 4208, 3120, 	 0, 0, 4208, 3120}, // video 
 { 4216, 3128,	  8,    8, 4208, 3120, 1052, 780,   0,	0, 1052,  780, 	 0, 0, 1052,  780}, //hight speed video
 { 4216, 3128,	  8,    8, 4208, 3120, 1052, 780,   0,	0, 1052,  780, 	 0, 0, 1052,  780}, //slim video
};
...
static int s5k3l8_read_otp(u16 addr, u8 *buf)
{
	...
	if (ret < 0)
		LOG_INF("read data from s5k3l8 primax otp e2prom failed!\n");
...
static void s5k3l8_read_otp_burst(u16 addr, u8 *otp_buf)
{
	...
	for (i = 0; i < OTP_DATA_SIZE; i += MAX_READ_WRITE_SIZE) {
		...
		if (ret < 0)
			LOG_INF("read lsc table from s5k3l8 primax otp e2prom failed!\n");
...
{
	memcpy((void *)data,(void *)(s5k3l8_primax_otp_buf + PDAF_DATA_OFFSET), PDAF_DATA_SIZE);
}
...
//	write_cmos_sensor_byte(0x03089, 0x01); //non continue mode
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = ((read_cmos_sensor(0x0000) << 8) | read_cmos_sensor(0x0001));
			...
			if ((ret >= 0) && (*sensor_id == imgsensor_info.sensor_id) && (module_id == 0x03)) {
...
otp_read:
	 ...
	 */
	s5k3l8_primax_otp_buf = (u8 *)kzalloc(OTP_DATA_SIZE, GFP_KERNEL);

	/* read lsc calibration from E2PROM */
	s5k3l8_read_otp_burst(OTP_START_ADDR, s5k3l8_primax_otp_buf);
	for (j = 0; j < OTP_DATA_SIZE; j++) {
		LOG_INF("========s5k3l8_primax_otp RegIndex-%d=====val:0x%x======\n", j, *(s5k3l8_primax_otp_buf + j));
	}
	return ERROR_NONE;
}
