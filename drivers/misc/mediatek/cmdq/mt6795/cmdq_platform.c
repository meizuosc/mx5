...
void cmdq_core_initial_backup_event(void)
{
	gCmdqBackupEvent.EventID[CMDQ_EVENT_BACKUP_SYNC_TOKEN_VENC_EOF] = CMDQ_SYNC_TOKEN_VENC_EOF;
	gCmdqBackupEvent.BackupValue[CMDQ_EVENT_BACKUP_SYNC_TOKEN_VENC_EOF] = 0;

	gCmdqBackupEvent.EventID[CMDQ_EVENT_BACKUP_SYNC_TOKEN_VENC_INPUT_READY] = CMDQ_SYNC_TOKEN_VENC_INPUT_READY;
	gCmdqBackupEvent.BackupValue[CMDQ_EVENT_BACKUP_SYNC_TOKEN_VENC_INPUT_READY] = 0;
}

void cmdq_core_event_backup(void)
{
	int i;
...
void cmdq_core_event_restore(void)
{
	int i;

	for (i = 0; i < CMDQ_EVENT_BACKUP_COUNT; i++) {
		CMDQ_REG_SET32(CMDQ_SYNC_TOKEN_UPD, (gCmdqBackupEvent.BackupValue[i] << 16) | gCmdqBackupEvent.EventID[i]);
	}
}
#endif
