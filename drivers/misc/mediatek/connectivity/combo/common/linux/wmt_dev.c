...
INT32 wmt_dev_tm_temp_query(void)
{
	...
	if(!mtk_wcn_stp_is_ready()|| !wmt_any_function_on() || is_lcd_off()){
	  WMT_INFO_FUNC("wmt stp is not ready  or lcd off\n");
	  return 40 ;/* */
	}
    //Query condition 1:
    ...
    // Dump information
    //    
    WMT_DBG_FUNC("[Thermal] idx_temp_table = %d,query_cond(%d) \n", idx_temp_table,query_cond);
    WMT_DBG_FUNC("[Thermal] now.time = %d, query.time = %d, REFRESH_TIME = %d\n", now_time.tv_sec, query_time.tv_sec, REFRESH_TIME);

    WMT_DBG_FUNC("[Thermal][0] = %d, [1] = %d, [2] = %d, [3] = %d, [4] = %d \n----\n", 
