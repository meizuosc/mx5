...
MTK_WCN_BOOL wmt_lib_handle_idc_msg(ipc_ilm_t *idc_infor)
{
	...
	if (MTK_WCN_BOOL_FALSE == bRet) {
		...
		lxop->op.au4OpData[0] = NULL ;//Add by MEIZU
	    }
	    else {
...
RETRY :
		...
		if (MTK_WCN_BOOL_FALSE == bRet) {			
			WMT_WARN_FUNC("put to active queue fail retry(%d)\n",retry);
			if(retry-- >=0){
			/*wakeup mtk_wmtd thread to proccess cmd  */
...
MTK_WCN_BOOL wmt_lib_hw_rst(VOID)
{
	...
	WMT_INFO_FUNC("call wmt_lib_hw_rst start\n");
	wmt_lib_state_init();
	...
	if (MTK_WCN_BOOL_FALSE == bRet) {
		...
		WMT_INFO_FUNC("OPID(WMT_OPID_HW_RST)ok\n");
	}
	return MTK_WCN_BOOL_TRUE;
}
...
MTK_WCN_BOOL wmt_lib_sw_rst(INT32 baudRst)
{
	...
	WMT_DBG_FUNC("wmt_lib_sw_rst start\n");
	/* <1> wmt state reset */
	wmt_lib_state_init();
