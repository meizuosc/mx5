...
INT32
wmt_core_tx(const PUINT8 pData,
...
INT32 wmt_core_ctrl(ENUM_WMT_CTRL_T ctrId, unsigned long *pPa1, unsigned long *pPa2)
{
	INT32 iRet = -1;
	WMT_CTRL_DATA ctrlData;
	unsigned long val1 = (pPa1) ? *pPa1 : 0;
	unsigned long  val2 = (pPa2) ? *pPa2 : 0;
...
static INT32 opfunc_func_on(P_WMT_OP pWmtOp)
{
	...
	mtk_wcn_set_power_off_flag(MTK_WCN_BOOL_TRUE);
...
static INT32 opfunc_pwr_sv(P_WMT_OP pWmtOp)
{
	...
	if (SLEEP == pWmtOp->au4OpData[0]) {
		...
		ret = wmt_core_tx((const PUINT8)(&WMT_SLEEP_CMD[0]), sizeof(WMT_SLEEP_CMD), &u4_result, 0);
		...
		ret = wmt_core_tx((const PUINT8)WMT_WAKEUP_CMD, sizeof(WMT_WAKEUP_CMD), &u4_result, 1);
		...
		/* (*kal_stp_flush_rx)(WMT_TASK_INDX); */
		ret = wmt_core_tx((const PUINT8)WMT_HOST_AWAKE_CMD, sizeof(WMT_HOST_AWAKE_CMD), &u4_result, 0);
		...
		if (osal_memcmp(evt_buf, (const PVOID)WMT_HOST_AWAKE_EVT, sizeof(WMT_HOST_AWAKE_EVT)) != 0) {
...
static INT32 opfunc_hw_rst(P_WMT_OP pWmtOp)
{
	...
	unsigned long ctrlPa1;
	unsigned long ctrlPa2;
	WMT_INFO_FUNC("opfunc_hw_rst start\n");
	...
	WMT_INFO_FUNC("opfunc_hw_rst end\n");
	return iRet;
...
static INT32 opfunc_sw_rst(P_WMT_OP pWmtOp)
{
	...
	WMT_INFO_FUNC("WMT-CORE: SW Rst start\n");
	iRet = wmt_core_stp_init();
	if (0 == iRet) {
		WMT_INFO_FUNC("WMT-CORE: SW Rst succeed\n");
		gMtkWmtCtx.eDrvStatus[WMTDRV_TYPE_WMT] = DRV_STS_FUNC_ON;
	} else {
		WMT_ERR_FUNC("WMT-CORE: SW Rst failed,uninit stp init\n");
		wmt_core_stp_deinit();
		gMtkWmtCtx.eDrvStatus[WMTDRV_TYPE_WMT] = DRV_STS_POWER_OFF;
	}
