...
static MTK_WCN_BOOL mtk_wcn_wmt_func_ctrl(ENUM_WMTDRV_TYPE_T type, ENUM_WMT_OPID_T opId)
{
	...
	if (WMTDRV_TYPE_WIFI == type)
		pSignal->timeoutValue = 4500; /*workaround, donot block system server/Init/Netd from longer than 5s, in case of ANR happens*/
	else
...
#endif
MTK_WCN_BOOL mtk_wcn_set_power_off_flag(MTK_WCN_BOOL value)
{
...
EXPORT_SYMBOL(mtk_wcn_set_power_off_flag);
