...
unsigned int mt_eint_get_count(unsigned int eint_num);
unsigned int mt_eint_get_wakeup_count(unsigned int eint_num);
...
int get_wifi_status(void)
{
	int wakeup_count = 0 ;
	int irq_count = 0 ;
	wakeup_count = mt_eint_get_wakeup_count(WIFI_IRQ);
    irq_count    = mt_eint_get_count(WIFI_IRQ) ;
	printk(KERN_CRIT"!!!!!!WIFI_WAKEUP!!!!! wakeup(%d)irq(%d)\n",
		  wakeup_count,irq_count);
	return 0 ;
}
...
#define CFG_WAKEUP_STATICS
#ifdef CFG_WAKEUP_STATICS
static struct early_suspend wakeup_early_suspend_desc = {
    .level = EARLY_SUSPEND_LEVEL_BLANK_SCREEN,
};

static void wakeup_early_suspend(struct early_suspend *h)
{
	lcd_off = 1 ;
 }

static void wakeup_late_resume(struct early_suspend *h)
{
	lcd_off = 0 ;
}
...
#endif
#define WIFI_EINT_PIN 4
static ssize_t wlan_threadloop_show(struct device *pdev,
		struct device_attribute *attr, char *buf)
{
	 struct net_device * netdev = container_of(pdev,struct net_device,dev); 
	 P_GLUE_INFO_T prGlueInfo = *((P_GLUE_INFO_T *) netdev_priv(netdev));
	 return snprintf(buf, PAGE_SIZE, "tx:%u,rx:%u,hif:%u\n",
			prGlueInfo->tx_loop,prGlueInfo->rx_loop,prGlueInfo->hif_loop);
} 

#if 0
static ssize_t wlan_irq_show(struct device *pdev,
		struct device_attribute *attr, char *buf)
{
	...
	count += snprintf(buf+count,PAGE_SIZE,"MCR_WHISR(%x)\n",value);
	return count ;
}

static ssize_t wlan_irq_store(struct device *pdev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if(input=0){
		kalDevRegWrite(prGlueInfo, MCR_WHLPCR, WHLPCR_INT_EN_CLR);
		mt_eint_mask(WIFI_EINT_PIN);
	}else {
		kalDevRegWrite(prGlueInfo, MCR_WHLPCR, WHLPCR_INT_EN_SET);
		mt_eint_unmask(WIFI_EINT_PIN);
	}
	return count ;
}
#endif
static struct device_attribute attrs[] = {
	__ATTR(threadloop, (S_IRUGO | S_IWGRP|S_IWUSR),
			wlan_threadloop_show,NULL),
#if 0
	__ATTR(irq, (S_IRUGO | S_IWGRP|S_IWUSR),
			wlan_irq_show,wlan_irq_store),
#endif
};

int wlan_create_sysfile(P_GLUE_INFO_T prGlueInfo)
{
	int attr_count = 0 ;
	int retval = 0 ;
	for (attr_count = 0; attr_count < ARRAY_SIZE(attrs); attr_count++) {
		retval = sysfs_create_file(&(prGlueInfo->prDevHandler->dev.kobj),
				&attrs[attr_count].attr);
		if (retval < 0) {
			dev_err(&(prGlueInfo->prDevHandler->dev),
...
int wlan_remove_sysfile(P_GLUE_INFO_T prGlueInfo)
{
	int attr_count = 0 ;
	for (attr_count--; attr_count >= 0; attr_count--) {
		sysfs_remove_file(&(prGlueInfo->prDevHandler->dev.kobj),
				&attrs[attr_count].attr);
	}
...
#ifdef CFG_WAKEUP_STATICS
wakeup_early_suspend_desc.suspend = wakeup_early_suspend ;
wakeup_early_suspend_desc.resume  = wakeup_late_resume ;
register_early_suspend(&wakeup_early_suspend_desc);
#endif
...
#if 0
	if(i4Status==0)
    wlan_create_sysfile(prGlueInfo);
#endif
	return i4Status;
} /* end of wlanProbe() */
...
#if 0
	wlan_remove_sysfile(prGlueInfo);
#endif
#if CFG_ENABLE_BT_OVER_WIFI
...
#ifdef CFG_WAKEUP_STATICS   
	unregister_early_suspend(&wakeup_early_suspend_desc);
#endif
