				       ...
				       IN UINT_8 aucMACAddress[], IN OUT PUINT_16 pu2PayloadLen)
{
	...
	if (prStaRec->ucDTIMPeriod==1) {
		u2ListenInterval = prStaRec->ucDTIMPeriod * MZ_LISTEN_BY_DTIM;
	}else if(prStaRec->ucDTIMPeriod>1){
    ...
    DBGLOG(INIT,INFO,("ListenInterval(%d)\n",u2ListenInterval));
			    ...
			    IN P_SW_RFB_T prSwRfb, IN UINT_8 aucBSSID[], OUT PUINT_16 pu2ReasonCode)
{
	...
	if ((prSwRfb->u2PacketLen - prSwRfb->u2HeaderLen) < REASON_CODE_FIELD_LEN) {
		//ASSERT(0);
		DBGLOG(SAA,INFO,("ERROR:packetlen(%d)Headlen(%d)\n",prSwRfb->u2PacketLen,prSwRfb->u2HeaderLen));
		*pu2ReasonCode = REASON_CODE_UNSPECIFIED ;
		return WLAN_STATUS_FAILURE;
	}
