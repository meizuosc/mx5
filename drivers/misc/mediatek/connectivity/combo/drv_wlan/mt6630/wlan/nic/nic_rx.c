...
VOID nicRxProcessEventPacket(IN P_ADAPTER_T prAdapter, IN OUT P_SW_RFB_T prSwRfb)
{
    ...
    DBGLOG(INIT, INFO, ("RX_EVENT: ID[%d][%s] SEQ[%u] LEN[%u]\n", 
        prEvent->ucEID,nicEvent2String(prEvent->ucEID),
        prEvent->ucSeqNum,
        ...
        while(QUEUE_IS_NOT_EMPTY(&prRxCtrl->rReceivedRfbList)) {
            ...
            while(QUEUE_IS_NOT_EMPTY(prTempRfbList)) {
				...
				do {
					...
					if (!is_lcd_off()||!kalIsWakeupByWlan(prAdapter)/* || !pvHeader*/)
						break;
	
					switch (prSwRfb->ucPacketType) {
					case RX_PKT_TYPE_RX_DATA:
						 if(pvHeader==NULL){
							DBGLOG(INIT,INFO,("RX_PKT_TYPE_RX_DATA is null\n"));
							break ;
						 }
					...
					case RX_PKT_TYPE_SW_DEFINED:
						...
						if(pvHeader==NULL){
							DBGLOG(INIT,INFO,("RX_PKT_TYPE_SW_DEFINED is null\n"));
							break ;
						 }
...
static inline int nicP2pRegisted(IN P_ADAPTER_T prAdapter)
{
	if(!prAdapter)
		return 0;
	return (prAdapter->fgIsP2PRegistered)&&
