...
VOID scnInit(IN P_ADAPTER_T prAdapter)
{
	...
	mutex_init(&prScanInfo->scanlock);
...
static int  scnCheckAndDumpDescList(P_SCAN_INFO_T prScanInfo,P_BSS_DESC_T bssDesc)
{
	char *pMin = (char*)(&prScanInfo->aucScanBuffer[0]);
    char *pMax = (char*)(&prScanInfo->aucScanBuffer[SCN_MAX_BUFFER_SIZE-1]);
	char *pBss = (char *)bssDesc ;
	P_BSS_DESC_T  pBssDesc ;
	int i = 0 ;
	
	if(pBss>=pMin&&pBss<=pMax)
		return 0 ;
	
	printk(KERN_ERR"Free next(%p)prev(%p)num(%d)\n",prScanInfo->rFreeBSSDescList.prNext,prScanInfo->rFreeBSSDescList.prPrev,prScanInfo->rFreeBSSDescList.u4NumElem);
	printk(KERN_ERR"Desc next(%p)pre(%p)num(%d)\n",prScanInfo->rBSSDescList.prNext,prScanInfo->rBSSDescList.prPrev,prScanInfo->rBSSDescList.u4NumElem);
	printk("pMin(%p)pBss(%p)pMax(%p)\n",pMin,pBss,pMax);
	
	for(i=0;i<CFG_MAX_NUM_BSS_LIST;i++){
		pBssDesc = (P_BSS_DESC_T )pMin ;
		printk(KERN_ERR"next(%p)prev(%p)\n",pBssDesc->rLinkEntry.prNext,pBssDesc->rLinkEntry.prPrev);
		pMin += ALIGN_4(sizeof(BSS_DESC_T));
	}
...
VOID scnUninit(IN P_ADAPTER_T prAdapter)
{
	...
	LINK_INITIALIZE(&prScanInfo->rFreeBSSDescList);
	LINK_INITIALIZE(&prScanInfo->rBSSDescList);
	mutex_destroy(&prScanInfo->scanlock);
	return;
}				/* end of scnUninit() */
				...
				IN BOOLEAN fgCheckSsid, IN P_PARAM_SSID_T prSsid)
{
	...
	mutex_lock(&prScanInfo->scanlock);

	prBSSDescList = &prScanInfo->rBSSDescList;

	/* Search BSS Desc from current SCAN result list. */
	LINK_FOR_EACH_ENTRY(prBssDesc, prBSSDescList, rLinkEntry, BSS_DESC_T) {
		scnCheckAndDumpDescList(prScanInfo,prBssDesc);
		if (EQUAL_MAC_ADDR(prBssDesc->aucBSSID, aucBSSID)) {
			if (fgCheckSsid == FALSE || prSsid == NULL) {
				mutex_unlock(&prScanInfo->scanlock);
				return prBssDesc;
			} else {
		       ...
		       IN ENUM_BSS_TYPE_T eBSSType, IN P_BSS_DESC_T SpecificprBssDesc)
{
	...
	if (SpecificprBssDesc) {
	  ...
	  mutex_lock(&prScanInfo->scanlock);
	  DBGLOG(INIT,INFO,("Search BSSDesc form current SCAN result list start\n"));
		/* Search BSS Desc from current SCAN result list. */
		LINK_FOR_EACH_ENTRY(prBssDesc, prBSSDescList, rLinkEntry, BSS_DESC_T) {
		scnCheckAndDumpDescList(prScanInfo,prBssDesc);
...
/* prBssDesc->u2RawLength=0; */
		...
		DBGLOG(INIT,INFO,("Search BSSDesc end\n"));
#if CFG_AUTO_CHANNEL_SEL_SUPPORT
