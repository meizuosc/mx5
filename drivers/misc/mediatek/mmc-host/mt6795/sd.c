...
/*1:insert ;0:removed */
 int msdc1_poll_sdcard_eint(void)
{
	int value = 0 ;
	value = mt_get_gpio_in(GPIO_SDHC_EINT_PIN);
	printk(KERN_ERR"%s:Sdcard irqvalue(%d)\n",__FUNCTION__,value);
    return value ;
}
#endif
...
#ifdef CONFIG_MZ_MA01
    if(hw->host_function==MSDC_SD&&(hw->flags&MSDC_REMOVABLE))
        mmc->pm_flags |= MMC_PM_SDCARD_POWER ;
#endif
