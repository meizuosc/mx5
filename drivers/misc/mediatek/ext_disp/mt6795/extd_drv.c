    ...
    for (;;)
    {
        ...
        else
        {
            for(layid = 0; layid < HW_OVERLAY_COUNT; layid++)
            {
                ...
                {
                    layer_3d_format |= mtkfb_query_buf_info(session_id, layid, lay_addr, 0) ;
                }
#endif                
            }
        ...
        hdmi_video_config(p->output_video_resolution, HDMI_VIN_FORMAT_RGB888, HDMI_VOUT_FORMAT_RGB888|layer_3d_format);        
#else
...
int hdmi_video_config(HDMI_VIDEO_RESOLUTION vformat, HDMI_VIDEO_INPUT_FORMAT vin, HDMI_VIDEO_OUTPUT_FORMAT vout)
{
    ...
    if((p->is_mhl_video_on == true)&&(p->vout != vout))
    {
        ///vformat = HDMI_VIDEO_RESOLUTION_NUM;  
