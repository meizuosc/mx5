...
//#define USE_SYSRAM
//#define RRC_MSG(...)   xlog_printk(ANDROID_LOG_DEBUG, "xlog/rrc", __VA_ARGS__)
//#define RRC_WRN(...)   xlog_printk(ANDROID_LOG_WARN,  "xlog/rrc", __VA_ARGS__)
//#define RRC_ERR(...)   xlog_printk(ANDROID_LOG_ERROR, "xlog/rrc", __VA_ARGS__)
#define RRC_MSG printk
#define RRC_WRN printk
#define RRC_ERR printk
...
static int is_touch_event = 0;
static int is_video_scenario = 0;

static int _rrc_sample_cnt = 0;
...
static struct task_struct *set_refresh_rate_task = NULL;
static int set_refresh_rate_task_wakeup = 0;


static wait_queue_head_t set_low_refresh_rate_wq;
static struct task_struct *set_low_refresh_rate_task = NULL;
static int set_low_refresh_rate_task_wakeup = 0;


static int rrc_curr_refresh_rate = 0;

static int _rrc_avg_fps_range = RRC_DRV_120Hz;

static int _is_new_change_refresh_event = 0;

static int _is_video_120hz_en = 0;
...
static int rrc_get_ddp_refresh_rate(void)
{
    ...
    printk("[RRC] get ddp refresh rate !!\n");
...
static int rrc_set_ddp_refresh_rate(int refresh_rate)
{
    ...
    printk("[RRC] set ddp refresh rate %d(%d)!!\n", target_rate, refresh_rate);
...
static int rrc_get_next_refresh_rate(void)
{
    ...
    for(i=RRC_DRV_TYPE_CAMERA_PREVIEW; i<=RRC_DRV_TYPE_VIDEO_WIFI_DISPLAY; i++){
        if(scenario_status[i] > 0){
            video_state++;
            if(_is_video_120hz_en && (i == RRC_DRV_TYPE_VIDEO_SWDEC_PLAYBACK || i == RRC_DRV_TYPE_VIDEO_PLAYBACK)){
...
static int rrc_set_low_refresh_rate_kthread_func(void *data)
{
    ...
    while(1)
    {
        ...
        while(1){
            ...
            if(_is_new_change_refresh_event){
                curr_refresh = rrc_get_curr_refresh_rate();
                //printk("[RRC_DRV] kthread_func : rrc_set_low_refresh_rate_kthread_func curr_refresh %d, sample_cnt %d !!\n", curr_refresh, rrc_low_sample_cnt);  
            ...
            if(++rrc_low_sample_cnt == RRC_DRV_LOW_SAMPLE_COUNT){
                rrc_low_sample_cnt = 0;
...
static int rrc_set_refresh_rate_kthread_func(void *data)
{
    ...
    while(1)
    {
        printk("[RRC_DRV] kthread_func : set_refresh_rate_task_wakeup %d (sleep)!!\n", set_refresh_rate_task_wakeup);  
        wait_event_interruptible(set_refresh_rate_wq, set_refresh_rate_task_wakeup);
        set_refresh_rate_task_wakeup = 0; 
        printk("[RRC_DRV] kthread_func : set_refresh_rate_task_wakeup %d (wake)!!\n", set_refresh_rate_task_wakeup);  
        ...
        printk("[RRC_DRV] kthread_func : curr %d, target %d!!\n", curr_refresh, target_refresh);
...
static int rrc_set_120Hz_kthread_func(void *data)
{
    /* A.1. set refresh_rate to 120Hz */
    
    /* A.2. monitor if FPS > 60FPS */
    
    /* A.2.1. if no: change refresh rate to 60Hz */
    
    /* A.2.2. if yes: monitor if FPS drop to 60FPS long time, change to 60Hz */
    
    
}

static int rrc_set_60Hz_kthread_func(void *data)
{
 
        
    /* 1. monitor for a while see that has touch event again */
    
    /* 1.a : if yes, postpone change to 60Hz for a while */
    
    /* 1.b : if no, change to 60Hz */   
...
//static int rrc_set_refresh_rate(int target_rate)
//{
//    
//    if (target_rate == RRC_DRV_120Hz){
//        /* 60Hz -> 120Hz (fast change)*/
//        
//        // trigger switch 120Hz flow
...
static int rrc_set_scenario_state(int scenario, int enable)
{
    ...
    if(enable) { 
        if( scenario == RRC_DRV_TYPE_TOUCH_EVENT){
            ...
            if( enable == 2 && (scenario == RRC_DRV_TYPE_VIDEO_SWDEC_PLAYBACK || scenario == RRC_DRV_TYPE_VIDEO_PLAYBACK)){
        ...
        status = scenario_back_status[scenario] ;
        if( _is_video_120hz_en && (scenario == RRC_DRV_TYPE_VIDEO_SWDEC_PLAYBACK || scenario == RRC_DRV_TYPE_VIDEO_PLAYBACK)){
    ...
    printk("[RRC_DRV]set_scenario: scenario %d, status %d, enable %d, switch %d, new_event %d!!\n", scenario, status, enable, switch_flag, new_event_flag);
...
static long rrc_ioctl(unsigned int cmd, unsigned long arg, struct file *file)
{
    ...
    {       
        ...
        case RRC_IOCTL_CMD_INIT: 
            RRC_MSG("[RRC_DRV] RRC Driver Initial and Lock\n");
        ...
        case RRC_IOCTL_CMD_SET_SCENARIO_TYPE:
            RRC_MSG("[RRC_DRV] RRC Driver SET scenario type!!\n");
            ...
            //RRC_MSG("[RRC_DRV] RRC Driver SET scenario type copy_from_user!!\n");
            ...
            RRC_MSG("[RRC_DRV] RRC Driver SET scenario type rrc_notify_scenario_event!!\n");
            result = rrc_notify_scenario_event(drv_data.scenario, drv_data.enable);  
            if(result < 0){
                RRC_MSG("[RRC_DRV] RRC Driver SET scenario type error %d!!\n", result);
                return -EFAULT;
            }

            //RRC_MSG("[RRC_DRV] RRC Driver SET scenario type copy_to_user!!\n");
        ...
        case RRC_IOCTL_CMD_DEINIT:
            ...
            RRC_MSG("[RRC_DRV] RRC Driver Deinit!!\n");
            if(*pStatus != RRC_PROCESS)
            {
...
/* Kernel interface */
static struct file_operations rrc_fops = {
	.owner		= THIS_MODULE,
	//.ioctl		= rrc_ioctl,
...
static int rrc_probe(struct platform_device *pdev)
{
	...
	{
	    RRC_MSG("Get RRC Device Major number (%d)\n", rrc_devno);
    }
...
static int __init rrc_init(void)
{
    ...
    RRC_MSG("RRC driver initialize\n");
	
    RRC_MSG("Register the RRC driver device\n");
    ...
    RRC_MSG("Register the RRC driver \n");    
    ...
    {
        printk("RRC: Cannot create rrc_set_refresh_rate_kthread_func!!\n");
        ret = -ENODEV;
        return ret;
    }
    ...
    {
        printk("RRC: Cannot create rrc_set_low_refresh_rate_kthread_func!!\n");
        ret = -ENODEV;
        return ret;
    }
