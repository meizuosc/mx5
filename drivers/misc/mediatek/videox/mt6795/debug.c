...
static void process_dbg_opt(const char *opt)
{
	...
	else if (0 == strncmp(opt, "switch_mode:", 12))
	{
		...
		primary_display_switch_path_test(sess_mode);
	}
	...
	else if (0 == strncmp(opt, "disp_sus", 8)) {
    	mz_suspend_disp();
	}
	else if (0 == strncmp(opt, "disp_res", 8)) {
    	mz_resume_disp();
	}
	else if (0 == strncmp(opt, "lcm_stop", 8)) {
    	mz_suspend_lcm();
	}
	else if (0 == strncmp(opt, "lcm_start", 9)) {
    	mz_resume_lcm();
	}
	...
	else if(0==strncmp(opt,"120keeplow:",9))
	{
	    extern void primary_display_enable_keep_low(unsigned int flag);	
	    char*p=(char*)opt+11;
	    UINT32 flag=simple_strtoul(p, &p, 10);
	    primary_display_enable_keep_low(flag);
        return;
	}
    else if (0 == strncmp(opt, "odbypass:", 9))
    {
        ...
        char *p = (char *)opt + 9;
        int bypass = (void*) simple_strtoul(p, &p, 16);
