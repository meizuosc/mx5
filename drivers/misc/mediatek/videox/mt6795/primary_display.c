...
static void primary_display_idlemgr_enter_idle(int need_lock)
{
			...
			disp_helper_get_option(DISP_HELPER_OPTION_IDLEMGR_SWTCH_DECOUPLE))
	{
		...
		if(!disp_helper_get_option(DISP_HELPER_OPTION_DEFAULT_DECOUPLE_MODE)){
			...
			__primary_display_switch_path(DISP_SESSION_DECOUPLE_MODE, pgc->session_id, need_lock, DISP_USER_IDLE);
	...
	// DISPMSG("idlemgr enter idle end\n");
	return ;
}
...
{
	int ret = 0;
	// DISPMSG("idlemgr leave idle begin\n");
	...
	if (primary_display_is_video_mode() &&
		...
		if(!disp_helper_get_option(DISP_HELPER_OPTION_DEFAULT_DECOUPLE_MODE)){
			...
			__primary_display_switch_path(session_mode_before_enter_idle, pgc->session_id, need_lock, DISP_USER_IDLE);
		}
	...
	// DISPMSG("idlemgr leave idle end\n");
	return;
}
...
static int _DL_switch_to_DC_fast(void)
{
	...
	cmdqRecReset(pgc->cmdq_handle_ovl1to2_config);
	if(pgc->decouple_type == DECOUPLE_UFOE){
		pgc->ovl2mem_path_handle = dpmgr_create_path(DDP_SCENARIO_PRIMARY_UFOE_MEMOUT, pgc->cmdq_handle_ovl1to2_config);
	}else if(pgc->decouple_type==DECOUPLE_OD){
		pgc->ovl2mem_path_handle = dpmgr_create_path(DDP_SCENARIO_PRIMARY_OD_MEMOUT, pgc->cmdq_handle_ovl1to2_config);
	}else{
		pgc->ovl2mem_path_handle = dpmgr_create_path(DDP_SCENARIO_PRIMARY_OVL_MEMOUT, pgc->cmdq_handle_ovl1to2_config);
	}
	if(pgc->ovl2mem_path_handle) {
...
int primary_display_esd_check(void)
{
	...
	_primary_path_lock(__func__);
	cur_stat = primary_get_state();

	if(cur_stat != DISP_ALIVE) {
		MMProfileLogEx(ddp_mmp_get_events()->esd_check_t, MMProfileFlagPulse, 1, 0);
		DISPCHECK("[ESD]primary display path is not alive (0x%x)?? -- skip esd check\n", cur_stat);
		_primary_path_unlock(__func__);
		goto done;
	}
...
static int primary_display_esd_check_worker_kthread(void *data)
{
	...
	while(1)
	{
		...
		{
			count++;
			msleep_interruptible(20000);//wait till system booted.
		}
		msleep_interruptible(5000); // esd check every 2s
...
static int _decouple_update_rdma_config_nolock()
{
...
static int _decouple_update_rdma_config()
{
	int ret;
	_primary_path_lock(__func__);
...
int primary_display_post_init(void)
{
	...
	if(disp_helper_get_stage() == DISP_HELPER_STAGE_NORMAL) {
		...
		if(decouple_update_rdma_config_thread == NULL) {
			decouple_update_rdma_config_thread = kthread_create(update_rdma_config_thread, NULL, "decouple_update_rdma_cfg");
...
int _display_set_lcm_refresh_rate(int fps)
{
	...
	int ret =0;
	static cmdqRecHandle cmdq_handle = NULL;
	static cmdqRecHandle cmdq_pre_handle = NULL;
	...
	if (fps == 120) {
		ret = __primary_display_switch_path(DISP_SESSION_DECOUPLE_MODE, MAKE_DISP_SESSION(DISP_SESSION_PRIMARY,0), 0, DISP_USER_FPS);
		/* adjust fps and od*/
		cmdqRecReset(cmdq_handle);
		_cmdq_insert_wait_frame_done_token_mira(cmdq_handle);
		...
		ret = __primary_display_switch_path(DISP_SESSION_DIRECT_LINK_MODE, MAKE_DISP_SESSION(DISP_SESSION_PRIMARY,0), 0, DISP_USER_FPS);
		od_need_start = 0;
	}
...
int primary_display_suspend(void)
{
	...
	primary_set_state(DISP_SLEEPED);
	_primary_path_unlock(__func__);
...
int primary_display_ipoh_hiberation_prepare(void)
{
	...
	if(disp_helper_get_option(DISP_HELPER_OPTION_DEFAULT_DECOUPLE_MODE)){
		...
		primary_display_idlemgr_kick(__func__, 0);
		__primary_display_switch_path(DISP_SESSION_DIRECT_LINK_MODE, MAKE_DISP_SESSION(DISP_SESSION_PRIMARY, 0), 0, DISP_USER_NORMAL);
		_primary_path_unlock(__func__);
	}
...
{
	DISPMSG("ipoh_restore_prepare in\n");
	primary_display_esd_check_enable(0);
	if(disp_helper_get_option(DISP_HELPER_OPTION_DEFAULT_DECOUPLE_MODE)){
		mmdvfs_set_step(SMI_BWC_SCEN_UI_IDLE, MMDVFS_VOLTAGE_HIGH);
		_primary_path_lock(__func__);
	...
	DISPMSG("ipoh_restore_prepare out\n");
	return 0;
}
...
int primary_display_trigger(int blocking, void *callback, int need_merge)
{
	...
	if(pgc->session_mode == DISP_SESSION_DIRECT_LINK_MODE){
		...
		if(need_merge == 0 || primary_is_sec()) {
			...
			DISPMSG("decouple need merge\n");
			atomic_set(&decouple_trigger_event, 1);
			wake_up_interruptible(&decouple_trigger_wq);
		}
...
static int decouple_trigger_worker_thread(void *data)
{
	...
	while(1)
	{
		wait_event_interruptible(decouple_trigger_wq, atomic_read(&decouple_trigger_event));
		dpmgr_wait_event(pgc->dpmgr_handle, DISP_PATH_EVENT_IF_VSYNC);
...
static int setup_disp_sec(disp_ddp_path_config *data_config, cmdqRecHandle cmdq_handle, int is_locked)
{
	...
	if((has_sec_layer != primary_is_sec())) {
		...
		if(pgc->session_mode == DISP_SESSION_DECOUPLE_MIRROR_MODE) {
			...
			if(is_locked)
				_primary_path_unlock(__func__);
			wait_event_interruptible(decouple_trigger_wq, atomic_read(&decouple_trigger_event)==0);
			if(is_locked)
				_primary_path_lock(__func__);
	...
	if (_should_config_ovl_input() && pgc->state == DISP_ALIVE) {
		...
		if (need_dc){
			...
			if(pgc->ovl_one_layer == 1) {
				DISPMSG("[DISPMODE] layer switch to DC\n");
				pgc->ovl_one_layer = 0;
				if(disp_helper_get_option(DISP_HELPER_OPTION_DEFAULT_DECOUPLE_MODE)){
					_primary_path_unlock(__func__);
					mmdvfs_set_step(SMI_BWC_SCEN_UI_IDLE, MMDVFS_VOLTAGE_HIGH);
					__primary_display_switch_path(DISP_SESSION_DECOUPLE_MODE, session_id, 1, DISP_USER_LAYER);
					mmdvfs_set_step(SMI_BWC_SCEN_UI_IDLE, MMDVFS_VOLTAGE_DEFAULT);
					_primary_path_lock(__func__);

				}else{
					__primary_display_switch_path(DISP_SESSION_DECOUPLE_MODE, session_id, 0, DISP_USER_LAYER);
				}
			}
		}else{
			if(one_layer_cnt >= one_layer_dl_condition){
				if(pgc->ovl_one_layer == 0) {
					DISPMSG("[DISPMODE] layer switch to DL\n");
					pgc->ovl_one_layer = 1;
					__primary_display_switch_path(DISP_SESSION_DIRECT_LINK_MODE, session_id, 0, DISP_USER_LAYER);
				}
			}else{
				one_layer_cnt ++;
			}
...
int primary_display_config_input_multiple(primary_disp_input_config* input)
{
	...
	//Check path config first, __primary_display_switch_path() will restore gce handle token, so it's safe for original ovl config to use
...
int primary_display_config_input(primary_disp_input_config* input)
{
	...
	int i=0;
	int layer =0;
	DISPMSG("primary_display_config_input begin\n");
	...
	_primary_path_unlock(__func__);
	DISPMSG("primary_display_config_input end\n");
	return ret;
}
				...
				if(disp_helper_get_option(DISP_HELPER_OPTION_DECOUPLE_UFOE_MODE)) {
					if(pgc->session_mode == DISP_SESSION_DECOUPLE_MODE){
						if (pq_place == 1) {
							...
							DISPMSG("ignore user_command when mode switch!!\n");
						}
					}else{
						if (pq_place == 2) {
							_cmdq_flush_config_handle_mira(handle, 0);
...
static int __primary_display_switch_mode(int sess_mode, unsigned int session)
{
	...
	DISPMSG("[DISPMODE]switch mode done, primary display is %s mode now\n", session_mode_spy(pgc->session_mode));
done:
...
static int __primary_display_switch_path(int request_mode, unsigned int session, int need_lock,enum DISP_USER user)
{
	...
	if (disp_helper_get_option(DISP_HELPER_OPTION_DEFAULT_DECOUPLE_MODE)
		...
		DISPMSG("[DISPMODE]: cur %s, req %s, res %s by user(%d)\n",
				session_mode_spy(pgc->session_mode),session_mode_spy(request_mode), session_mode_spy(dst_mode), user);
		if (pgc->session_mode != dst_mode) {
			if(disp_helper_get_option(DISP_HELPER_OPTION_DECOUPLE_UFOE_MODE)) {
				/*decouple mirror <--> decoule ufoe need temp mode directlink */
				if((pgc->session_mode == DISP_SESSION_DECOUPLE_MODE && dst_mode == DISP_SESSION_DECOUPLE_MIRROR_MODE) ||
						(pgc->session_mode == DISP_SESSION_DECOUPLE_MIRROR_MODE && dst_mode == DISP_SESSION_DECOUPLE_MODE)) {
					/* swith to directlink*/
					__primary_display_switch_mode(DISP_SESSION_DIRECT_LINK_MODE,session);
					/*re-select dst mode*/
					tmp_mode = dst_mode;
					dst_mode = select_session_mode(tmp_mode,user);
					DISPMSG("[DISPMODE]:reselect cur %s, req %s, res %s by user(%d)\n",
							session_mode_spy(pgc->session_mode),session_mode_spy(tmp_mode), session_mode_spy(dst_mode), user);
				}
			}
...
int primary_display_switch_mode(int sess_mode, unsigned int session, int force)
{
	...
	if(primary_display_is_mirror_mode() == _is_mirror_mode(sess_mode)) {
...
int primary_display_switch_mode_for_mmdvfs(int sess_mode, unsigned int session, int blocking)
{
	int ret = 0;
	if (disp_helper_get_option(DISP_HELPER_OPTION_DEFAULT_DECOUPLE_MODE)) {
		DISPMSG("[mmdvfs] switch to mode %d begin\n", sess_mode);
		_primary_protect_mode_switch();
		_primary_path_lock(__func__);
		...
		if (sess_mode == DISP_SESSION_DIRECT_LINK_MODE) {
			...
			DISPMSG("[mmdvfs]power state 1\n");
		}else if(sess_mode == DISP_SESSION_DECOUPLE_MODE) {
			pgc->mmdvfs_power_state = 0;
			DISPMSG("[mmdvfs]power state 0, display suspend %d\n",pgc->state == DISP_SLEEPED);
			if (pgc->state == DISP_ALIVE) {
				primary_display_idlemgr_kick(__func__, 0);
		...
		_primary_path_unlock(__func__);
		DISPMSG("[mmdvfs] switch to mode %d end\n", sess_mode);
	} else {
		DISPMSG("mmdvfs switch request skipped\n");
	}
	return ret;
}
...
int _set_hbm_by_cmdq(unsigned int onoff)
{
	int ret=0;
	LCM_DRIVER *lcm_drv = pgc->plcm->drv;
	cmdqRecHandle cmdq_handle_hbm = NULL;
	ret = cmdqRecCreate(CMDQ_SCENARIO_PRIMARY_DISP,&cmdq_handle_hbm);
	if(ret!=0)
	{
		DISPCHECK("fail to create primary cmdq handle for hbm\n");
		return -1;
	}

	cmdqRecReset(cmdq_handle_hbm);
	_cmdq_handle_clear_dirty(cmdq_handle_hbm);
	_cmdq_insert_wait_frame_done_token_mira(cmdq_handle_hbm);
	if (lcm_drv->set_hbm) {
		lcm_drv->set_hbm(cmdq_handle_hbm, onoff);
	}
	cmdqRecSetEventToken(cmdq_handle_hbm, CMDQ_SYNC_TOKEN_CONFIG_DIRTY);
	_cmdq_flush_config_handle_mira(cmdq_handle_hbm, 1);
	cmdqRecDestroy(cmdq_handle_hbm);
	cmdq_handle_hbm = NULL;
	return ret;
}

int primary_display_sethbm(unsigned int onoff)
{
	DISPFUNC();
	int ret = 0;
...
int primary_display_freeze(void)
{
	...
	if(disp_helper_get_option(DISP_HELPER_OPTION_DEFAULT_DECOUPLE_MODE)) {
		_primary_path_lock(__func__);
		__primary_display_switch_path(DISP_SESSION_DIRECT_LINK_MODE, MAKE_DISP_SESSION(DISP_SESSION_PRIMARY,0), 0, DISP_USER_FORCE);
		_primary_path_unlock(__func__);
	}
	return 0;
}
...
static int stop_smart_ovl_nolock(int *TWO_PIPE_opt_backup, int *IDLEMGR_DISABLE_ROUTINE_IRQ_backup)
{
	int ret;
	*TWO_PIPE_opt_backup = disp_helper_get_option(DISP_HELPER_OPTION_TWO_PIPE_INTERFACE_PATH);
	*IDLEMGR_DISABLE_ROUTINE_IRQ_backup = disp_helper_get_option(DISP_HELPER_OPTION_IDLEMGR_DISABLE_ROUTINE_IRQ);
	disp_helper_set_option(DISP_HELPER_OPTION_TWO_PIPE_INTERFACE_PATH, 0);
	disp_helper_set_option(DISP_HELPER_OPTION_IDLEMGR_DISABLE_ROUTINE_IRQ, 0);
	return 0;
}

static int restart_smart_ovl_nolock(int TWO_PIPE_opt_backup, int IDLEMGR_DISABLE_ROUTINE_IRQ_backup)
{
	disp_helper_set_option(DISP_HELPER_OPTION_TWO_PIPE_INTERFACE_PATH, TWO_PIPE_opt_backup);
	disp_helper_set_option(DISP_HELPER_OPTION_IDLEMGR_DISABLE_ROUTINE_IRQ, IDLEMGR_DISABLE_ROUTINE_IRQ_backup);
	return 0;
}

/*add for smart ovl*/
static int TWO_PIPE_opt_backup, IDLEMGR_DISABLE_ROUTINE_IRQ_backup;
...
int display_enter_tui()
{
	...
	//stop_smart_ovl_nolock(&TWO_PIPE_opt_backup, &IDLEMGR_DISABLE_ROUTINE_IRQ_backup);

	tui_session_mode_backup = pgc->session_mode;
	__primary_display_switch_path(DISP_SESSION_DECOUPLE_MODE, pgc->session_id, 0, DISP_USER_TUI);
...
int display_exit_tui()
{
	...
	msleep(32);
	__primary_display_switch_path(tui_session_mode_backup, pgc->session_id, 0, DISP_USER_TUI);
	//DISP_REG_SET(NULL, DISP_REG_RDMA_INT_ENABLE, 0xffffffff);

	//restart_smart_ovl_nolock(TWO_PIPE_opt_backup, IDLEMGR_DISABLE_ROUTINE_IRQ_backup);
	_primary_path_unlock(__func__);
