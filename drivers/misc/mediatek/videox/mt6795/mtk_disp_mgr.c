...
int disp_destroy_session (disp_session_config* config)
{
    ...
    if(DISP_SESSION_TYPE(session) == DISP_SESSION_MEMORY )
	{
		disp_od_mhl_force(1);// unlock OD when exit whd mode
        ovl2mem_deinit();
       /// _ovl2mem_out_cached_config.mode = 0;
	}
	else if(DISP_SESSION_TYPE(session) == DISP_SESSION_EXTERNAL)
    {
    	disp_od_mhl_force(1);// unlock OD when apply mhl mode
#ifdef CONFIG_MTK_HDMI_SUPPORT        
...
int _ioctl_trigger_session(unsigned long arg)
{
	...
	{
	    DISPPR_FENCE("T+/%s%d/%d/%d\n", disp_session_mode_spy(session_id), DISP_SESSION_DEV(session_id), config.present_fence_idx,config.need_merge);
    }
    else
    {
