...
#if 0
	DISP_REG_SET_FIELD(handle, DATAPATH_CON_FLD_LAYER_GREQ,
			idx_off_site+DISP_REG_OVL_DATAPATH_CON, 0x6);
		...
		void *handle)
{
	...
	DDPMSG("ovl%d, layer=%d, source=%s, off(x=%d, y=%d), dst(%d, %d, %d, %d),pitch=%d,"
		...
		"sur_aen=%d,sur_alpha=0x%x, yuv_range=%d, sec=%d,ovlsec=%d\n",
		...
		dst_alpha<<2 | src_alpha, yuv_range, sec, is_engine_sec);
#else
			...
			void *handle)
{
	...
	if (handle && ovl_idx==0 && ((primary_display_is_decouple_mode()==1) ||(primary_display_is_video_mode()==0))){
