...
void set_color_bypass(DISP_MODULE_ENUM module, int bypass, void* cmdq_handle)
{
    ...
    {
        // disable R2Y/Y2R in Color Wrapper
        DISP_REG_SET(cmdq_handle, DISP_COLOR_CM1_EN + offset, 0);
        DISP_REG_SET(cmdq_handle, DISP_COLOR_CM2_EN + offset, 0);

        DISP_REG_SET(cmdq_handle, DISP_COLOR_CFG_MAIN + offset,(1<<29)); // all bypass
        DISP_REG_SET(cmdq_handle, DISP_COLOR_START + offset, 0x00000003); //color start
    }
    else
    {
        DISP_REG_SET(cmdq_handle, DISP_COLOR_CFG_MAIN + offset,(0<<29)); //color enable
        DISP_REG_SET(cmdq_handle, DISP_COLOR_START + offset, 0x00000001); //color start

        // enable R2Y/Y2R in Color Wrapper
        DISP_REG_SET(cmdq_handle, DISP_COLOR_CM1_EN + offset, 1);
        DISP_REG_SET(cmdq_handle, DISP_COLOR_CM2_EN + offset, 1);   // also set no rounding on Y2R
    }
}
#endif
