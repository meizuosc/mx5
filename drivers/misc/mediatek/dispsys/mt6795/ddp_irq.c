...
irqreturn_t disp_irq_handler(int irq, void *dev_id)
{
	...
	if(irq==dispsys_irq[DISP_REG_DSI0] || irq==dispsys_irq[DISP_REG_DSI1])
    {
		...
		if (atomic_read(&esd_check_bycmdq) == 1)
		{
	...
	{                                                                                                                                                                                       
		...
		reg_val = DISP_REG_GET(DISP_REG_RDMA_INT_STATUS+DISP_RDMA_INDEX_OFFSET(index));                                                                                                  
		if (reg_val & (1 << 0)) {                                                                          
		...
		if (reg_val & (1 << 4)) {
			...
			MMProfileLogEx(ddp_mmp_get_events()->rdma_underflow, MMProfileFlagPulse, 
					cnt_rdma_underflow, dsi_line_cnt);
			MMProfileLogEx(ddp_mmp_get_events()->SCREEN_UPDATE[index], MMProfileFlagPulse, reg_val, 0);                                                                        
		...
		disp_rdma_notify(reg_val);

		/* clear intr */                                                                                   
		DISP_CPU_REG_SET(DISP_REG_RDMA_INT_STATUS + DISP_RDMA_INDEX_OFFSET(index), ~reg_val);                  
	} 
	...
	else if(irq==dispsys_irq[DISP_REG_OD])                                                                                                                                                 
	{
		module = DISP_REG_OD;
		disp_od_irq_handler();
	}
