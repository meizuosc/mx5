...
static int wdma_reset(DISP_MODULE_ENUM module,void * handle) 
{
    ...
    if (!handle) {
		...
		cmdqRecPoll(handle, disp_addr_convert(idx*DISP_WDMA_INDEX_OFFSET+DISP_REG_WDMA_FLOW_CTRL_DBG), 1, 0x1);
        //add comdq polling
    }
...
#else
    DDPDBG("module %s, src(w=%d,h=%d), clip(x=%d,y=%d,w=%d,h=%d),out_fmt=%s,dst_address=0x%lx,dst_p=%d,spific_alfa=%d,alpa=%d,handle=0x%p,sec%d\n",
        ddp_get_module_name(module),srcWidth,srcHeight,
        clipX,clipY,clipWidth,clipHeight,  
        fmt_string(out_format),dstAddress,dstPitch,  
        useSpecifiedAlpha,alpha,handle,sec);
