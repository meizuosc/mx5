...
int rdma_enable_irq(DISP_MODULE_ENUM module, void *handle, DDP_IRQ_LEVEL irq_level)
{
	...
	switch(irq_level)
	{
		case DDP_IRQ_LEVEL_ALL:
		    DISP_REG_SET(handle,DISP_RDMA_INDEX_OFFSET(idx)+ DISP_REG_RDMA_INT_ENABLE, 0x1E);
			break;
		case DDP_IRQ_LEVEL_ERROR:
			DISP_REG_SET(handle,DISP_RDMA_INDEX_OFFSET(idx)+ DISP_REG_RDMA_INT_ENABLE, 0x18);	// only enable underflow/abnormal irq
			break;
		case DDP_IRQ_LEVEL_NONE:
			DISP_REG_SET(handle,DISP_RDMA_INDEX_OFFSET(idx)+ DISP_REG_RDMA_INT_ENABLE, 0x0);
			break;
		default:
			break;
	}
...
int rdma_start(DISP_MODULE_ENUM module, void * handle) {
    ...
    DISP_REG_SET(handle,DISP_RDMA_INDEX_OFFSET(idx)+ DISP_REG_RDMA_INT_ENABLE, 0x1E);
    DISP_REG_SET_FIELD(handle,GLOBAL_CON_FLD_ENGINE_EN, DISP_RDMA_INDEX_OFFSET(idx)+ DISP_REG_RDMA_GLOBAL_CON, 1);
...
int rdma_stop(DISP_MODULE_ENUM module,void * handle) {
    ...
    DISP_REG_SET_FIELD(handle,GLOBAL_CON_FLD_ENGINE_EN, DISP_RDMA_INDEX_OFFSET(idx)+ DISP_REG_RDMA_GLOBAL_CON, 0);
    DISP_REG_SET(handle,DISP_RDMA_INDEX_OFFSET(idx)+ DISP_REG_RDMA_INT_ENABLE, 0);
    DISP_REG_SET(handle,DISP_RDMA_INDEX_OFFSET(idx)+ DISP_REG_RDMA_INT_STATUS, 0);
    return 0;
}
...
int rdma_reset(DISP_MODULE_ENUM module,void * handle) {
    ...
    DISP_REG_SET_FIELD(handle,GLOBAL_CON_FLD_SOFT_RESET, DISP_RDMA_INDEX_OFFSET(idx)+ DISP_REG_RDMA_GLOBAL_CON, 1);
    while((DISP_REG_GET(DISP_RDMA_INDEX_OFFSET(idx)+ DISP_REG_RDMA_GLOBAL_CON)&0x700)==0x100)
    {
        delay_cnt++;
        udelay(10);
        if(delay_cnt>10000)
        {
            ret = -1;
            DDPERR("rdma%d_reset timeout, stage 1! DISP_REG_RDMA_GLOBAL_CON=0x%x \n", idx, DISP_REG_GET(DISP_RDMA_INDEX_OFFSET(idx)+ DISP_REG_RDMA_GLOBAL_CON));
            break;
        }
    }      
    DISP_REG_SET_FIELD(handle,GLOBAL_CON_FLD_SOFT_RESET, DISP_RDMA_INDEX_OFFSET(idx)+ DISP_REG_RDMA_GLOBAL_CON, 0);
    delay_cnt =0;
    while((DISP_REG_GET(DISP_RDMA_INDEX_OFFSET(idx)+ DISP_REG_RDMA_GLOBAL_CON)&0x700)!=0x100)
    {
        delay_cnt++;
...
void rdma_set_ultra(unsigned int idx, unsigned int width, unsigned int height, unsigned int bpp, unsigned int frame_rate,void * handle)
{
  ...
  DISP_REG_SET(handle,DISP_RDMA_INDEX_OFFSET(idx)+ DISP_REG_RDMA_MEM_GMC_SETTING_0, 
    ultra_low_level|(pre_ultra_low_ofs<<8)|(ultra_high_ofs<<16)|(pre_ultra_high_ofs<<24));

  DISP_REG_SET_FIELD(handle,FIFO_CON_FLD_OUTPUT_VALID_FIFO_THRESHOLD, DISP_RDMA_INDEX_OFFSET(idx)+ DISP_REG_RDMA_FIFO_CON, fifo_valid_size);
                    ...
                    void * handle)
{
    ...
    {
	    DISP_REG_SET_FIELD(handle,SIZE_CON_0_FLD_MATRIX_ENABLE, DISP_RDMA_INDEX_OFFSET(idx) + DISP_REG_RDMA_SIZE_CON_0, 1);
	    DISP_REG_SET_FIELD(handle,SIZE_CON_0_FLD_MATRIX_INT_MTX_SEL, DISP_RDMA_INDEX_OFFSET(idx) + DISP_REG_RDMA_SIZE_CON_0, color_matrix);
    }
    ...
    {
        DISP_REG_SET_FIELD(handle,SIZE_CON_0_FLD_MATRIX_ENABLE, DISP_RDMA_INDEX_OFFSET(idx) + DISP_REG_RDMA_SIZE_CON_0, 0);
        DISP_REG_SET_FIELD(handle,SIZE_CON_0_FLD_MATRIX_INT_MTX_SEL, DISP_RDMA_INDEX_OFFSET(idx) + DISP_REG_RDMA_SIZE_CON_0, 0);
    }
    
    DISP_REG_SET_FIELD(handle,GLOBAL_CON_FLD_ENGINE_EN, DISP_RDMA_INDEX_OFFSET(idx)+ DISP_REG_RDMA_GLOBAL_CON, 1);

    DISP_REG_SET_FIELD(handle,GLOBAL_CON_FLD_MODE_SEL, DISP_RDMA_INDEX_OFFSET(idx)+ DISP_REG_RDMA_GLOBAL_CON, mode);
    // FORMAT & SWAP only works when RDMA memory mode, set both to 0 when RDMA direct link mode.
    DISP_REG_SET_FIELD(handle,MEM_CON_FLD_MEM_MODE_INPUT_FORMAT, DISP_RDMA_INDEX_OFFSET(idx)+ DISP_REG_RDMA_MEM_CON, ((mode == RDMA_MODE_DIRECT_LINK) ? 0 : input_format_reg&0xf));
    DISP_REG_SET_FIELD(handle,MEM_CON_FLD_MEM_MODE_INPUT_SWAP, DISP_RDMA_INDEX_OFFSET(idx)+ DISP_REG_RDMA_MEM_CON, ((mode == RDMA_MODE_DIRECT_LINK) ? 0 : input_swap));

	if(sec != DISP_SECURE_BUFFER) {
    	DISP_REG_SET(handle,DISP_RDMA_INDEX_OFFSET(idx)+ DISP_REG_RDMA_MEM_START_ADDR, address);
	} else {
		int m4u_port;
		...
		cmdqRecWriteSecure(handle, disp_addr_convert(DISP_RDMA_INDEX_OFFSET(idx)+DISP_REG_RDMA_MEM_START_ADDR), 
    ...
    DISP_REG_SET(handle,DISP_RDMA_INDEX_OFFSET(idx)+ DISP_REG_RDMA_MEM_SRC_PITCH, pitch);
    DISP_REG_SET(handle,DISP_RDMA_INDEX_OFFSET(idx)+ DISP_REG_RDMA_INT_ENABLE, 0x1F);
    DISP_REG_SET_FIELD(handle,SIZE_CON_0_FLD_OUTPUT_FRAME_WIDTH, DISP_RDMA_INDEX_OFFSET(idx)+ DISP_REG_RDMA_SIZE_CON_0, width);
    DISP_REG_SET_FIELD(handle,SIZE_CON_1_FLD_OUTPUT_FRAME_HEIGHT, DISP_RDMA_INDEX_OFFSET(idx)+ DISP_REG_RDMA_SIZE_CON_1, height);

    rdma_set_ultra(idx, width, height, bpp, rdma_fps[idx], handle);	

	// xuecheng, for video mode low power test
#if 0
	DISP_REG_SET(handle, DISP_RDMA_INDEX_OFFSET(idx)+ DISP_REG_RDMA_MEM_GMC_SETTING_1, 0x99);
#else
	DISP_REG_SET(handle, DISP_RDMA_INDEX_OFFSET(idx)+ DISP_REG_RDMA_MEM_GMC_SETTING_1, 0x60);
#endif
#if 1
    if(ufoe_enable==0)  //UFOE bypassed, enable RDMA underflow intr, else disable RDMA underflow intr
    {
	    DISP_REG_SET_FIELD(handle,FIFO_CON_FLD_FIFO_UNDERFLOW_EN, DISP_RDMA_INDEX_OFFSET(idx)+ DISP_REG_RDMA_FIFO_CON, 1);	 
        // DISP_REG_SET_FIELD(handle,FIFO_CON_FLD_OUTPUT_VALID_FIFO_THRESHOLD, DISP_RDMA_INDEX_OFFSET(idx)+ DISP_REG_RDMA_FIFO_CON, 0x100);
    }
    else
    {
	    DISP_REG_SET_FIELD(handle,FIFO_CON_FLD_FIFO_UNDERFLOW_EN, DISP_RDMA_INDEX_OFFSET(idx)+ DISP_REG_RDMA_FIFO_CON, 1);	    
		// force set rdma output threshold to maximum value, to avoid ufoe caused rdma underflow.
        DISP_REG_SET_FIELD(handle,FIFO_CON_FLD_OUTPUT_VALID_FIFO_THRESHOLD, DISP_RDMA_INDEX_OFFSET(idx)+ DISP_REG_RDMA_FIFO_CON, 496); //FHD:304, HD:203, QHD:151
        //DISP_REG_SET_FIELD(handle,FIFO_CON_FLD_OUTPUT_VALID_FIFO_THRESHOLD, DISP_RDMA_INDEX_OFFSET(idx)+ DISP_REG_RDMA_FIFO_CON, width*3*3/16/2); //FHD:304, HD:203, QHD:151
    }
#endif
    return 0;
}
...
int rdma_build_cmdq(DISP_MODULE_ENUM module,void *cmdq_trigger_handle, CMDQ_STATE state)
{
	...
	{
	    DISP_REG_SET_FIELD(cmdq_trigger_handle,FIFO_CON_FLD_FIFO_UNDERFLOW_EN, DISP_RDMA_INDEX_OFFSET(idx)+ DISP_REG_RDMA_FIFO_CON, 0);
	}
	else if(state == CMDQ_AFTER_STREAM_SOF)
	{
	    DISP_REG_SET_FIELD(cmdq_trigger_handle,FIFO_CON_FLD_FIFO_UNDERFLOW_EN, DISP_RDMA_INDEX_OFFSET(idx)+ DISP_REG_RDMA_FIFO_CON, 1);
	}
	}
	return 0;
}
...
void rdma_set_target_line(DISP_MODULE_ENUM module, unsigned int line,void * handle)
{
    ...
    DISP_REG_SET(handle,DISP_RDMA_INDEX_OFFSET(idx)+ DISP_REG_RDMA_TARGET_LINE, line);
}
...
static int setup_rdma_sec(DISP_MODULE_ENUM module, disp_ddp_path_config* pConfig, void *handle)
{
	...
	cmdq_engine = CMDQ_ENG_DISP_RDMA0 + rdma_idx;
