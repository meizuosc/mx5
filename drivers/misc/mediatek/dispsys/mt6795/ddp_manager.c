...
static int assign_default_irqs_table(DDP_SCENARIO_ENUM scenario,DDP_IRQ_EVENT_MAPPING * irq_events)
{
    ...
    switch(scenario)
    {
        ...
        case DDP_SCENARIO_PRIMARY_ALL_UFOE_WDMA:
        ...
        case DDP_SCENARIO_PRIMARY_RDMA_UFOE_MEMOUT:
...
int dpmgr_path_add_memout(disp_path_handle dp_handle, ENGINE_DUMP engine,void * cmdq_handle )
{
	...
	if(engine == ENGINE_OVL0 || engine == ENGINE_OVL1)
	    handle->scenario = DDP_SCENARIO_PRIMARY_ALL;
    else if (engine==ENGINE_UFOE)
		handle->scenario = DDP_SCENARIO_PRIMARY_ALL_UFOE_WDMA ;
	else if(engine == ENGINE_OD)
		handle->scenario = DDP_SCENARIO_PRIMARY_OD_1TO2;
	else
		ASSERT(0);
    // update connected
...
{
    int ret = 0;
    DDP_SCENARIO_ENUM disconnect_scenario, target_scenario;
    ASSERT(dp_handle != NULL);
    ddp_path_handle handle = (ddp_path_handle)dp_handle;
	...
	if(handle->scenario == DDP_SCENARIO_PRIMARY_ALL) {
		disconnect_scenario = DDP_SCENARIO_PRIMARY_OVL_MEMOUT;
		target_scenario = DDP_SCENARIO_PRIMARY_DISP;
	} else if(handle->scenario == DDP_SCENARIO_SUB_ALL) {
		disconnect_scenario = DDP_SCENARIO_SUB_OVL_MEMOUT;
		target_scenario = DDP_SCENARIO_SUB_DISP;
	} else if(handle->scenario == DDP_SCENARIO_PRIMARY_ALL_UFOE_WDMA) {
		disconnect_scenario = DDP_SCENARIO_PRIMARY_RDMA_UFOE_MEMOUT;
		target_scenario = DDP_SCENARIO_PRIMARY_DISP;
	} else if(handle->scenario == DDP_SCENARIO_PRIMARY_OD_1TO2) {
		disconnect_scenario = DDP_SCENARIO_PRIMARY_OD_MEMOUT;
		target_scenario = DDP_SCENARIO_PRIMARY_DISP;
	} else {
		DDPMSG("error %s: scenario is wrong %d\n", __func__, handle->scenario);
		BUG();
	}
	
    ddp_disconnect_path(disconnect_scenario, cmdq_handle);

    handle->scenario = target_scenario;
    // update connected
    ddp_connect_path(handle->scenario,cmdq_handle);
    ddp_mutex_set(handle->hwmutexid, handle->scenario, handle->mode, cmdq_handle);
...
int dpmgr_path_enable_irq(disp_path_handle dp_handle, void * cmdq_handle, DDP_IRQ_LEVEL irq_level)
{
	...
	for( i=module_num-1; i >= 0  ;i--)
	{
	    ...
	    if(ddp_modules_driver[module_name] != 0)
	    {
	        ...
	        {
	            DISP_LOG_D("scenario %s, module %s enable irq level %d\n",ddp_get_scenario_name(handle->scenario),
...
int dpmgr_path_reset(disp_path_handle dp_handle, int encmdq)
{
    ...
    for( i=0; i< module_num;i++)
    {
        ...
        if(ddp_modules_driver[module_name] != 0)
        {
            ...
            {
                DISP_LOG_D("scenario %s  reset module %s \n",ddp_get_scenario_name(handle->scenario),
                    ddp_get_module_name(module_name));
...
int dpmgr_path_user_cmd(disp_path_handle dp_handle, int msg, unsigned long arg, void *cmdqhandle)
{
    ...
    switch(msg)
    {
        ...
        case DISP_IOCTL_PQ_GET_DISP_STATUS:
        {
        	...
        	extern int primary_display_is_decouple_mode(void);
            ...
            st.suspend = primary_display_is_sleepd();
        	if(copy_to_user((void*)arg, &st, sizeof(DISP_PQ_STATUS)))
        	{
        		DISP_LOG_I("DISP_IOCTL_PQ_GET_DISP_STATUS, copy_to_user failed\n");
        	       ret = -EFAULT;
        	}
        	ret = 0;
        	break;
