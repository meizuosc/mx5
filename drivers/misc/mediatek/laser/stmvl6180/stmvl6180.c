...
static int laser_cal_data[2] = {0xFFFFFFFF, 0xFFFFFFFF};
	...
	switch(a_u4Command)
	{
		...
		case VL6180_IOCTL_GETDATA:
			...
			else	if( g_s4Laser_Opened == 2 )	
			{
				...
				ParamVal = Range.range_mm;
				LOG_INF("[stmvl6180]Get result distance:%d\n", ParamVal);
...
static int Laser_Open(struct inode * a_pstInode, struct file * a_pstFile)
{
    LOG_INF("Start \n");
    /*
     * FIXME: calibration data is random when calibration hasn't not been done, this
     * will cause the original offset value is overwriten. Discuss this.
     */
#if 0
    if ((g_Laser_OffsetCalib == 0xFFFFFFFF) || (g_Laser_XTalkCalib == 0xFFFFFFFF)) {
		laser_data_read_emmc(laser_cal_data, sizeof(laser_cal_data));
		g_Laser_OffsetCalib = laser_cal_data[0];
		g_Laser_XTalkCalib = laser_cal_data[1];
		LOG_INF("stmvl6180 cal data from emmc, offset_cal:%d, XTalk_cal:%d.\n", g_Laser_OffsetCalib, g_Laser_XTalkCalib);
    }
#endif
