...
extern int secmem_api_unref_pa(u32 sec_handle);
...
/* Define weak dummy function for integration development */
	...
	tuiAllocBuffer_t *allocbuffer, size_t allocsize, uint32_t number)
{
	...
	ret = secmem_api_alloc_pa(4096, allocsize*number+TUI_EXTRA_MEM_SIZE, &refcount, &sec_handle);
	if (ret == 0) {
		g_tui_secmem_handle = sec_handle;
		allocbuffer[0].pa = (uint64_t) sec_handle;
		allocbuffer[1].pa = (uint64_t) (sec_handle + allocsize);
	} else {
		return TUI_DCI_ERR_INTERNAL_ERROR;
	}
...
void hal_tui_free(void)
{
	...
	if (g_tui_secmem_handle) {
		secmem_api_unref_pa(g_tui_secmem_handle);
