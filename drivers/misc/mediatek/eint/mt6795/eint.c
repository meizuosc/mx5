...
typedef struct 
{
    ...
    unsigned int wakeup_count[EINT_MAX_CHANNEL];
    unsigned int need_wakelock[EINT_MAX_CHANNEL];
} eint_func;
...
#ifdef CONFIG_OF_IRQ
static unsigned int irq_number[EINT_MAX_CHANNEL+1] ;
#endif
...
#define EINT_WAKETIME  (250*HZ)/1000
...
{
    wake_lock_timeout(&EINT_suspend_lock, EINT_WAKETIME);
}
...
int mt_eint_to_irq(unsigned eint_number)
{
	if(eint_number<0||eint_number>EINT_MAX_CHANNEL)
		return -EINVAL ;
	return irq_number[eint_number] ;
}
/*
...
static int __init mt_eint_init(void)
{
    ...
    eint_drv->get_wakeup_count = mt_eint_get_wakeup_count;
...
EXPORT_SYMBOL(mt_eint_to_irq);
EXPORT_SYMBOL(mt_eint_set_deint);
EXPORT_SYMBOL(mt_eint_clr_deint);
EXPORT_SYMBOL(mt_eint_get_wakeup_count);
#if defined(EINT_TEST)
