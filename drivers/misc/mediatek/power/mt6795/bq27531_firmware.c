	...
	//for(i=0;i<5;i++)//test 5 cmd
	{
		...
		switch(bqfs_index[i].i2c_cmd)
		{
			case 'W':
				...
				pr_notice("ww_debug write %d\n", ret);
				break;
			case 'X':
				pr_notice("ww_debug sleep %d\n", bqfs_index[i].i2c_addr);
			...
			case 'C':
				{
					...
					for(j=0;j<bqfs_index[i].data_size-1;j++)
					{
						...
						pr_notice("ww_debug !(%d) data1=0x%x, data2=0x%x\n", j, data1, data2);
						if(data1!=data2)
						{
							pr_notice("ERROR! bq27531 fw upgrade error! data1=0x%x, data2=0x%x\n", data1, data2);
							kfree(buf);
...
int bq27531_check_fw_ver(void)
{
	...
	pr_notice("ww_debug chi id =0x%x, fw ver = 0x%x, dfver = 0x%x\n", chipid, fwver, dfver);
