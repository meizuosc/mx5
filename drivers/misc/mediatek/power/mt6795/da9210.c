...
kal_uint32 da9210_read_byte(kal_uint8 cmd, kal_uint8 *returnData)
{
    ...
    if (ret < 0)
    {
        pr_notice("[da9210_read_byte] ret=%d\n", ret);
...
kal_uint32 da9210_write_byte(kal_uint8 cmd, kal_uint8 writeData)
{
    ...
    if (ret < 0)
    {
        pr_notice("[da9210_write_byte] ret=%d\n", ret);
...
kal_uint32 da9210_read_interface (kal_uint8 RegNum, kal_uint8 *val, kal_uint8 MASK, kal_uint8 SHIFT)
{
    ...
    //pr_notice("[da9210_read_interface] Reg[%x]=0x%x\n", RegNum, da9210_reg);

    da9210_reg &= (MASK << SHIFT);
    *val = (da9210_reg >> SHIFT);

    //pr_notice("[da9210_read_interface] val=0x%x\n", *val);
...
kal_uint32 da9210_config_interface (kal_uint8 RegNum, kal_uint8 val, kal_uint8 MASK, kal_uint8 SHIFT)
{
    ...
    ret = da9210_read_byte(RegNum, &da9210_reg);
    //pr_notice("[da9210_config_interface] Reg[%x]=0x%x\n", RegNum, da9210_reg);

    da9210_reg &= ~(MASK << SHIFT);
    da9210_reg |= (val << SHIFT);

    ret = da9210_write_byte(RegNum, da9210_reg);
    //pr_notice("[da9210_config_interface] write Reg[%x]=0x%x\n", RegNum, da9210_reg);

    // Check
    //da9210_read_byte(RegNum, &da9210_reg);
    //pr_notice("[da9210_config_interface] Check Reg[%x]=0x%x\n", RegNum, da9210_reg);
...
void da9210_dump_register(void)
{
    kal_uint8 i=0;
    //----------------------------------------------------------------
    pr_notice("[da9210] page 0,1: ");
    pr_notice("[0x%x]=0x%x ", 0x0, da9210_get_reg_value(0x0));
    for (i=0x50;i<=0x5D;i++) {
        pr_notice("[0x%x]=0x%x ", i, da9210_get_reg_value(i));
    }
    for (i=0xD0;i<=0xD9;i++) {
        pr_notice("[0x%x]=0x%x ", i, da9210_get_reg_value(i));
    }
...
int set_da9210_buck_en(int en_bit)
{
    ...
    if(g_da9210_driver_ready==1)
    {
        if(g_da9210_hw_exist==1)
        {
            ...
            pr_notice("[set_da9210_buck_en] en_bit=%d\n", en_bit);
        }
        else
        {
            pr_notice("[set_da9210_buck_en] da9210 driver is not exist\n");
        }
    }
    else
    {
        pr_notice("[set_da9210_buck_en] da9210 driver is not ready\n");
    }
...
void da9210_hw_init(void)
{
    ...
    if(g_vproc_vsel_gpio_number!=0)
    {
        ...
        pr_notice( "[da9210_hw_init] ext_buck_vproc_vsel(1)\n");
    }
    ...
    pr_notice("[da9210_hw_init] [0x0]=0x%x, [0x58]=0x%x, [0x59]=0x%x, [0x5A]=0x%x, [0x5D]=0x%x, [0xD1]=0x%x, [0xD2]=0x%x, [0xD6]=0x%x, [0xD8]=0x%x, [0xD9]=0x%x\n",
...
void da9210_hw_init_v2(void)
{
    ...
    {
        ext_buck_vproc_vsel(1);
        pr_notice( "[da9210_hw_init_v2] ext_buck_vproc_vsel(1)\n");
    }
    ...
    pr_notice("[da9210_hw_init_v2] [0x0]=0x%x, [0x58]=0x%x, [0x59]=0x%x, [0x5A]=0x%x, [0x5D]=0x%x, [0xD1]=0x%x, [0xD2]=0x%x, [0xD6]=0x%x, [0xD8]=0x%x, [0xD9]=0x%x\n",
        da9210_get_reg_value(0x0),
        da9210_get_reg_value(0x58), da9210_get_reg_value(0x59),
        da9210_get_reg_value(0x5A), da9210_get_reg_value(0x5D),
        da9210_get_reg_value(0xD1), da9210_get_reg_value(0xD2),
    ...
    pr_notice("[da9210_hw_init_v2] Done\n");
}
...
void da9210_hw_component_detect(void)
{
    ...
    pr_notice("[da9210_hw_component_detect] exist=%d, Reg[0x105][7:4]=0x%x\n",
...
void bigcore_power_init(void)
{
    ...
    if (0x6795 == code)
    {
        pr_notice("[bigcore_power_init] NA\n");
    }
    else
    {
        ...
        //if (1)
        {
            ...
            pr_notice("[bigcore_power_init] Done. ret=%d, ver=%d.\n", ret, ver);
        }
    }
}
...
void bigcore_power_off(void)
{
    ...
    if (0x6795 == code)
    {
        pr_notice("[bigcore_power_off] NA\n");
    }
    else
    {
        if (ver == CHIP_SW_VER_01)
        //if (1)
        {
            ...
            pr_notice("[bigcore_power_off] Done. ver=%d\n", ver);
        }
        else
        {
            ...
            if(reg_val==0x0) {
                ...
                pr_notice("[bigcore_power_off] wrong reg_val=%d\n", reg_val);
            }
            pr_notice("[bigcore_power_off] Turn off bigcore vproc and vsram. ver=%d\n", ver);
            pr_notice("[bigcore_power_off] [0x%x]=0x%x, [0x%x]=0x%x, [0x%x]=0x%x, reg_val=%d\n",
...
void bigcore_power_on(void)
{
    ...
    else
    {
        ...
        //if (1)
        {
            ...
            ext_buck_vproc_vsel(1);
            pr_notice("[bigcore_power_on] Done. ver=%d\n", ver);
        }
        else
        {
            /* turn on ca15l vsram */
            ...
            if(reg_val==0x0) {
                ...
                pr_notice("[bigcore_power_on] wrong reg_val=%d\n", reg_val);
            }
            /* turn on ca15l vproc(ext buck) */
            set_da9210_buck_en(1);
            pr_notice("[bigcore_power_on] Turn on bigcore vproc and vsram. ver=%d\n", ver);
            pr_notice("[bigcore_power_on] [0x%x]=0x%x, [0x%x]=0x%x, [0x%x]=0x%x, reg_val=%d\n",
                0x63C, upmu_get_reg_value(0x63C),
                0x18, upmu_get_reg_value(0x18),
                0x524, upmu_get_reg_value(0x524),
                reg_val
...
static int da9210_driver_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    pr_notice("[da9210_driver_probe] g_da9210_hw_exist=%d, g_da9210_driver_ready=%d\n",
        g_da9210_hw_exist, g_da9210_driver_ready);

    if(g_da9210_hw_exist==0)
    {
        pr_notice("[da9210_driver_probe] return err\n");
        return err;
    }
...
{
    pr_notice("[show_da9210_access] 0x%x\n", g_reg_value_da9210);
...
static ssize_t store_da9210_access(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
    ...
    pr_notice("[store_da9210_access] \n");

    if(buf != NULL && size != 0)
    {
        pr_notice("[store_da9210_access] buf is %s \n",buf);
        reg_address = simple_strtoul(buf,&pvalue,16);

        if(size > 4)
        {
            reg_value = simple_strtoul((pvalue+1),NULL,16);
            pr_notice("[store_da9210_access] write da9210 reg 0x%x with value 0x%x !\n",reg_address,reg_value);

            if(reg_address < 0x100)
            {
                pr_notice("[store_da9210_access] page 0,1\n");
                da9210_config_interface(0x0, 0x0, 0xF, 0); // select to page 0,1
            }
            else
            {
                pr_notice("[store_da9210_access] page 2,3\n");
                da9210_config_interface(0x0, 0x2, 0xF, 0); // select to page 2,3
        ...
        else
        {
            ...
            pr_notice("[store_da9210_access] read da9210 reg 0x%x with value 0x%x !\n",reg_address,g_reg_value_da9210);
            pr_notice("[store_da9210_access] Please use \"cat da9210_access\" to get value\r\n");
        }
    }
    return size;
}
...
{
    pr_notice("[show_da9210_vosel_pin] g_da9210_vosel_pin=%d\n", g_da9210_vosel_pin);
    return sprintf(buf, "%u\n", g_da9210_vosel_pin);
}
...
static int da9210_user_space_probe(struct platform_device *dev)
{
    ...
    pr_notice("******** da9210_user_space_probe!! ********\n" );
...
static int __init da9210_init(void)
{
    ...
    pr_notice("[da9210_init] da9210_AUTO_DETECT_DISABLE\n");
    g_da9210_hw_exist=0;
    g_da9210_driver_ready=1;
    ...
    pr_notice("[da9210_init] g_vproc_vsel_gpio_number=0x%x, g_vproc_en_gpio_number=0x%x\n",
        g_vproc_vsel_gpio_number, g_vproc_en_gpio_number);

    if(g_vproc_vsel_gpio_number != 0)
    {
        pr_notice("[da9210_init] init start. ch=%d\n", da9210_BUSNUM);

        i2c_register_board_info(da9210_BUSNUM, &i2c_da9210, 1);

        if(i2c_add_driver(&da9210_driver)!=0)
        {
            pr_notice("[da9210_init] failed to register da9210 i2c driver.\n");
        }
        else
        {
            pr_notice("[da9210_init] Success to register da9210 i2c driver.\n");
        }

        // da9210 user space access interface
        ret = platform_device_register(&da9210_user_space_device);
        if (ret) {
            pr_notice("****[da9210_init] Unable to device register(%d)\n", ret);
            return ret;
        }
        ret = platform_driver_register(&da9210_user_space_driver);
        if (ret) {
            pr_notice("****[da9210_init] Unable to register driver (%d)\n", ret);
            return ret;
        }
    }
    else
    {
        pr_notice("[da9210_init] DCT no define EXT BUCK\n");
        g_da9210_hw_exist=0;
        g_da9210_driver_ready=1;
    }
