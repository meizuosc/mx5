...
kal_uint32 bq24160_config_interface (kal_uint8 RegNum, kal_uint8 val, kal_uint8 MASK, kal_uint8 SHIFT)
{
    ...
    //pr_notice("[bq24160_config_interface] Check Reg[%x]=0x%x\n", RegNum, bq24160_reg);
...
void bq24160_hw_component_detect(void)
{
    ...
    pr_notice("[bq24160_hw_component_detect] exist=%d, Reg[0x04]=0x%x\n",
        g_bq24160_hw_exist, val);
}

int is_bq24160_exist(void)
{
    pr_notice("[is_bq24160_exist] g_bq24160_hw_exist=%d\n", g_bq24160_hw_exist);
...
void bq24160_dump_register(void)
{
    ...
    for (i=0;i<bq24160_REG_NUM;i++)
    {
        ...
        pr_notice("[0x%x]=0x%x ", i, bq24160_reg[i]);
    }
    pr_notice("\n");
}
...
{
    #if 0
    battery_log(BAT_LOG_CRTI,"[bq24160_hw_init] From Albert\n");
    ...
    #if 0
    battery_log(BAT_LOG_CRTI,"[bq24160_hw_init] From Tim\n");
    ...
    battery_log(BAT_LOG_CRTI,"[bq24160_hw_init] After HW init\n");
...
static int bq24160_driver_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    battery_log(BAT_LOG_CRTI,"[bq24160_driver_probe] \n");
...
{
    battery_log(BAT_LOG_CRTI,"[show_bq24160_access] 0x%x\n", g_reg_value_bq24160);
...
static ssize_t store_bq24160_access(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
    ...
    battery_log(BAT_LOG_CRTI,"[store_bq24160_access] \n");

    if(buf != NULL && size != 0)
    {
        battery_log(BAT_LOG_CRTI,"[store_bq24160_access] buf is %s and size is %d \n",buf,size);
        reg_address = simple_strtoul(buf,&pvalue,16);

        if(size > 3)
        {
            reg_value = simple_strtoul((pvalue+1),NULL,16);
            battery_log(BAT_LOG_CRTI,"[store_bq24160_access] write bq24160 reg 0x%x with value 0x%x !\n",reg_address,reg_value);
        ...
        else
        {
            ...
            battery_log(BAT_LOG_CRTI,"[store_bq24160_access] read bq24160 reg 0x%x with value 0x%x !\n",reg_address,g_reg_value_bq24160);
            battery_log(BAT_LOG_CRTI,"[store_bq24160_access] Please use \"cat bq24160_access\" to get value\r\n");
        }
    }
    return size;
}
...
static int bq24160_user_space_probe(struct platform_device *dev)
{
    ...
    battery_log(BAT_LOG_CRTI,"******** bq24160_user_space_probe!! ********\n" );
...
static int __init bq24160_init(void)
{
    ...
    battery_log(BAT_LOG_CRTI,"[bq24160_init] init start. ch=%d\n", bq24160_BUSNUM);

    i2c_register_board_info(bq24160_BUSNUM, &i2c_bq24160, 1);

    if(i2c_add_driver(&bq24160_driver)!=0)
    {
        battery_log(BAT_LOG_CRTI,"[bq24160_init] failed to register bq24160 i2c driver.\n");
    }
    else
    {
        battery_log(BAT_LOG_CRTI,"[bq24160_init] Success to register bq24160 i2c driver.\n");
    }

    // bq24160 user space access interface
    ret = platform_device_register(&bq24160_user_space_device);
    if (ret) {
        battery_log(BAT_LOG_CRTI,"****[bq24160_init] Unable to device register(%d)\n", ret);
        return ret;
    }
    ret = platform_driver_register(&bq24160_user_space_driver);
    if (ret) {
        battery_log(BAT_LOG_CRTI,"****[bq24160_init] Unable to register driver (%d)\n", ret);
        return ret;
    }
