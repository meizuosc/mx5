...
///{MEIU_BSP
#ifdef CONFIG_MZ_M85
static unsigned int mz_pcb_hw_rev = 0;
#endif
///@}
...
///MEIZU_BSP{
    ...
    if (mz_pcb_hw_rev == 0)
    {
        tps6128x_config_interface(0x1, 0x2, 0x3, 5); // ENABLE[1:0]=10 ,force pass-through mode
        tps6128x_config_interface(0x4, 0x1, 0x1,  5); // Current limit diasble
    }else{
#endif
///@}
	...
	{
		pr_notice("[tps6128x_hw_init] sw workaround for tps61280 no-A chip,wait for 10ms\n");
...
static ssize_t store_tps6128x_access(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
    ...
    if(buf != NULL && size != 0)
    {
        pr_notice("[store_tps6128x_access] buf is %s \n",buf);
