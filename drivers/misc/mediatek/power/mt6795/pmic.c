    ...
    if(mt6331_upmu_get_pwrkey_deb()==1)
    {
        ...
        if(g_boot_mode == KERNEL_POWER_OFF_CHARGING_BOOT && timer_pre != 0)
        {
                ...
                if(long_pwrkey_press)   //500ms
                {
                    ...
                    //arch_reset(0, NULL); // do not reset in kpoc
                }
        }
        #endif
        ...
        printk(KERN_EMERG "power key release\n");
    }
    else
    {
        ...
        printk(KERN_EMERG "power key press\n");
    }
...
void homekey_int_handler(void)
{
    ...
    if(mt6331_upmu_get_homekey_deb()==1)
    {
        ...
        printk(KERN_EMERG "home key release\n");
    }
    else
    {
        ...
        printk(KERN_EMERG "home key press\n");
    }
...
void chrdet_int_handler(void)
{
    ...
    if (!upmu_get_rgs_chrdet())
    {
        ...
        if(boot_mode == KERNEL_POWER_OFF_CHARGING_BOOT || boot_mode == LOW_POWER_OFF_CHARGING_BOOT || boot_mode == META_BOOT)
        {
...
{
    pr_notice( "[dct_pmic_VIBR_enable] %d pmic.c:do nothing here,meizu vibr\n", dctEnable);
...
void dct_pmic_VSRAM_DVFS1_sel(kal_uint32 volt)
{
    ...
    val= ( (volt) * 1000 - 700000 ) / 6250; //uv

    if(val > 0x7F)
        val=0x7F;

/* Modified by Meizu BSP-M jiangziyu */
//    if(volt == VOL_DEFAULT)
...
void PMIC_INIT_SETTING_V1(void)
{
    ...
    if(mt6331_chip_version >= PMIC6331_E2_CID_CODE)
    {
        ...
        ret = pmic_config_interface(0x262,0x0,0x1,0); // [0:0]: VSRAM_DVFS1_VOSEL_CTRL; YP
...
void PMIC_INIT_SETTING_V2(void)
{
    ...
    if(mt6331_chip_version >= PMIC6331_E1_CID_CODE)
    {
        ...
        //{@ MEIZU_BSP; TP/LCD/FP sleep lowpower setting
        ret = pmic_config_interface(0x528,0x1,0x1,0); // [0:0]: RG_VGP2_LP_CTRL; wbwu;LCD SLEEP mode setting
        ret = pmic_config_interface(0x528,0x0,0x3,5); // [6:5]: RG_VGP2_SRCLK_MODE_SEL; wbwu, sync with golden setting
        ret = pmic_config_interface(0x52A,0x1,0x1,0); // [0:0]: RG_VGP3_LP_CTRL; wbwu;LCD SLEEP mode setting
        ret = pmic_config_interface(0x52A,0x0,0x3,5); // [6:5]: RG_VGP3_SRCLK_MODE_SEL;wbwu, sync with golden setting
        ret = pmic_config_interface(0x580,0x1,0x1,0); // [0:0]: RG_VIBR_LP_CTRL; wbwu;FPC SLEEP mode setting
        ret = pmic_config_interface(0x580,0x0,0x3,5); // [6:5]: RG_VIBR_SRCLK_MODE_SEL;wbwu, sync with golden setting
        ret = pmic_config_interface(0x550,0x1,0x1,0); // [6:5]: RG_VMCH_LP_CTRL; wbwu;TP SLEEP mode setting
        ret = pmic_config_interface(0x550,0x0,0x3,5); // [6:5]: RG_VMCH_SRCLK_MODE_SEL; wbwu, sync with golden setting
        ret = pmic_config_interface(0x54e,0x1,0x1,0); // [6:5]: RG_VMC_LP_CTRL; wbwu;TP SLEEP mode setting
        ret = pmic_config_interface(0x54e,0x0,0x3,5); // [6:5]: RG_VMC_SRCLK_MODE_SEL; wbwu, sync with golden setting
        //@}
        ret = pmic_config_interface(0x534,0x44,0x7F,9); // [15:9]: RG_VSRAM_DVFS1_VOSEL; YP
        ret = pmic_config_interface(0x538,0x0,0x1,2); // [2:2]: RG_VGP2_NDIS_EN; Fandy: disable GP2 discharge
        ret = pmic_config_interface(0x54A,0x1,0x1,0); // [0:0]: RG_VIO28_LP_CTRL; YP Niou, sync with goldne setting
...
//{@ MEIZU_BSP
void pmic_set_tp_lowpower(int enable)
{
	U32 ret = 0;
	...
	if(enable){
	        ...
	        ret = pmic_config_interface(0x550,0x1,0x1,1); // [6:5]: RG_VMCH_LP_CTRL; wbwu;TP SLEEP mode setting
	        ret = pmic_config_interface(0x54e,0x1,0x1,1); // [6:5]: RG_VMC_LP_CTRL; wbwu;TP SLEEP mode setting
	}
}
//@}
