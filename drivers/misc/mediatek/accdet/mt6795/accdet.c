...
#ifdef MEIZU_M85
#define FIX_WORK_DELAY_TIME	1000  /* delay 1s and then run fix callback */
static struct delayed_work weird_headphone_fix_work;
static struct workqueue_struct *weird_headphone_fix_workqueue = NULL;

#define HEADSET_PLUG_OUT_DELAY_TIME	500 /* Wait 500ms to send plug out event after headphone plug out */
static struct delayed_work headset_plug_out_delay_work;

#define PLUG_DELAY_EVENT_TIME  msecs_to_jiffies(2000) /* Delay 2 second to report press event after headset plug in */

#define KEY_RELEASE	0
#define KEY_PRESS	1
static int key_state = KEY_RELEASE;
...
static char calibration_state_str[CALIBRATION_NUM][STATE_LEN] = {
	"INIT",
	"PENDING",
	"COMPLETE",
	"ERROR",
...
static char calibrate_command[] = "up_key_calibrate";
static struct work_struct accdet_calibration_work;

extern int accdet_data_write(int up_key);
extern int accdet_data_read(int *up);
...
#endif
static void headset_plug_out_delay_work_callback(struct work_struct *work)
{
...
#if defined ACCDET_EINT || defined ACCDET_EINT_IRQ
static void weird_headphone_fix(struct work_struct *work)
{
...
//Disable ACCDET
   ...
   //KE under fastly plug in and plug out
    if (cur_eint_state == EINT_PIN_PLUG_IN) {
		...
		if (delayed_work_pending(&headset_plug_out_delay_work))
			cancel_delayed_work_sync(&headset_plug_out_delay_work);
#endif
		mutex_lock(&accdet_eint_irq_sync_mutex);
		eint_accdet_sync_flag = 1;
		mutex_unlock(&accdet_eint_irq_sync_mutex);
		...
		cancel_delayed_work_sync(&weird_headphone_fix_work);
		  ...
		  cable_pin_recognition = 0;
			...
			if (key_state == KEY_PRESS) {
				ACCDET_DEBUG("[Accdet] PlugOut Key %d Press\n", cur_key);
				send_key_event(cur_key, 0);
			}
			/* If the device is headphone(HEADSET_NO_MIC), wait for 500msec to send plug out event. */
			if (mCableType == HEADSET_NO_MIC) {
				accdet_status = PLUG_OUT;
				cable_type = NO_DEVICE;
				if (delayed_work_pending(&headset_plug_out_delay_work))
					cancel_delayed_work_sync(&headset_plug_out_delay_work);
				schedule_delayed_work(&headset_plug_out_delay_work, msecs_to_jiffies(HEADSET_PLUG_OUT_DELAY_TIME));
			} else {
				headset_plug_out();
			}
		#else
...
#define DW_KEY_THR		 (161) //0.161v=161000uv
#define UP_KEY_THR       (80) //0.08v=80000uv
#define MD_KEY_THR		 (0)

int up_key_thr = UP_KEY_THR;
int md_key_thr = MD_KEY_THR;
int dw_key_thr = DW_KEY_THR;
#else
#define DW_KEY_HIGH_THR	 (500) //0.50v=500000uv
...
static void multi_key_detection(int current_status)
{
	...
	key_state = !current_status ? KEY_PRESS: KEY_RELEASE;
}
...
static void accdet_workqueue_func(void)
{
	...
	if (delayed_work_pending(&weird_headphone_fix_work)) {
	...
	switch(accdet_status)
    {
	    ...
	    //ALPS00038030:reduce the time of remote button pressed during incoming call
		    ...
		    {	
				...
				if(1 == eint_accdet_sync_flag) {   
#ifdef MEIZU_M85
                                    if (time_is_before_jiffies(old_jiffies + PLUG_DELAY_EVENT_TIME)) {
...
#ifdef MEIZU_M85
    if (calibration_state == CALIBRATION_PENDING) {
        printk("%s(), accdet calibration is running, return ...\n", __func__);
        return;
    }
#endif
...
#ifdef MEIZU_M85
static void accdet_calibration_work_callback(struct work_struct *work)
{
#define NUM	10
#define RETRIES	60  /* TimeOut: 60 * 10 * 10 = 6000 msec */
	int i, voltage, tmp_value = 0, retry = 0;

	calibration_state = CALIBRATION_PENDING;
RETRY:
	for (i = 0; i < NUM; i++) {
		msleep(10);
		voltage = Accdet_PMIC_IMM_GetOneChannelValue(1);
		if (voltage < DW_KEY_HIGH_THR && tmp_value <= voltage)
			tmp_value = voltage;
	}
	if (tmp_value < UP_KEY_THR || tmp_value >= DW_KEY_HIGH_THR) {
		if (retry++ <= RETRIES) { msleep(1); goto RETRY;}
		calibration_state = CALIBRATION_ERROR;
	} else {
		dw_key_thr = tmp_value + 40; /* Add 40mv as threshold */
...
static ssize_t accdet_calibrate_cmd_show(struct device *dev, struct device_attribute *attr, const char *buf)
{
	int count;

	count = sprintf(buf, "cmd = %s\n", (cmd_index == -1) ? "no supported command": calibrate_command);
	return count;
}

static ssize_t accdet_calibrate_cmd_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	char *cmd;
	int len, index = 0;

	/* receive control command */
        printk("%s(), recive command: %s\n",__func__, buf);

	if (!strncmp(buf, calibrate_command, strlen(calibrate_command))) {
		cmd_index = 0;
		accdet_auxadc_switch(1);
		schedule_work(&accdet_calibration_work);
	} else {
		cmd_index = -1;
	}
...
static ssize_t accdet_calibrate_data_show(struct device *dev, struct device_attribute *attr, const char *buf)
{
	int err = 0, count = 0;
	int up_key = 0;

	sprintf(buf + count, "%d", dw_key_thr);

	return  strlen(buf);
}

static ssize_t accdet_calibrate_data_store(struct device *dev, struct device_attribute *attr, const char *buf, int count)
{
	int err = 0;
	int up_key;

	printk("%s(), buf = %s\n", __func__, buf);
	err = sscanf(buf, "%d", &up_key);
	if (err == 1) {
		dw_key_thr = up_key;
	} else {
		printk("%s(), parse buffer failed\n", __func__);
	}
	return  count;
}

static ssize_t accdet_calibrate_state_show(struct device *dev, struct device_attribute *attr, const char *buf)
{
	int count;
	count = sprintf(buf, "state = %s\n", calibration_state_str[calibration_state]);

	return count;
}

static DEVICE_ATTR(cmd, S_IWUSR | S_IRUGO, accdet_calibrate_cmd_show, accdet_calibrate_cmd_store);
static DEVICE_ATTR(data, S_IWUSR | S_IRUGO, accdet_calibrate_data_show, accdet_calibrate_data_store);
static DEVICE_ATTR(state, S_IRUGO, accdet_calibrate_state_show, NULL);
...
static int accdet_create_calibration_attr(struct device *dev)
{
	int idx, err = 0;
	int num =  ARRAY_SIZE(device_attrs);

	for(idx = 0; idx < num; idx++)
	{
		if((err = device_create_file(dev, device_attrs[idx])))
		{
			ACCDET_DEBUG("device_create_file (%s) = %d\n", device_attrs[idx]->attr.name, err);
			break;
		}
	}
	return err;
...
int mt_accdet_probe(struct platform_device *pdev)
{
	int ret = 0;
...
#ifdef MEIZU_M85
	calibration_state = CALIBRATION_INIT;
#endif
...
#ifdef MEIZU_M85
	if ((ret = accdet_create_calibration_attr(&pdev->dev))==0)
		meizu_sysfslink_register_n(&pdev->dev, "accdet_calibration");
#endif
	...
	{	
		...
		INIT_DELAYED_WORK(&headset_plug_out_delay_work, headset_plug_out_delay_work_callback);
		INIT_WORK(&accdet_calibration_work, accdet_calibration_work_callback);
	#endif
