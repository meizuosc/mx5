...
static 	mt_i2c *iic;
static long timeout = 0, polltime = 0, wait_exit = 0;

ssize_t iic_state_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	_i2c_dump_info(iic);
	return sprintf(buf, "iic struct: i2c_addr:0x%x, pollen:%d,dmaen:%d,trans_stop:%d,timeout:%ld,polltime:%ld, msg_len:%u, wait_exit:%ld, irq_stat:0x%x, intr_stat:0x%x\n",
			iic->addr, iic->poll_en,iic->dma_en,atomic_read(&iic->trans_stop), timeout, polltime, iic->msg_len, wait_exit, iic->irq_stat, (i2c_readl(iic, OFFSET_INTR_STAT)));
}
EXPORT_SYMBOL(iic_state_show);
