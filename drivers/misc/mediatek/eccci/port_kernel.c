...
void ccci_md_exception_notify(struct ccci_modem *md, MD_EX_STAGE stage)
{
	...
	if ((md->ex_stage == EX_INIT_DONE) && (stage == MD_WDT))
	{
		// Don't switch md->stage in this case, it's a workaround for unexcepted state:
		// md logger send/receive packet while MD WDT occurs.
	}
	else
	{
		md->ex_stage = stage;
