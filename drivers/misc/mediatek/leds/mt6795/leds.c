...
static int led_switch_breath_pmic(enum mt65xx_led_pmic pmic_type, struct nled_setting* led, int enable)
{
  ...
  // The current step is in blink_on_time
  if(led->blink_on_time <= ISINK_5)
    cur_step = led->blink_on_time;
  else
    cur_step = ISINK_5; //24mA

  //cur_step = ISINK_1; //8mA

  LEDS_DEBUG("[LED]led_blink_pmic: cur_step=%d\n", cur_step);  
