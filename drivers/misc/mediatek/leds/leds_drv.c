...
#else
int setMaxbrightness(int max_level, int enable) {return 0;}
#endif
...
static ssize_t store_hbm(struct device *dev, struct device_attribute *attr,
				  const char *buf, size_t size)
{
	...
	primary_display_sethbm(value);

	return size;
}

static ssize_t show_hbm(struct device *dev, struct device_attribute *attr, char *buf)
{
	return 0;
}

static DEVICE_ATTR(hbm, 0664, show_hbm, store_hbm);
...
static int __init mt65xx_leds_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < MT65XX_LED_TYPE_TOTAL; i++) {
		...
		if (strcmp(g_leds_data[i]->cdev.name, "lcd-backlight") == 0) {
			...
			rc = device_create_file(g_leds_data[i]->cdev.dev, &dev_attr_hbm);
			if (rc) {
				LEDS_DRV_DEBUG("[LED]device_create_file hbm fail!\n");
			}

			bl_setting = &g_leds_data[i]->cust;
			meizu_sysfslink_register_n(g_leds_data[i]->cdev.dev, "lcd-backlight");
		}
		else
		{
			if(strcmp(g_leds_data[i]->cdev.name,"mx-led") == 0)
			{
