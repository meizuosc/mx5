...
void cs_cpu_oslock(void __iomem *addr_base)
{
    ...
    if(!oslm) {
        ...
        //pr_notice("[ETM LOG] ETMOSLAR  &0x%llx=0x%x\n",(__pa(addr_base))+ETMOSLAR,cs_cpu_read(addr_base,ETMOSLAR));
    }
   //pr_notice("[ETM LOG] %s Done\n",__func__);
}
void cs_cpu_unlock(void __iomem *addr_base) 
{
    ...
    result = cs_cpu_read(addr_base, ETMLSR) & 0x3;
    //pr_notice("[ETM LOG] %s\n",__func__);
    //pr_notice("[ETM LOG] ETMLSR  &0x%llx=0x%x\n",(__pa(addr_base))+ETMLSR,cs_cpu_read(addr_base,ETMLSR));
    /* if software lock locked? */
    switch(result) {
    case SW_LOCK_IMPLEMENTED | SW_LOCK_LOCKED:
        //pr_notice("[ETM LOG]addr @ %p locked\n", addr_base);
        cs_cpu_write(addr_base, ETMLAR, 0xC5ACCE55);
        //pr_notice("[ETM LOG] ETMLAR  &0x%llx=0x%x\n",(__pa(addr_base))+ETMLAR,cs_cpu_read(addr_base,ETMLAR));
        dsb();
        break;
    case SW_LOCK_IMPLEMENTED:
...
void cs_cpu_funnel_setup(void)
{
	...
	cs_cpu_write(tracer.funnel_regs, 0x000, funnel_ports);
        //pr_notice("[ETM LOG] funnel_ports &0x%llx=0x%x\n",(__pa(tracer.funnel_regs)),cs_cpu_read(tracer.funnel_regs,0x000));
        //pr_notice("[ETM LOG] %s Done\n",__func__);
#if 0
...
#if 0
		if (enable_count++) {
			printk("check 0x10830000~0x10830100\n");
			while (1);
		}
#endif
			...
			struct device_attribute *attr, char *buf)
{
	int v, etb_legnth;
    etb_legnth=0;
    pr_notice("[ETM LOG] etb_length_show, tracer->etb_regs = 0x%llx\n", __pa(tracer.etb_regs));
            ...
            const char *buf, size_t n)
{
    ...
    for(i=0;i<num_possible_cpus();i++){
    	pr_notice("[ETM LOG][%s] ETMOSLSR &0x%llx=0x%x\n",__func__,(__pa(tracer.etm_regs[i] ))+ETMOSLSR,cs_cpu_read(tracer.etm_regs[i] ,ETMOSLSR));
    	pr_notice("[ETM LOG][%s] ETMLSR &0x%llx=0x%x\n",__func__,(__pa(tracer.etm_regs[i] ))+ETMLSR,cs_cpu_read(tracer.etm_regs[i] ,ETMLSR));
    }
#endif
    return n;
}
...
static int etm_probe(struct platform_device *pdev)
{
    ...
    pr_notice("[ETM LOG]get num from DT = %d\n", tracer.nr_etm_regs); 
	...
	pr_notice("[ETM LOG]etm %d @ pa 0x%llx\n", i+1, __pa(tracer.etm_regs[i]));
    }
...
int etb_probe(struct platform_device *pdev)
{
    ...
    pr_notice("[ETM LOG] XXXXXXXXXXXXXX  %s\n",__func__);
	mutex_lock(&tracer.mutex);
    ...
    if(!etb_base) {
        ...
        pr_notice("[ETM LOG][ETM LOG]of_iomap for etb pa @ 0x%llx\n", __pa(etb_base));
    }
    ...
    if(!etr_base) {
        ...
        pr_notice("[ETM LOG][ETM LOG]of_iomap for etr pa @ 0x%llx\n", __pa(etr_base));
    }
    ...
    if(!tracer.funnel_regs) {
        ...
        pr_notice("[ETM LOG][ETM LOG]of_iomap for funnel pa @ 0x%llx\n", __pa(tracer.funnel_regs));
    }
    ...
    if(!tracer.dem_regs) {
        ...
        pr_notice("[ETM LOG][ETM LOG]of_iomap for dem pa @ 0x%llx\n", __pa(tracer.dem_regs));
    }
...
restart_trace(struct notifier_block *nfb, unsigned long action, void *hcpu)
{
	...
	action=action & 0xf;
	//pr_notice("[ETM LOG] cpu %lx do %s,action: 0x%lx\n",cpu,__func__,action);
	switch (action) {
	case CPU_STARTING:
		switch (cpu) {
		case 1:
...
static int __init etm_init(void)
{
	...
	printk("[ETM] register etm driver\n");
	err = platform_driver_register(&etm_driver);
	if (err) {
		return err;
	}

	printk("[ETM] register etb driver\n");
	err = platform_driver_register(&etb_driver);
	if (err) {
		return err;
	}

	printk("[ETM] init done\n");
	return 0;
}
