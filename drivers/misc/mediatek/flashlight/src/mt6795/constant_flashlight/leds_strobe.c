...
int g_bLedOn1=0;
int g_bLedOn2=0;
static DEFINE_SPINLOCK(g_strobeDataSync);
...
extern struct wake_lock flash_wake_lock;
static DEFINE_MUTEX(flash_mutex);
static DEFINE_MUTEX(flashOnOff_mutex);
static int g_bWaitLock=0;
/*****************************************************************************
Functions
*****************************************************************************/
...
int fl_fashPreOn6332(void)
{
    ...
    mutex_lock(&flashOnOff_mutex);
    if(g_bPreOn==0)
    {
        //setReg6332(0x806C,	0x0,	0x1,	3);
        //stopCharging();
        PK_DBG("qq fl_fashPreOn6332 ln=%d\n",__LINE__);
        if(bat_is_charger_exist()==0)
    mt6331_upmu_set_rg_int_en_chrdet(0);
...
int fl_setFlashOnOff6332_1(int onOff)
{
    ...
    else
    {
        ...
        if(g_bPreOn==1 && (g_bLedOn1==0 && g_bLedOn2==0))
        {
...
static int constant_flashlight_ioctl(unsigned int cmd, unsigned long arg)
{
	...
	PK_DBG("constant_flashlight_ioctl() line=%d cmd=%d, ior=%d, iow=%d iowr=%d arg=%lu\n",__LINE__, cmd, ior, iow, iowr, arg);
	PK_DBG("constant_flashlight_ioctl() line=%d cmd-ior=%d, cmd-iow=%d cmd-iowr=%d arg=%lu\n",__LINE__, cmd-ior, cmd-iow, cmd-iowr, arg);
    switch(cmd)
    {

        case FLASH_IOC_GET_REG:
            logI("FLASH_IOC_GET_REG %lu 0x%x ##", arg, (int)arg);
