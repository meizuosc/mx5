...
#define LCM_ID_GIS_W			0x13
#define LCM_ID_GIS_B			0x14
...
//upgrade app
static unsigned char aucFW_APP_11[] =
{
#include "FT5346_MA01_Ofilm_LCD11_white_firmware.i"
};

static unsigned char aucFW_APP_12[] =
{
#include "FT5346_MA01_Ofilm_LCD12_black_firmware.i"
};

static unsigned char aucFW_APP_13[] =
{
#include "FT5346_MA01_GIS_LCD13_white_firmware.i"
};

static unsigned char aucFW_APP_14[] =
{
#include "FT5346_MA01_GIS_LCD14_black_firmware.i"
};

struct CtpFwInfo ctp_fw_app[] =
{
	{0x51, 0x11, aucFW_APP_11, sizeof(aucFW_APP_11)},
	{0x51, 0x12, aucFW_APP_12, sizeof(aucFW_APP_12)},
	{0xcb, 0x13, aucFW_APP_13, sizeof(aucFW_APP_13)},
	{0xcb, 0x14, aucFW_APP_14, sizeof(aucFW_APP_14)},
};
...
int fts_ctpm_auto_upgrade(struct i2c_client *client)
{
	...
	for(i = 0; i < 5; ++i) {
		...
		pr_info("[FTS] %d: lcm_id = 0x%x, fm_ver = 0x%x, vendor_id = 0x%x\n",
			    i, ctp_info.lcm_id,
...
fw_exit:
	...
	cur_tp_info.fw_version = ctp_info.fw_version;
	return upgrade_ret;
}
				...
				struct device_attribute *attr, char *buf)
{
	...
	count += sprintf(tmp + count, "MA01");

	if (cur_tp_info.lcm_id == LCM_ID_AUO_W)
		count += sprintf(tmp + count, "AUOW");
	else if (cur_tp_info.lcm_id == LCM_ID_AUO_B)
		count += sprintf(tmp + count, "AUOB");
	else if (cur_tp_info.lcm_id == LCM_ID_GIS_W)
		count += sprintf(tmp + count, "GISW");
	else if (cur_tp_info.lcm_id == LCM_ID_GIS_B)
		count += sprintf(tmp + count, "GISB");
	else
		count += sprintf(tmp + count, "UNKNOWN");
				...
				struct device_attribute *attr, char *buf)
{
	...
	if (cur_tp_info.vendor_id == 0x51) {
		if (cur_tp_info.fw_version == 0x0A)
			SetParamData(ft5x46_test_config_ofilm_0x0A);
		else
			SetParamData(ft5x46_test_config_ofilm);
	} else if (cur_tp_info.vendor_id == 0xcb)
		SetParamData(ft5x46_test_config_gis);
	else
...
static ssize_t ft5x46_margin_control_show(struct device *dev, struct device_attribute *attr,
				char *buf)
{
	int ret;
	int count = 0;
	u8 data1,data2,data3,data4;
	pr_info(" start ft5x46_margin_control_show \n");
	struct i2c_client *client = container_of(dev, struct i2c_client, dev);
	ret = fts_read_reg(client, MEIZU_MARGIN_LEFT, &data1);
	if (ret < 0) {
		count += sprintf(buf + count, "read margin registor left faild\n");
		return -EPERM;
	}

	ret = fts_read_reg(client, MEIZU_MARGIN_RIGHT, &data2);
	if (ret < 0) {
		count += sprintf(buf + count, "read margin registor right faild\n");
		return -EPERM;
	}

	ret = fts_read_reg(client, MEIZU_MARGIN_UP, &data3);
	if (ret < 0) {
		count += sprintf(buf + count, "read margin registor up faild\n");
		return -EPERM;
	}

	ret = fts_read_reg(client, MEIZU_MARGIN_DOWN, &data4);
	if (ret < 0) {
		count += sprintf(buf + count, "read margin registor down faild\n");
		return -EPERM;
	}
	count += sprintf(buf + count, "margin_control: left [%d], right [%d], up [%d], down [%d]\n", data1, data2, data3, data4);
	return count ;
}

static ssize_t ft5x46_margin_control_store(struct device *dev, struct device_attribute *attr,
				const char *buf, size_t count)
{
	int ret;
	int cnt = 0;
	...
	const char * data = buf ;
	arg1 = (unsigned char)data[0];
	arg2 = (unsigned char)data[1];
	pr_info("[%s]:margin control write [%x][%x] \n", __func__, arg1, arg2);
	switch(arg1){
		case MAR_LEFT:
			regaddr = MEIZU_MARGIN_LEFT;
			pr_info("set left margin addr to %d\n", arg2);
			break;
		case MAR_RIGHT:
			regaddr = MEIZU_MARGIN_RIGHT;
			pr_info("set right margin addr to %d\n", arg2);
			break;
		case MAR_TOP:
			regaddr = MEIZU_MARGIN_UP;
			pr_info("set top margin addr to %d\n", arg2);
			break;
		case MAR_BOTTOM:
			regaddr = MEIZU_MARGIN_DOWN;
			pr_info("set bottom margin addr to %d\n", arg2);
			break;
		default:
			pr_err("write margin mode data[ %x ] is invalid\n", arg2);
			break;
	}
	...
	struct i2c_client *client = container_of(dev, struct i2c_client, dev);
	ret = fts_write_reg(client, regaddr, arg2);
	if (ret < 0) {
		pr_err("set margin registor faild\n");
		return -EPERM;
	}
...
static ssize_t ft5x46_margin_control_test_show(struct device *dev, struct device_attribute *attr,
				char *buf)
{
	int ret;
	int count = 0;
...
static ssize_t ft5x46_margin_control_test_store(struct device *dev, struct device_attribute *attr,
				const char *buf, size_t count)
{
	int ret;
	int cnt = 0;
...
#endif
#ifdef MEIZU_MARGIN_CONTROL
static DEVICE_ATTR(margin_control, (S_IRUGO|S_IWUSR|S_IWGRP), ft5x46_margin_control_show, ft5x46_margin_control_store);
static DEVICE_ATTR(margin_control_test, (S_IRUGO|S_IWUSR|S_IWGRP), ft5x46_margin_control_test_show, ft5x46_margin_control_test_store);
#endif
