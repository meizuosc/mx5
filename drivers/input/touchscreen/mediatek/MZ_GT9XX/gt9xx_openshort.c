...
#define MAX_LIMIT_VALUE_GROUP2 3250	// screen max limit
...
0x00,0x38,0x04,0x80,0x07,0x0A,0x05,0x00,0x01,0x08,0x28,0x08,0x64,0x3C,0x03,0x05,\
0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x8C,0x0C,0x0E,0x30,0x00,\
0x6D,0x0F,0x00,0x00,0x00,0x99,0x03,0x1D,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,\
0x00,0x00,0x00,0x1E,0x46,0x94,0x40,0x03,0x00,0x00,0x00,0x04,0xB6,0x20,0x00,0xA0,\
0x26,0x00,0x8E,0x2D,0x00,0x7F,0x36,0x00,0x75,0x40,0x00,0x75,0x00,0x00,0x00,0x00,\
0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,\
0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,\
0x02,0x04,0x06,0x08,0x0A,0x0C,0x0E,0x10,0x12,0x14,0x16,0x18,0x1A,0x1C,0x00,0x00,\
0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0xFF,0x14,0x13,\
0x12,0x10,0x0F,0x0C,0x0A,0x08,0x06,0x04,0x02,0x00,0x16,0x1D,0x18,0x1F,0x1C,0x20,\
0x1E,0x24,0x21,0x26,0x22,0x28,0xFF,0xFF,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,\
...
static ssize_t gtp_sysfs_appid_show(struct device *dev,struct device_attribute *attr, char *buf)
{
	...
	if (panel_id == 0x55)
		count += sprintf(buf + count, "BIELW");
	else if (panel_id == 0xAA)
		count += sprintf(buf + count, "BIELB");
	else
		count += sprintf(buf + count, "UNKNOW");
	...
	cfg_version = tmp[2];
	*/
	count += sprintf(buf + count, ":%04X-%02X:GT9XX:0x%02X:%02X\n",
...
static ssize_t gtp_sysfs_margin_control_show(struct device *dev, struct device_attribute *attr,
				char *buf)
{
	int ret;
	int count = 0;
	...
	ret = gtp_i2c_read_dbl_check(client, MEIZU_MARGIN_LEFT, &data, 1);
 	if (SUCCESS == ret)
		count += sprintf(buf + count, "\n left [%d] ", data);

	ret = gtp_i2c_read_dbl_check(client, MEIZU_MARGIN_RIGHT, &data, 1);
	if (SUCCESS == ret)
		count += sprintf(buf + count, "right [%d] ", data);

	ret = gtp_i2c_read_dbl_check(client, MEIZU_MARGIN_UP, &data, 1);
	if (SUCCESS == ret)
		count += sprintf(buf + count, "up [%d] ", data);

	ret = gtp_i2c_read_dbl_check(client, MEIZU_MARGIN_DOWN, &data, 1);
	if (SUCCESS == ret)
		count += sprintf(buf + count, "down [%d] \n", data);

	return count ;
}

static ssize_t gtp_sysfs_margin_control_store(struct device *dev, struct device_attribute *attr,
				const char *buf, size_t count)
{
	...
	switch(arg1){
		...
		case MAR_BOTTOM:
			...
			break;
		default:
			pr_info("write margin mode data[ %x ] is invalid\n", arg2);
			break;
	}

	if (regaddr) {
		ret =  gtp_write_register(client, regaddr, arg2);
		if (ret < 0)
			pr_err("[%s]:margin control write [%x][%x] error\n", __func__, arg1, arg2);
	}

	return ret;
}

static ssize_t gtp_sysfs_margin_control_test_show(struct device *dev, struct device_attribute *attr,
				char *buf)
{
	int ret;
	int count = 0;
...
static ssize_t gtp_sysfs_margin_control_test_store(struct device *dev, struct device_attribute *attr,
				const char *buf, size_t count)
{
	int regaddr= 0;
	int ret = -1;
...
#ifdef MEIZU_MARGIN_CONTROL
static DEVICE_ATTR(margin_control, S_IRUGO|S_IWUSR, gtp_sysfs_margin_control_show, gtp_sysfs_margin_control_store);
static DEVICE_ATTR(margin_control_test, S_IRUGO|S_IWUSR, gtp_sysfs_margin_control_test_show, gtp_sysfs_margin_control_test_store);
#endif
...
#ifdef MEIZU_MARGIN_CONTROL
    ret |= sysfs_create_file(goodix_debug_kobj, &dev_attr_margin_control.attr);
    ret |= sysfs_create_file(goodix_debug_kobj, &dev_attr_margin_control_test.attr);
#endif
...
#ifdef MEIZU_MARGIN_CONTROL
    sysfs_remove_file(goodix_debug_kobj, &dev_attr_margin_control.attr);
    sysfs_remove_file(goodix_debug_kobj, &dev_attr_margin_control_test.attr);
#endif
}
