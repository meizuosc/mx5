...
static s32 tpd_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    if ( mz_system_lcd_id == MA01_LCD_FTS_ID ) {
	pr_err("%s: mz_system_lcd_id:  %x \n", __func__, mz_system_lcd_id);
	return -1;
    }
    ...
    ret = gtp_i2c_read_dbl_check(i2c_client_point, 0x81A1, &lcm_id, 1);
    if (ret == FAIL)
    {
        GTP_ERROR("GTP read lcm_id failed.");
    }

    ret = gtp_i2c_read_dbl_check(i2c_client_point, 0x81A0, &panel_id, 1);
    if (ret == FAIL)
    {
        GTP_ERROR("GTP read panel_id failed.");
    }
