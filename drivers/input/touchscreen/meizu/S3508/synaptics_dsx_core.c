...
static struct device_attribute attrs[] = {
	...
	__ATTR(mode, (S_IRUGO | S_IWGRP|S_IWUSR),
			...
			synaptics_rmi4_store_error),
#ifdef M85_S3508_GESTURE_SUPPORT
		...
		struct synaptics_rmi4_fn *fhandler)
{
  ...
  if(rmi4_data->gesture_enable){
  		info_printk("handler gesture begin\n");
			...
			unsigned long action, void *data)
{
	...
	info_printk("hall_notify action[%ld]mode[%d]\n",action,rmi4_data->mode);
...
static int synaptics_rmi4_set_input_dev(struct synaptics_rmi4_data *rmi4_data)
{
	...
	retval = meizu_sysfslink_register_n(&rmi4_data->input_dev->dev, "tp");
	if (retval < 0)
		dev_err(&rmi4_data->input_dev->dev, "could not create meizu sysfs files\n");
