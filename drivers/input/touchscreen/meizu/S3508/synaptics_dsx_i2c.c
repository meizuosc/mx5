			 ...
			 unsigned short addr, unsigned char *data, unsigned short length)
{
	 ...
	 if(!gprDMABuf_va){
	   gprDMABuf_va = (u8 *)dma_alloc_coherent(&rmi4_data->i2c_client->dev, I2C_DMA_RBUF_SIZE, &gprDMABuf_pa, GFP_KERNEL);
	   if(!gprDMABuf_va){
		 ...
		 unsigned short addr, unsigned char *data, unsigned short length)
 {
	 ...
	 if(!gpwDMABuf_va){
	   gpwDMABuf_va = (u8 *)dma_alloc_coherent(&rmi4_data->i2c_client->dev, I2C_DMA_WBUF_SIZE, &gpwDMABuf_pa, GFP_KERNEL);
	   if(!gpwDMABuf_va){
		 printk("[Error] Allocate DMA I2C Buffer failed!\n");
	   }
	 }
...
static unsigned char rmi4_button_codes[] = {KEY_HOME};
 ...
 int synaptics_rmi4_i2c_remove(struct i2c_client *client)
{
	...
	kfree(read_msg) ;
	if(gpwDMABuf_va){ 
		 dma_free_coherent(&client->dev, I2C_DMA_WBUF_SIZE, gpwDMABuf_va, gpwDMABuf_pa); 
		 gpwDMABuf_va = NULL; 
		 gpwDMABuf_pa = NULL; 
	}
	if(gprDMABuf_va){ 
		 dma_free_coherent(&client->dev, I2C_DMA_RBUF_SIZE, gprDMABuf_va, gprDMABuf_pa); 
		 gprDMABuf_va = NULL; 
		 gprDMABuf_pa = NULL; 
	}
	return 0;
