...
#ifdef CONFIG_OF 
    enable_irq(mt_gpio_to_irq(CUST_EINT_TOUCH_INT_NUM));
#else 
...
static irqreturn_t tpd_irq_handler(int irq,void *arg)
{
	tpd_flag = 1 ;
	dbg_printk("+++\n");
	wake_up_interruptible(&waiter);	 
...
static void hw_register_irq(void)
{
    ...
    info_printk("CUST_EINT_TOUCH_INT_NUM(%d)\n",CUST_EINT_TOUCH_INT_NUM);
#ifdef CONFIG_OF
	request_irq(mt_gpio_to_irq(CUST_EINT_TOUCH_INT_NUM),tpd_irq_handler,\
	IRQF_TRIGGER_LOW,"mtk-tpd",NULL);
    disable_irq(mt_gpio_to_irq(CUST_EINT_TOUCH_INT_NUM));
	
#else 
	mt_eint_registration(CUST_EINT_TOUCH_INT_NUM, EINTF_TRIGGER_LOW, tpd_eint_handler, 0);	
	mt_eint_mask(CUST_EINT_TOUCH_INT_NUM);
#endif
...
static int hw_power_onoff(int on)
{
	...
	if(on){
	 ...
	 retval |= hwPowerOn(MT6331_POWER_LDO_VMC , VOL_1800, "TPD1.8");
	 msleep(20);
	 retval = mt_set_gpio_dir(GPIO_CTP_RST_PIN, GPIO_DIR_OUT);
	 retval |=mt_set_gpio_out(GPIO_CTP_RST_PIN, GPIO_OUT_ONE);
	}else {
		retval	= hwPowerDown(MT6331_POWER_LDO_VMCH,"TPD2.8") ;
		retval |= hwPowerDown(MT6331_POWER_LDO_VMC,"TPD1.8") ;
	}
  ...
  static int hw_irq_disable(void)
{
	...
	info_printk("TPD enter sleep start(%d)\n",(0x7fffffff)&GPIO_CTP_EINT_PIN);
#ifdef CONFIG_OF  
	disable_irq(mt_gpio_to_irq(CUST_EINT_TOUCH_INT_NUM));
#else 
	mt_eint_mask(CUST_EINT_TOUCH_INT_NUM);
