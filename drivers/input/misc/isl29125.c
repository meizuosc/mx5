...
#define ALS_INFO(fmt,args...)  printk("isl29125:%s " fmt,__func__,##args)
static unsigned int work_times = 0;
...
static struct i2c_board_info i2c_devs_info[] = {
	{
	    I2C_BOARD_INFO("als", ISL29125_I2C_ADDR),
	}
};
#if MEIZU_CCM
enum als_range { 
...
#if CONFIG_MZ_MA01
static s32 XYZCCM_RangeW[3][3] ={// white board

	{	556L,	12487L,	-6272L},// X col
	{	-2853L,	16343L,	-6914L },// Y col
	{	-3411L, 1101L,  21845L}, // Z col
};

static s32 XYZCCM_RangeB[3][3] ={//black board
	{	-705L,	17989L,	-11744L},// X col
	{	-4700L,	21845L,	-11846L },// Y col
	{	3550L, -5134L,  17390L}, // Z col
};
#else
static s32 XYZCCM_RangeW[3][3] ={
	{	2404L,	13808L,	-4812L},// X col
	{	-509L,	16512L,	-3965L },// Y col
	{	-5268L, 2261L, 21845L}, // Z col
};

static s32 XYZCCM_RangeB[3][3] ={
	{	715L,	19255L,	-11147},// X col
	{	-1833L,	21845L,	-10946L },// Y col
	{	-1848L, 1498L, 14927L}, // Z col
};
#endif
...
/*
 * Check delayed work is working or not.
 */
static ssize_t show_delayed_work_status(struct device *dev, struct device_attribute *attr, char *buf)
{
	short int reg;
	struct i2c_client *client = to_i2c_client(dev);

	mutex_lock(&isl29125_info->rwlock_mutex);
	ALS_INFO("work invoked times:%u\n", work_times);
	sprintf(buf, "work times:%u\n", work_times);
	mutex_unlock(&isl29125_info->rwlock_mutex);
...
static bool first_time_enable = true;
				...
				struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if(val == 1) {
		...
		queue_delayed_work(isl29125_info->queue, &isl29125_info->sensor_dwork, msecs_to_jiffies(POLL_DELAY));	// 0ms
	    mutex_unlock(&isl29125_info->rwlock_mutex);
	} else {
	    mutex_lock(&isl29125_info->rwlock_mutex);
	    ...
	    work_times = 0;
		ALS_INFO("disable:%lu, work_times:%u\n", val, work_times);
...
{
#if CONFIG_MZ_MA01
	return sprintf(buf, "%d,%s\n", isl29125_info->tptype, (isl29125_info->tptype==1)?"Black":"White");
#else
				...
				struct device_attribute *attr, const char *buf, size_t count)
{
	...
	memset(isl29125_info->tp, '\0', sizeof(isl29125_info->tp));
   	strncpy(isl29125_info->tp, buf, sizeof(isl29125_info->tp) - 1); 
				...
				struct device_attribute *attr, const char *buf, size_t count)
{
	...
	input_report_abs(sensor_input, ABS_LUX, val);
...
{
	return sprintf(buf, "gpio state:%d\n", mt_get_gpio_out(GPIO_ALS_EINT_PIN));
}
...
static DEVICE_ATTR(als_data, ISL29125_SYSFS_PERMISSIONS , show_lux, NULL);
...
static DEVICE_ATTR(delayed_work_status, ISL29125_SYSFS_PERMISSIONS , show_delayed_work_status, NULL);



// mandatory for android
static DEVICE_ATTR(als_enable, ISL29125_SYSFS_PERMISSIONS ,isl29125_show_enable_sensor, isl29125_store_enable_sensor);
...
static DEVICE_ATTR(als_batch, ISL29125_SYSFS_PERMISSIONS ,NULL, isl29125_store_batch);
static DEVICE_ATTR(als_flush, ISL29125_SYSFS_PERMISSIONS ,NULL, isl29125_store_flush);
static DEVICE_ATTR(gpio_state, ISL29125_SYSFS_PERMISSIONS ,gpio_state_show, gpio_state_store);
...
static struct attribute *isl29125_attributes[] = {
	...
	&dev_attr_delayed_work_status.attr,
...
void initialize_isl29125(struct i2c_client *client)
{
...
#else
	// 0 2 4: black ; 1 3: white
	int lcd_id = mz_system_lcd_id & 0x07;
	if (lcd_id == 0 || lcd_id == 2 || lcd_id == 4) {
		isl29125_info->tptype = 1;
	}
#endif
	isl29125_info->record_count = 0;
	...
	i2c_smbus_write_byte_data(client, CONFIG3_REG, 0x0C);// 0x0c:interrupt pin output high; 0x1D
...
#if MEIZU_CCM
static unsigned long long lux_last = 0;
...
static void isl29125_work_handler(struct work_struct *work)
{
	...
	isl29125->lux = lux;
	// adjust before framework get in
	if (!first_time_enable && get_boot_mode() == NORMAL_BOOT) {
#if 1
		static int set_lux = -1;
		int should_set_lux = 100;

		ALS_INFO("first time enable\n");

		switch (lux) {
		case     0 ...     2: should_set_lux =   1; break;
		case     3 ...     5: should_set_lux =  10; break;
		case     6 ...    10: should_set_lux =  40; break;
		case    11 ...    20: should_set_lux =  50; break;
		case    21 ...    25: should_set_lux =  60; break;
		case    26 ...    60: should_set_lux =  70; break;
		case    61 ...    85: should_set_lux =  80; break;
		case    86 ...   110: should_set_lux =  80; break;
		case   111 ...   128: should_set_lux =  90; break;
		case   129 ...   256: should_set_lux = 100; break;
		case   257 ...   384: should_set_lux = 104; break;
		case   385 ...   512: should_set_lux = 110; break;
		case   513 ...   640: should_set_lux = 116; break;
		case   641 ...   768: should_set_lux = 122; break;
		case   769 ...   896: should_set_lux = 128; break;
		case   897 ...  1024: should_set_lux = 134; break;
		case  1025 ...  2048: should_set_lux = 182; break;
		case  2049 ...  4096: should_set_lux = 230; break;
		case  4097 ...  6144: should_set_lux = 250; break;
		case  6145 ...  8192: should_set_lux = 255; break;
		case  8193 ... 10240: should_set_lux = 255; break;
		case 10241 ... 12288: should_set_lux = 255; break;
		case 12289 ... 14336: should_set_lux = 255; break;
		case 14337 ... 16384: should_set_lux = 255; break;
		case 16385 ... 18432: should_set_lux = 255; break;
		case 18433 ... 65535: should_set_lux = 255; break;
		default:              should_set_lux = 255; break;
		}
		if (set_lux == -1)
			set_lux = should_set_lux;

		if (set_lux + 1 <= should_set_lux)
			set_lux += 1;
		else if (set_lux - 1 >= should_set_lux)
			set_lux -= 1;
		primary_display_setbacklight(set_lux);
		...
		queue_delayed_work(isl29125->queue, &isl29125->sensor_dwork, msecs_to_jiffies(isl29125->conversion_time * 4 / 3));	// restart timer
#else
		static int test = 0;
		if (++test)
			primary_display_setbacklight(1);
#endif
	} else {

#if 0
		/*
		 * average the lux reported
		 */
		isl29125->record_arry[isl29125->record_count] = isl29125->lux;
		isl29125->record_count = (isl29125->record_count+1)%ALS_SAMPLE_COUNT;
		isl29125->lux /= isl29125->record_count+1;
#endif

			if( delt_lux>0 ) { // never report the same value to hardware layer
				delt_lux = -1;
			} else {
				delt_lux = 1;
			}

		if( (isl29125->prev_lux==isl29125->lux)&&(isl29125->lux) ) {
			input_report_abs(sensor_input, ABS_LUX, isl29125->lux + delt_lux);
		} else {
		...
		queue_delayed_work(isl29125->queue, &isl29125->sensor_dwork, msecs_to_jiffies(isl29125->conversion_time * 4));	// restart timer


		/*
		 * bug:  lux dont keep changing
		 */
		if( lux_last==isl29125->lux ) {
			times++;
		} else {
			lux_last = isl29125->lux;
			times = 0;
		}
		if(  times>=10 ) { // 10*isl29125->conversion_time * 4 == 10*100*4 == 4 seconds
			ALS_INFO("lux(%llu) dont keep changing for (%u)-times, something wrong\n",isl29125->lux, times);
			ALS_INFO("R=%lu, G=%lu, B=%lu, GIR=%lu, CCT=%d, LUX=%llu, cofe %ld\n",
			...
			/*
			 * Reset or Reinit here ?
			 */
		}

		/*
		 * is delayed work still working ?
		 */
		work_times++;
		/*
...
static int isl_late_resume(struct early_suspend *h)
{
	...
	if (isl29125_info->sensor_enable) {
		queue_delayed_work(isl29125_info->queue, &isl29125_info->sensor_dwork, msecs_to_jiffies(POLL_DELAY));
		ALS_INFO("isl queue work\n");
	}
...
static int isl_sensor_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if(ret) {                                                                   
		printk(KERN_ERR "%s: Failed to create input\n", __FUNCTION__);                    
		goto err;                                                           
	}                                                                           
	...
	meizu_sysfslink_register(&client->dev);
	...
	isl29125->queue = create_singlethread_workqueue("isl29125wq");
	// schedule now
	if (!first_time_enable && get_boot_mode() == NORMAL_BOOT)
		queue_delayed_work(isl29125->queue, &isl29125_info->sensor_dwork, 100);	// 0ms
