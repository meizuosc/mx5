...
static enum eint_index no_wakelock_index[] = {EINT_WIFI};
...
unsigned int pre_hour = 0;
extern unsigned int rt_hour;
...
static bool check_need_wakelock(unsigned int index)
{
	int i,size = sizeof(no_wakelock_index)/sizeof(no_wakelock_index[0]);
	for(i = 0; i < size; i++)
		if(index == no_wakelock_index[i])
			return false;
	return true;
}
...
{
    if(eint_num < EINT_MAX_CHANNEL)
        return EINT_FUNC.wakeup_count[eint_num];

    return 0;
}

unsigned int mt_eint_get_count_today(unsigned int eint_num)
{
    if(eint_num < EINT_MAX_CHANNEL)
        return EINT_FUNC.cnt_today[eint_num];
...
static irqreturn_t mt_eint_demux(unsigned irq, struct irq_desc *desc)
{
    ...
    for (reg_base = 0; reg_base < EINT_MAX_CHANNEL; reg_base+=32) {
        ...
        for(offset = 0; offset < 32; offset++){
            ...
            if (status_check) {
                dbgmsg("Got eint:%d\n",index);
                if(EINT_FUNC.need_wakelock[index]){
			dbgmsg("check_need_wakelock EINT%d\n", index);
			tasklet_schedule(&eint_tasklet);
		}
...
static ssize_t eint_count_all_show(struct device_driver *driver, char *buf)
{
	...
	if(mt_eint_get_count(i))
                ptr += sprintf(ptr, "eint %d, \tmask=0x%x, \tcnt_all=%d, \twakeup_cnt=%d, \tcnt_today=%d \n",
                        i,
			mt_eint_get_mask(i),
                        mt_eint_get_count(i),
			mt_eint_get_wakeup_count(i),
			mt_eint_get_count_today(i));
    }

    return ptr-buf;
}

DRIVER_ATTR(eint_count, 0644, eint_count_all_show, NULL);
...
void mt_eint_print_status(void)
{
    ...
    if (rt_hour < pre_hour && pre_hour > 0) {
	for(index = 0; index < EINT_MAX_CHANNEL; index++)
		EINT_FUNC.cnt_today[index] = 0;
    }
    pre_hour = rt_hour;
    ...
    printk(KERN_DEBUG"EINT_STA:");
     for (reg_base = 0; reg_base < EINT_MAX_CHANNEL; reg_base+=32) {
            ...
            for(offset = 0; offset < 32; offset++){
                ...
                if (status_check)
		{
                    printk(KERN_ERR"EINT %d is pending\n",index);
                    EINT_FUNC.wakeup_count[index]++;
		    EINT_FUNC.cnt_today[index]++;
                }
            }
     }
     printk(KERN_DEBUG"\n");
...
static int __init mt_eint_init(void)
{
	...
	if (of_property_read_u32(node, "max_eint_num", &EINT_MAX_CHANNEL)) {
		...
		EINT_FUNC.eint_func = (eint_funct *) kmalloc(sizeof(eint_funct)*EINT_MAX_CHANNEL, GFP_KERNEL);
        	...
        	///{@ MEIZU_BSP
        	EINT_FUNC.wakeup_count = (unsigned int *) kmalloc(sizeof(unsigned int)*EINT_MAX_CHANNEL, GFP_KERNEL);
		EINT_FUNC.cnt_today = (unsigned int *) kmalloc(sizeof(unsigned int)*EINT_MAX_CHANNEL, GFP_KERNEL);
		EINT_FUNC.need_wakelock= (unsigned int *) kmalloc(sizeof(unsigned int)*EINT_MAX_CHANNEL, GFP_KERNEL);
        	///@}
    ...
    for (i = 0; i < EINT_MAX_CHANNEL; i++) {
        ...
        ///{@ MEIZU_BSP
	EINT_FUNC.count[i] = 0;
        EINT_FUNC.wakeup_count[i] = 0;
        EINT_FUNC.cnt_today[i] = 0;
        ///@}
        if(check_need_wakelock(i))
		EINT_FUNC.need_wakelock[i] = 1;
	else
		EINT_FUNC.need_wakelock[i] = 0;
    ...
    ret |= driver_create_file(&eint_driver.driver, &driver_attr_eint_count);
    if (ret) {
