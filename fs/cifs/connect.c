...
#define RECONNECT_TIME_SCREEN_ON  (3000)/* 3s*/ 
#define RECONNECT_TIME_SCREEN_OFF  (5*60*1000)/*5mins */

extern int cifs_is_lcd_off(void);

void cifs_reconnect_sleep(void)
{
	int retry = RECONNECT_TIME_SCREEN_OFF/RECONNECT_TIME_SCREEN_ON ;

	do{
		if(cifs_is_lcd_off()){
			msleep(RECONNECT_TIME_SCREEN_ON);
			retry--;
		}else{ 
			msleep(RECONNECT_TIME_SCREEN_ON);
			retry = 0;
...
cifs_reconnect(struct TCP_Server_Info *server)
{
	...
	do {
		...
		if (rc) {
			printk(KERN_INFO"%s:reconnect error %d\n",__FUNCTION__, rc);
			cifs_reconnect_sleep();
		} else {
