                                   ...
                                   struct snd_pcm_hw_params *hw_params)
{
    ...
    // -------------------------------------------------------
    printk("%s dma_bytes = %zu dma_area = %p dma_addr = 0x%x\n",__func__,
...
static int mtk_dl1bt_probe(struct platform_device *pdev)
{
    ...
    if (pdev->dev.of_node)
    {
        dev_set_name(&pdev->dev, "%s", MT_SOC_VOIP_BT_AND_SPEAKER_OUT);
    }
...
{
    { .compatible = "mediatek,mt_soc_pcm_dl1_bt_i2s0", },
    {}
};
#endif
...
{
    .driver = {
        .name = MT_SOC_VOIP_BT_AND_SPEAKER_OUT,
        .owner = THIS_MODULE,
#ifdef CONFIG_OF
...
static int __init mtk_soc_dl1bt_platform_init(void)
{
    ...
    PRINTK_AUDDRV("%s \n", __func__);
#ifndef CONFIG_OF
    soc_mtk_dl1bt_dev = platform_device_alloc(MT_SOC_VOIP_BT_AND_SPEAKER_OUT, -1);
