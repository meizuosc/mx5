...
static const char *Audio_Debug_Setting[] = {"Off", "Speaker", "Headphone", "Earpiece"};
...
static int AudioDebug_Setting_Set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
    ...
    output_device = ucontrol->value.enumerated.item[0];
    if (output_device == 1) /* Enable SideTone for Speaker */
        EnableSideGenHw(Soc_Aud_InterConnectionOutput_O00, Soc_Aud_MemIF_Direction_DIRECTION_OUTPUT, true);
    else /* Enable Sindtone for Headphone & Earpiece */
        EnableSideGenHw(Soc_Aud_InterConnectionOutput_O03, Soc_Aud_MemIF_Direction_DIRECTION_OUTPUT, true);
    msleep(5 * 1000);
    if (output_device == 1) /* Disable SidTone for Speaker */
        EnableSideGenHw(Soc_Aud_InterConnectionOutput_O00, Soc_Aud_MemIF_Direction_DIRECTION_OUTPUT, false);
    else /* Disable SindTone for Headphone & Earpiece */
